...
args+=( "-kubeconfig=${KUBECONFIG:-/home/khulnasoft/.kube/config}" )
args+=( "-namespace=${NAMESPACE:-default}" )
args+=( "-timeout=120m" )

if [[ "${KHULNASOFT_REPO_ROOT:-}" != "" ]]; then
  echo "Running in Khulnasoft workspace. Fetching USER_NAME and USER_TOKEN"
