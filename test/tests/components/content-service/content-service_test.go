...
import (
	...
	content_service_api "github.com/khulnasoft-com/khulnasoft/content-service/api"
	"github.com/khulnasoft-com/khulnasoft/test/pkg/integration"
)

var (
	khulnasoftBuiltinUserID = "00000000-0000-0000-0000-000000000000"
)
...
func TestUploadUrl(t *testing.T) {
	tests := []struct {
		...
		{
			Name:         "simple name",
			InputOwnerID: khulnasoftBuiltinUserID,
			InputName:    "test-blob",
		},
		{
			Name:         "new user",
...
func TestUploadDownloadBlob(t *testing.T) {
	f := features.New("UploadDownloadBlob").
		...
		Assess("it should upload and download blob", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			resp, err := bs.UploadUrl(ctx, &content_service_api.UploadUrlRequest{OwnerId: khulnasoftBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
			...
			resp2, err := bs.DownloadUrl(ctx, &content_service_api.DownloadUrlRequest{OwnerId: khulnasoftBuiltinUserID, Name: "test-blob"})
			if err != nil {
				t.Fatal(err)
			}
