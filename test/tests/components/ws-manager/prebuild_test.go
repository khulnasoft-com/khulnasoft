	...
	f := features.New("prebuild").
		...
		Assess("it should create a prebuild and succeed the defined tasks", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			tests := []struct {
				...
				Task             []khulnasoft.TasksItems
				FF               []wsmanapi.WorkspaceFeatureFlag
			}{
				{
					Name:             "classic",
					ContextURL:       "https://github.com/khulnasoft-com/empty",
...
const (
	prebuildLogPath string = "/workspace/.khulnasoft"
		...
		Assess("it should open workspace from prebuild successfully", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			for _, test := range tests {
				...
				t.Run(test.Name, func(t *testing.T) {
					...
					// check the folder permission is khulnasoft
					checkFolderPermission(t, rsa, "/workspace")

					// check the files/folders permission under .git/ is khulnasoft
...
// - make sure the workspace's init task ran (the init task will create a file `incremental.txt` see https://github.com/khulnasoft-com/test-incremental-workspace/blob/main/.khulnasoft.yml)
	...
	f := features.New("prebuild").
		...
		Assess("it should open a workspace from with an older prebuild initializer successfully and run the init task", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			tests := []struct {
				...
				{
					Name:                    "classic",
					RemoteUri:               "https://github.com/khulnasoft-com/test-incremental-workspace",
...
func checkPrebuildLogExist(t *testing.T, cfg *envconf.Config, rsa *integration.RpcClient, ws *integration.LaunchWorkspaceDirectlyResult, wsRoot string) {
	...
	// we fall back to check the init task message within the /workspace/.khulnasoft/prebuild-log-* or not
	...
	// somehow, the init task message does not exist within the /workspace/.khulnasoft/prebuild-log-*
...
// checkFolderPermission checks the folder UID and GID is khulnasoft
...
// checkGitFolderPermission checks the files/folders UID and GID under .git/ is khulnasoft
