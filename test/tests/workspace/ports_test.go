...
func TestRegularWorkspacePorts(t *testing.T) {
	...
	// This branch exposes a python server on port 3000 as part of the Khulnasoft tasks.
	testRepo := "https://github.com/khulnasoft-com/khulnasoft-test-repo/tree/integration-test/ports"
	testRepoName := "khulnasoft-test-repo"
	...
	f := features.New("ports").
		...
		Assess("it can open and access workspace ports", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			serverOpts := []integration.KhulnasoftServerOpt{integration.WithKhulnasoftUser(username)}
			server, err := api.KhulnasoftServer(serverOpts...)
			if err != nil {
				t.Fatal(err)
			}

			// Must change supervisor address from localhost to 10.0.5.2.
			err = server.SetEnvVar(ctx, &khulnasoft.UserEnvVarValue{
				Name:              "SUPERVISOR_ADDR",
				Value:             `10.0.5.2:22999`,
				RepositoryPattern: "khulnasoft-com/" + testRepoName,
			})
			if err != nil {
				t.Fatal(err)
			}
			defer func() {
				err := server.DeleteEnvVar(ctx, &khulnasoft.UserEnvVarValue{
					Name:              "SUPERVISOR_ADDR",
					RepositoryPattern: "khulnasoft-com/" + testRepoName,
				})
				if err != nil {
			...
			nfo, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, testRepo, username, api, integration.WithKhulnasoftUser(username))
			if err != nil {
				t.Fatal(err)
			}
