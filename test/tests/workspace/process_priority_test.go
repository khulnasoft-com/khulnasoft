...
func TestProcessPriority(t *testing.T) {
	...
	f := features.New("process priority").
		...
		Assess("it has set process priority", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			nfo, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, "https://github.com/khulnasoft-com/empty", username, api, integration.WithKhulnasoftUser(username))
			if err != nil {
				t.Fatal(err)
			}
...
func determinePriority(process string) (int, error) {
	...
	if strings.HasSuffix(process, "/ide/bin/khulnasoft-code") {
		return -10, nil
	}
