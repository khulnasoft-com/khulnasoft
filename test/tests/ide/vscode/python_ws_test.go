...
func TestPythonExtWorkspace(t *testing.T) {
	...
	f := features.New("PythonExtensionWorkspace").
		...
		Assess("it can run python extension in a workspace", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			nfo, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, "github.com/khulnasoft-com/python-test-workspace", username, api)
			if err != nil {
				t.Fatal(err)
			}
			defer func() {
			...
			secretKey, err := api.CreateKhulnasoftOneTimeSecret(fmt.Sprintf("%x", hash))
			if err != nil {
				t.Fatal(err)
			}

			sessionCookie, err := api.KhulnasoftSessionCookie(userId, secretKey)
			if err != nil {
				t.Fatal(err)
			}
