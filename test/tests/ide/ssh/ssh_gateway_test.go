...
func TestSSHGatewayConnection(t *testing.T) {
	...
	f := features.New("TestSSHGatewayConnection").
		...
		Assess("it can connect to a workspace via SSH gateway", func(testCtx context.Context, t *testing.T, cfg *envconf.Config) context.Context {
			...
			// This env var caused an incident https://www.khulnasoftstatus.com/incidents/26gwnhcpvqqx before
			// Which was introduced by PR https://github.com/khulnasoft-com/khulnasoft/pull/13822
			// And fixed by PR https://github.com/khulnasoft-com/khulnasoft/pull/13858
			_ = server.SetEnvVar(ctx, &khulnasoft.UserEnvVarValue{
				Name:              "TEST",
				RepositoryPattern: "*/*",
				Value:             "\\\"test space\\\"",
			})
...
World`,
			...
			nfo, stopWs, err := integration.LaunchWorkspaceFromContextURL(t, ctx, "github.com/khulnasoft-com/empty", username, api)
			if err != nil {
				t.Fatal(err)
			}
			defer func() {
