...
This directory contains Khulnasoft's integration tests, including the framework that makes them possible.

Integration tests work by instrumenting Khulnasoft's components to modify and verify its state.
...
| start workspace | obtaining a Khulnasoft API token, calling "createWorkspace" and watching for successful startup events.                                       |
...
- DB access to the Khulnasoft DB

# Running the tests

## Automatically at Khulnasoft
...
You may want to run tests to assert whether a Khulnasoft installation is successfully integrated.
...
This is best for when you're actively developing Khulnasoft.

Test will work if images that they use are already cached by Khulnasoft instance. If not, they might fail if it takes too long to pull an image.
...
2. Tests that require correct user (user should have github OAuth integration setup with khulnasoft). Run those with `-username=<khulnasoft_username>`. Make sure to load https://github.com/khulnasoft-com/khulnasoft-test-repo and https://github.com/khulnasoft-com/khulnasoft workspaces inside your khulnasoft that you are testing to preload those images onto your node. Wait for it to finish pulling those image, this will ensure that test will not fail due to timeout while waiting to pull an image for the first time.
...
2. In your Khulnasoft (preview) environment, log into the preview environment, set `USER_NAME` to the user you logged in with, and set `USER_TOKEN` to any (does not have to be valid).
...
go test -v ./... \
    -run <test> \
    -namespace=default \
    -username=<khulnasoft_user_with_oauth_setup> \
