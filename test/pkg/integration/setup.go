...
func Setup(ctx context.Context) (string, string, env.Environment, bool, string, bool) {
	var (
		...
		enterprise      bool
		gitlab          bool
		waitKhulnasoftReady time.Duration
	...
	if defaultKubeConfig == "" {
		defaultKubeConfig = "/home/khulnasoft/.kube/config"
	}
	...
	flagset.DurationVar(&waitKhulnasoftReady, "wait-khulnasoft-timeout", 5*time.Minute, `wait time for Khulnasoft components before starting integration test`)
	...
	testenv.Setup(
		waitOnKhulnasoftRunning(e.Namespace(), waitKhulnasoftReady),
	)
...
func waitOnKhulnasoftRunning(namespace string, waitTimeout time.Duration) env.Func {
	klog.V(2).Info("Checking status of Khulnasoft components...")
	return func(ctx context.Context, cfg *envconf.Config) (context.Context, error) {
		...
		err := wait.PollImmediate(1*time.Second, waitTimeout, func() (bool, error) {
			...
			klog.V(2).Info("All Khulnasoft components are running...")
			return true, nil
		})
		if err != nil {
			return ctx, nil
...
func logKhulnasoftStatus(t *testing.T, client klient.Client, namespace string) {
	var allPods corev1.PodList
	err := client.Resources(namespace).List(context.Background(), &allPods)
	if err != nil {
		t.Logf("failed to list pods to log khulnasoft status: %v", err)
		return
	}
	...
	tw.Flush()
	t.Logf("Khulnasoft components status:\n" + buf.String())
}
