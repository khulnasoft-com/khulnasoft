...
import (
	...
	ide "github.com/khulnasoft-com/khulnasoft/ide-service-api/config"
...
const (
	khulnasoftBuiltinUserID             = "builtin-user-workspace-probe-0000000"
...
func LaunchWorkspaceDirectly(t *testing.T, ctx context.Context, api *ComponentAPI, opts ...LaunchWorkspaceDirectlyOpt) (*LaunchWorkspaceDirectlyResult, StopWorkspaceFunc, error) {
	...
	options := launchWorkspaceDirectlyOptions{
		BaseImage: "docker.io/khulnasoft/workspace-full:latest",
	}
	for _, o := range opts {
		err := o(&options)
		if err != nil {
		...
		Spec: &wsmanapi.StartWorkspaceSpec{
			...
			Envvars: []*wsmanapi.EnvironmentVariable{
				...
				// from ws-manager in these tests we need to set it here ourselves.
				{
					...
					Value: "https://open-vsx.khulnasoft.com/",
				},
			},
		},
	}
...
// LaunchWorkspaceFromContextURL force-creates a new workspace using the Khulnasoft server API,
...
func LaunchWorkspaceFromContextURL(t *testing.T, ctx context.Context, contextURL string, username string, api *ComponentAPI, serverOpts ...KhulnasoftServerOpt) (*protocol.WorkspaceInfo, StopWorkspaceFunc, error) {
...
// LaunchWorkspaceWithOptions force-creates a new workspace using the Khulnasoft server API,
// and waits for the workspace to start. If any step along the way fails, this function will
// fail the test.
//
// When possible, prefer the less complex LaunchWorkspaceDirectly.
func LaunchWorkspaceWithOptions(t *testing.T, ctx context.Context, opts *LaunchWorkspaceOptions, username string, api *ComponentAPI, serverOpts ...KhulnasoftServerOpt) (*protocol.WorkspaceInfo, StopWorkspaceFunc, error) {
	var (
		defaultServerOpts []KhulnasoftServerOpt
		stopWs            StopWorkspaceFunc = nil
		err               error
	)

	if username != "" {
		defaultServerOpts = []KhulnasoftServerOpt{WithKhulnasoftUser(username)}
	}
	...
	server, err := api.KhulnasoftServer(append(defaultServerOpts, serverOpts...)...)
	if err != nil {
	...
	for i := 0; i < 3; i++ {
		...
		if err != nil {
			...
			if scode == codes.NotFound || scode == codes.Unavailable {
				t.Log("retry strarting a workspace because cannnot start workspace: %w", err)
				time.Sleep(1 * time.Second)
				api.ClearKhulnasoftServerClientCache()
				server, err = api.KhulnasoftServer(append(defaultServerOpts, serverOpts...)...)
				if err != nil {
					return nil, nil, xerrors.Errorf("cannot start server: %w", err)
				}
