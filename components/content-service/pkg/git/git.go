...
type Client struct {
	...
	// Config values to be set on clone provided through `.khulnasoft.yml`
	...
	// if true will run git command as khulnasoft user (should be executed as root that has access to sudo in this case)
	RunAsKhulnasoftUser bool
...
func (c *Client) GitWithOutput(ctx context.Context, ignoreErr *string, subcommand string, args ...string) (out []byte, err error) {
	...
	env = append(env, "HOME=/home/khulnasoft")
	...
	cmdName := "git"
	if c.RunAsKhulnasoftUser {
		cmdName = "sudo"
		fullArgs = append([]string{"-u", "khulnasoft", "git"}, fullArgs...)
	}
