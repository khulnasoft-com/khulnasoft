...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/content-service/pkg/archive"
	"github.com/khulnasoft-com/khulnasoft/content-service/pkg/git"
	"github.com/khulnasoft-com/khulnasoft/content-service/pkg/storage"
)

const (
	// WorkspaceReadyFile is the name of the ready file we're placing in a workspace
	WorkspaceReadyFile = ".khulnasoft/ready"

	// KhulnasoftUID is the user ID of the khulnasoft user
	KhulnasoftUID = 33333

	// KhulnasoftGID is the group ID of the khulnasoft user group
	KhulnasoftGID = 33333
...
type NewFromRequestOpts struct {
	// ForceKhulnasoftUserForGit forces khulnasoft:khulnasoft ownership on all files produced by the Git initializer.
	// For FWB workspaces the content init is run from supervisor which runs as UID 0. Using this flag, the
	// Git content is forced to the Khulnasoft user. All other content (backup, prebuild, snapshot) will already
	// have the correct user.
	ForceKhulnasoftUserForGit bool
}
...
func NewFromRequest(ctx context.Context, loc string, rs storage.DirectDownloader, req *csapi.WorkspaceInitializer, opts NewFromRequestOpts) (i Initializer, err error) {
	...
	if _, ok := spec.(*csapi.WorkspaceInitializer_Empty); ok {
		...
		initializer, err = newGitInitializer(ctx, loc, ir.Git, opts.ForceKhulnasoftUserForGit)
		...
		for _, gi := range ir.Prebuild.Git {
			gitinit, err := newGitInitializer(ctx, loc, gi, opts.ForceKhulnasoftUserForGit)
			if err != nil {
				return nil, err
			}
...
// Returns gRPC errors.
func newGitInitializer(ctx context.Context, loc string, req *csapi.GitInitializer, forceKhulnasoftUser bool) (*GitInitializer, error) {
	if req.Config == nil {
	...
	return &GitInitializer{
		Client: git.Client{
			...
			RunAsKhulnasoftUser:   forceKhulnasoftUser,
...
// Some workspace content may have a `/dst/.khulnasoft` file or directory. That would break
// the workspace ready file placement (see https://github.com/khulnasoft-com/khulnasoft/issues/7694).
// This function ensures that workspaces do not have a `.khulnasoft` file or directory present.
func EnsureCleanDotKhulnasoftDirectory(ctx context.Context, wspath string) error {
	...
	dotKhulnasoft := filepath.Join(wspath, ".khulnasoft")
	stat, err := os.Stat(dotKhulnasoft)
	...
	candidateFN := filepath.Join(wspath, ".khulnasoft.yaml")
	if _, err := os.Stat(candidateFN); err == nil {
		// Our candidate file already exists, hence we cannot just move things.
		// As fallback we'll delete the .khulnasoft entry.
		return os.RemoveAll(dotKhulnasoft)
	}

	err = mv(dotKhulnasoft, candidateFN)
	if err != nil {
		return err
	}
...
func PlaceWorkspaceReadyFile(ctx context.Context, wspath string, initsrc csapi.WorkspaceInitSource, metrics csapi.InitializerMetrics, uid, gid int) (err error) {
	...
	khulnasoftDir := filepath.Join(wspath, filepath.Dir(WorkspaceReadyFile))
	err = os.MkdirAll(khulnasoftDir, 0777)
	if err != nil {
		return xerrors.Errorf("cannot create directory for workspace ready file: %w", err)
	}
	err = os.Chown(khulnasoftDir, uid, gid)
	if err != nil {
