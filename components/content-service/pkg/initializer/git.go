...
type GitInitializer struct {
	...
	// If true, the Git initializer will chown(khulnasoft) after the clone
	Chown bool
}
...
func (ws *GitInitializer) Run(ctx context.Context, mappings []archive.IDMapping) (src csapi.WorkspaceInitSource, stats csapi.InitializerMetrics, err error) {
	...
	gitClone := func() error {
		...
		// make sure that folder itself is owned by khulnasoft user prior to doing git clone
		// this is needed as otherwise git clone will fail if the folder is owned by root
		if ws.RunAsKhulnasoftUser {
			args := []string{"khulnasoft", ws.Location}
		...
		if err != nil {
			if strings.Contains(err.Error(), "Access denied") {
				err = &backoff.PermanentError{
					Err: fmt.Errorf("Access denied. Please check that Khulnasoft was given permission to access the repository"),
				}
			}
	...
	defer func() {
		...
		args := []string{"-R", "-L", "khulnasoft", ws.Location}
		cmd := exec.Command("chown", args...)
		res, cerr := cmd.CombinedOutput()
		if cerr != nil && !process.IsNotChildProcess(cerr) {
			err = git.OpFailedError{
