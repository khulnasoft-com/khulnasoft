...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/common-go/watch"
	khulnasoftapi "github.com/khulnasoft-com/khulnasoft/khulnasoft-protocol"
	api "github.com/khulnasoft-com/khulnasoft/ide-service-api"
	"github.com/khulnasoft-com/khulnasoft/ide-service-api/config"
...
func (s *IDEServiceServer) ResolveWorkspaceConfig(ctx context.Context, req *api.ResolveWorkspaceConfigRequest) (resp *api.ResolveWorkspaceConfigResponse, err error) {
	...
	if os.Getenv("CONFIGCAT_SDK_KEY") != "" {
		resp.Envvars = append(resp.Envvars, &api.EnvironmentVariable{
			Name:  "KHULNASOFT_CONFIGCAT_ENABLED",
			Value: "true",
		})
	}

	var wsConfig *khulnasoftapi.KhulnasoftConfig
	...
	if req.Type == api.WorkspaceType_REGULAR {
		...
		if preferToolbox {
			preferToolboxEnv := api.EnvironmentVariable{
				Name:  "KHULNASOFT_PREFER_TOOLBOX",
				Value: "true",
			}
			debuggingEnv := api.EnvironmentVariable{
				Name:  "KHULNASOFT_TOOLBOX_DEBUGGING",
...
func getPrebuilds(config *khulnasoftapi.KhulnasoftConfig, alias string) *khulnasoftapi.Prebuilds {
...
func getProductConfig(config *khulnasoftapi.KhulnasoftConfig, alias string) *khulnasoftapi.JetbrainsProduct {
	defer func() {
	...
	productConfig, ok := v.(*khulnasoftapi.JetbrainsProduct)
	if !ok {
		return nil
	}
