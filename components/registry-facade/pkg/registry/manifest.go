...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/registry-facade/api"
)
...
func (reg *Registry) handleManifest(ctx context.Context, r *http.Request) http.Handler {
	...
	if err != nil {
		...
		// ref: https://github.com/khulnasoft-com/khulnasoft/blob/1a3c4b0bb6f13fe38481d21ddd146747c1a1935f/components/node-labeler/cmd/run.go#L291
...
func (mh *manifestHandler) getManifest(w http.ResponseWriter, r *http.Request) {
	...
	err := func() error {
		...
		case images.MediaTypeDockerSchema2Manifest, ociv1.MediaTypeImageManifest:
			...
			manifest.Annotations["io.khulnasoft.workspace-image.size"] = strconv.Itoa(originImageSize)
			manifest.Annotations["io.khulnasoft.workspace-image.ref"] = mh.Spec.BaseRef
			...
			//
			// fixes https://github.com/khulnasoft-com/khulnasoft/pull/3397
