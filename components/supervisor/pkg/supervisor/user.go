...
func AddKhulnasoftUserIfNotExists() error {
	ok, err := hasGroup(khulnasoftGroupName, khulnasoftGID)
	if err != nil {
		return err
	}
	if !ok {
		err = addGroup(khulnasoftGroupName, khulnasoftGID)
		if err != nil {
			return err
		}
	}
	if err := addSudoer(khulnasoftGroupName); err != nil {
		log.WithError(err).Error("add khulnasoft sudoers")
	}

	targetUser := &user.User{
		Uid:      strconv.Itoa(khulnasoftUID),
		Gid:      strconv.Itoa(khulnasoftGID),
		Username: khulnasoftUserName,
		HomeDir:  "/home/" + khulnasoftUserName,
	}
...
func hasGroup(name string, gid int) (bool, error) {
	...
	if grpByID == nil && grpByName != nil {
		...
		return true, xerrors.Errorf("group named %s exists but uses different GID %s, should be: %d", name, grpByName.Gid, khulnasoftGID)
	}
...
func hasUser(u *user.User) (bool, error) {
	...
	if userByID == nil && userByName != nil {
		...
		return true, xerrors.Errorf("user named %s exists but uses different UID %s, should be: %d", u.Username, userByName.Uid, khulnasoftUID)
	}
...
func addSudoer(group string) error {
	...
	khulnasoftSudoer := []byte(fmt.Sprintf("%%%s ALL=NOPASSWD:ALL", group))
	// Line starts with "%khulnasoft ..."
	re := regexp.MustCompile(fmt.Sprintf("(?m)^%%%s\\s+.*?$", group))
	if len(re.FindStringIndex(string(b))) > 0 {
		nb := re.ReplaceAll(b, khulnasoftSudoer)
