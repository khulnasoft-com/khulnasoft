...
//   2. IDE: Khulnasoft supports different IDEs, all of which have different configuration needs.
//   3. DesktopIDE: Khulnasoft supports to connect external IDEs (like desktop IDEs).
...
type WorkspaceConfig struct {
	...
	WorkspaceContextURL string `env:"KHULNASOFT_WORKSPACE_CONTEXT_URL"`

	// WorkspaceUrl is an URL for which workspace is accessed.
	WorkspaceUrl string `env:"KHULNASOFT_WORKSPACE_URL"`

	// WorkspaceClass denotes the class of the workspace
	WorkspaceClass string `env:"KHULNASOFT_WORKSPACE_CLASS"`

	// WorkspaceClassInfo denotes the detail of workspace class
	WorkspaceClassInfo *WorkspaceClassInfo `env:"KHULNASOFT_WORKSPACE_CLASS_INFO"`

	// DefaultWorkspaceImage is the default image of current workspace
	DefaultWorkspaceImage string `env:"KHULNASOFT_DEFAULT_WORKSPACE_IMAGE"`
	...
	RepoRoot string `env:"KHULNASOFT_REPO_ROOT"`
	...
	RepoRoots string `env:"KHULNASOFT_REPO_ROOTS"`
	...
	PreventMetadataAccess bool `env:"KHULNASOFT_PREVENT_METADATA_ACCESS"`
	...
	GitUsername string `env:"KHULNASOFT_GIT_USER_NAME"`
	// GitEmail makes supervisor configure the global user.email Git setting.
	GitEmail string `env:"KHULNASOFT_GIT_USER_EMAIL"`

	// CommitAnnotationEnabled controls whether to annotate commits with the Khulnasoft instance host
	CommitAnnotationEnabled bool `env:"KHULNASOFT_COMMIT_ANNOTATION_ENABLED"`

	// Tokens is a JSON encoded list of WorkspaceKhulnasoftToken
	Tokens string `env:"THEIA_SUPERVISOR_TOKENS"`

	// WorkspaceID is the ID of the workspace
	WorkspaceID string `env:"KHULNASOFT_WORKSPACE_ID"`

	// WorkspaceInstanceID is the instance ID of the workspace
	WorkspaceInstanceID string `env:"KHULNASOFT_INSTANCE_ID"`

	// KhulnasoftHost points to the Khulnasoft API server we're to talk to
	KhulnasoftHost string `env:"KHULNASOFT_HOST"`

	// KhulnasoftTasks is the task configuration of the workspace
	KhulnasoftTasks string `env:"KHULNASOFT_TASKS"`

	// KhulnasoftHeadless controls whether the workspace is running headless
	KhulnasoftHeadless string `env:"KHULNASOFT_HEADLESS"`
	...
	WorkspaceContext string `env:"KHULNASOFT_WORKSPACE_CONTEXT"`

	// WorkspaceClusterHost is a host under which this workspace is served, e.g. ws-eu11.khulnasoft.com
	WorkspaceClusterHost string `env:"KHULNASOFT_WORKSPACE_CLUSTER_HOST"`
	...
	TerminationGracePeriodSeconds *int `env:"KHULNASOFT_TERMINATION_GRACE_PERIOD_SECONDS"`

	// OwnerId is the user id who owns the workspace
	OwnerId string `env:"KHULNASOFT_OWNER_ID"`
	...
	ConfigcatEnabled bool `env:"KHULNASOFT_CONFIGCAT_ENABLED"`

	SSHGatewayCAPublicKey string `env:"KHULNASOFT_SSH_CA_PUBLIC_KEY"`

	// Comma-separated list of host:<base64ed user:password> pairs to authenticate against docker registries
	KhulnasoftImageAuth string `env:"KHULNASOFT_IMAGE_AUTH"`
}

// WorkspaceKhulnasoftToken is a list of tokens that should be added to supervisor's token service.
type WorkspaceKhulnasoftToken struct {
	api.SetTokenRequest
	TokenOTS string `json:"tokenOTS"`
}

// TaskConfig defines khulnasoft task shape.
...
func (c WorkspaceConfig) Validate() error {
	if !(0 < c.IDEPort && c.IDEPort <= math.MaxUint16) {
		return xerrors.Errorf("KHULNASOFT_THEIA_PORT must be between 0 and %d", math.MaxUint16)
	}
	...
	if _, _, err := c.KhulnasoftAPIEndpoint(); err != nil {
		return err
	}
...
// GetTokens parses tokens from KHULNASOFT_TOKENS and possibly downloads OTS.
func (c WorkspaceConfig) GetTokens(downloadOTS bool) ([]WorkspaceKhulnasoftToken, error) {
	if c.Tokens == "" {
		return nil, nil
	}

	var tks []WorkspaceKhulnasoftToken
...
// KhulnasoftAPIEndpoint produces the data required to connect to the Khulnasoft API.
func (c WorkspaceConfig) KhulnasoftAPIEndpoint() (endpoint, host string, err error) {
	gphost, err := url.Parse(c.KhulnasoftHost)
	if err != nil {
		return
	}
...
func (c WorkspaceConfig) isPrebuild() bool {
	return c.KhulnasoftHeadless == "true" || c.DebugWorkspaceType == api.DebugWorkspaceType_prebuild
}

// getKhulnasoftTasks returns true if the workspace is headless.
func (c WorkspaceConfig) isHeadless() bool {
	return c.KhulnasoftHeadless == "true"
}
...
// getKhulnasoftTasks parses khulnasoft tasks.
func (c Config) getKhulnasoftTasks() (tasks []TaskConfig, err error) {
	if c.KhulnasoftTasks != "" {
		var configured *[]TaskConfig
		err = json.Unmarshal([]byte(c.KhulnasoftTasks), &configured)
		if err != nil {
...
func (c WorkspaceConfig) getCommit() (commit *khulnasoft.Commit, err error) {
