...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/common-go/process"
	"github.com/khulnasoft-com/khulnasoft/supervisor/api"
	"github.com/khulnasoft-com/khulnasoft/supervisor/pkg/activation"
...
const (
	...
	logsDir             = "/workspace/.khulnasoft/logs"
	dockerUpLogFilePath = logsDir + "/docker-up.log"

	khulnasoftUserId  = 33333
	khulnasoftGroupId = 33333
)
...
func socketActivationForDocker(parentCtx context.Context, wg *sync.WaitGroup, term *terminal.Mux, cfg *Config, telemetry analytics.Writer, notifications *NotificationService, cstate ContentState) {
	...
	credentialsWritten, err := insertCredentialsIntoConfig(cfg.KhulnasoftImageAuth)
	if err != nil {
	...
	notifyDockerUpFail := func() error {
		...
		telemetry.Track(analytics.TrackMessage{
			...
			Event:    "khulnasoft_activate_docker_fail_notification",
...
func listenToDockerSocket(parentCtx context.Context, term *terminal.Mux, cfg *Config, telemetry analytics.Writer, logFile *os.File, stdout, stderr io.Writer) error {
	...
	_ = os.Chown(fn, khulnasoftUID, khulnasoftGID)
	...
	for ctx.Err() == nil {
		err = activation.Listen(ctx, l, func(socketFD *os.File) error {
			...
			telemetry.Track(analytics.TrackMessage{
				Identity: analytics.Identity{UserID: cfg.OwnerId},
				Event:    "khulnasoft_activate_docker",
				Properties: map[string]interface{}{
					"instanceId":     cfg.WorkspaceInstanceID,
					"workspaceId":    cfg.WorkspaceID,
					"debugWorkspace": cfg.isDebugWorkspace(),
...
func openDockerUpLogFile() (*os.File, error) {
	...
	if err := os.Chown(logsDir, khulnasoftUID, khulnasoftGID); err != nil {
	...
	if err := os.Chown(dockerUpLogFilePath, khulnasoftUID, khulnasoftGID); err != nil {
		_ = logFile.Close()
...
func insertCredentialsIntoConfig(imageAuth string) (int, error) {
	...
	err := insertDockerRegistryAuthentication(authConfig, khulnasoftUserId, khulnasoftGroupId)
	if err != nil {
