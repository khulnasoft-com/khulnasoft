...
func TestBuildChildProcEnv(t *testing.T) {
	withBaseline := func(i []string) []string {
		return append(i,
			...
			"HISTFILE=/workspace/.khulnasoft/.shell_history",
	...
	tests := []struct {
		...
		},
		{
			...
			Input:       []string{"KHULNASOFT_TOKENS=foobar"},
			Expectation: withBaseline(nil),
		},
		{
		...
		},
		{
			Name:  "ots",
			Input: []string{},
			OTS:   `[{"name":"foo","value":"bar"},{"name":"KHULNASOFT_TOKENS","value":"foobar"}]`,
			Expectation: []string{
				"HOME=/home/khulnasoft",
				"BROWSER=/.supervisor/browser.sh",
				"HISTFILE=/workspace/.khulnasoft/.shell_history",
				"PROMPT_COMMAND=history -a", "SUPERVISOR_ADDR=localhost:8080", "USER=khulnasoft", "foo=bar"},
		},
		{
			Name:  "failed ots",
			Input: []string{},
			...
			Expectation: []string{
				...
				"BROWSER=/.supervisor/browser.sh",
				"HISTFILE=/workspace/.khulnasoft/.shell_history",
				"PROMPT_COMMAND=history -a", "SUPERVISOR_ADDR=localhost:8080", "USER=khulnasoft"},
		},
	}
...
func TestIsBlacklistedEnvvar(t *testing.T) {
	tests := []struct {
		...
		{Name: "khulnasoft tokens", Input: "KHULNASOFT_TOKENS", Expectation: true},
		{Name: "khulnasoft tokens child", Input: "KHULNASOFT_TOKENS_GITHUB", Expectation: true},
