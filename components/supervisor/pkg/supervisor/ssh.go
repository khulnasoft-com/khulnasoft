...
func prepareSSHKey(ctx context.Context, sshkey string) error {
	...
	// We need to force HOME because the Khulnasoft user might not have existed at the start of the container
	...
	err = os.Chown(sshkey, khulnasoftUID, khulnasoftGID)
	if err != nil {
...
func ensureSSHDir(cfg *Config) error {
	...
	_ = exec.Command("chown", "-R", fmt.Sprintf("%d:%d", khulnasoftUID, khulnasoftGID), d).Run()
...
func configureSSHDefaultDir(cfg *Config) {
	...
	file, err := os.OpenFile("/home/khulnasoft/.bash_profile", os.O_WRONLY|os.O_APPEND|os.O_CREATE, 0o644)
	if err != nil {
...
func configureSSHMessageOfTheDay() {
	msg := []byte(`Welcome to Khulnasoft: Always ready to code. Try the following commands to get started:

	gp tasks list         List all your defined tasks in .khulnasoft.yml
...
For more information, see the Khulnasoft documentation: https://khulnasoft.com/docs
`)
