...
type APIInterface interface {
	GetToken(ctx context.Context, query *khulnasoft.GetTokenSearchOptions) (res *khulnasoft.Token, err error)
	OpenPort(ctx context.Context, port *khulnasoft.WorkspaceInstancePort) (res *khulnasoft.WorkspaceInstancePort, err error)
	UpdateGitStatus(ctx context.Context, status *khulnasoft.WorkspaceInstanceRepoStatus) (err error)
	WorkspaceUpdates(ctx context.Context) (<-chan *khulnasoft.WorkspaceInstance, error)
...
const (
	// KindKhulnasoft marks tokens that provide access to the Khulnasoft server API.
	KindKhulnasoft = "khulnasoft"
)
...
type Service struct {
	...
	subs     map[chan *khulnasoft.WorkspaceInstance]struct{}
	subMutex sync.Mutex
...
func NewServerApiService(ctx context.Context, cfg *ServiceConfig, tknsrv api.TokenServiceServer) *Service {
	...
	if err != nil {
		log.WithError(err).Error("cannot get token for Khulnasoft API")
		return nil
	}
	...
	service := &Service{
		...
		subs:       make(map[chan *khulnasoft.WorkspaceInstance]struct{}),
	}
...
func (s *Service) GetToken(ctx context.Context, query *khulnasoft.GetTokenSearchOptions) (res *khulnasoft.Token, err error) {
	if s == nil {
...
func (s *Service) UpdateGitStatus(ctx context.Context, status *khulnasoft.WorkspaceInstanceRepoStatus) (err error) {
	if s == nil {
		return errNotConnected
	}
	startTime := time.Now()
...
func (s *Service) OpenPort(ctx context.Context, port *khulnasoft.WorkspaceInstancePort) (res *khulnasoft.WorkspaceInstancePort, err error) {
	if s == nil {
		return nil, errNotConnected
	}
	startTime := time.Now()
	...
	// see https://github.com/khulnasoft-com/khulnasoft/blob/2967579c330de67090d975661a6e3e1cd970ab68/components/server/src/workspace/khulnasoft-server-impl.ts#L1521
	return port, nil
}
...
func (s *Service) WorkspaceUpdates(ctx context.Context) (<-chan *khulnasoft.WorkspaceInstance, error) {
	if s == nil {
		return nil, errNotConnected
	}
	ch := make(chan *khulnasoft.WorkspaceInstance)
	s.subMutex.Lock()
	s.subs[ch] = struct{}{}
	s.subMutex.Unlock()
...
func workspaceStatusToWorkspaceInstance(status *v1.WorkspaceStatus) *khulnasoft.WorkspaceInstance {
	instance := &khulnasoft.WorkspaceInstance{
		CreationTime: status.Instance.CreatedAt.String(),
		ID:           status.Instance.InstanceId,
		Status: &khulnasoft.WorkspaceInstanceStatus{
			ExposedPorts: []*khulnasoft.WorkspaceInstancePort{},
	...
	for _, port := range status.Instance.Status.Ports {
		info := &khulnasoft.WorkspaceInstancePort{
			Port: float64(port.Port),
			URL:  port.Url,
		}
		if port.Policy == v1.PortPolicy_PORT_POLICY_PUBLIC {
			info.Visibility = khulnasoft.PortVisibilityPublic
		} else {
			info.Visibility = khulnasoft.PortVisibilityPrivate
		}
		if port.Protocol == v1.PortProtocol_PORT_PROTOCOL_HTTPS {
			info.Protocol = khulnasoft.PortProtocolHTTPS
		} else {
			info.Protocol = khulnasoft.PortProtocolHTTP
		}
