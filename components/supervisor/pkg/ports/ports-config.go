...
func (configs *Configs) Get(port uint32) (*SortConfig, ConfigKind, bool) {
	...
	for _, rangeConfig := range configs.instanceRangeConfigs {
		if rangeConfig.Start <= port && port <= rangeConfig.End {
			return &SortConfig{
				PortConfig: khulnasoft.PortConfig{
					Port:        float64(port),
...
func (service *ConfigService) update(config *khulnasoft.KhulnasoftConfig, current *Configs) bool {
	currentPortConfigs, currentRangeConfigs := current.instancePortConfigs, current.instanceRangeConfigs
	var ports []*khulnasoft.PortsItems
	if config != nil {
		ports = config.Ports
	}
...
func parseInstanceConfigs(ports []*khulnasoft.PortsItems) (portConfigs map[uint32]*SortConfig, rangeConfigs []*RangeConfig) {
