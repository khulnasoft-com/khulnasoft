...
func TestPortsUpdateState(t *testing.T) {
	...
	type ConfigChange struct {
		instance []*khulnasoft.PortsItems
	}
	type Change struct {
		Config      *ConfigChange
		Served      []ServedPort
	...
	tests := []struct {
		...
		},
		{
			...
			Changes: []Change{
				{
					Config: &ConfigChange{instance: []*khulnasoft.PortsItems{
		...
		{
			Desc: "port status has the name attribute set as soon as the port gets exposed, if there was a name configured in Khulnasoft's Workspace",
			Changes: []Change{
				{
					Config: &ConfigChange{instance: []*khulnasoft.PortsItems{
		...
		},
		{
			...
			// see also https://github.com/khulnasoft-com/openvscode-server/blob/5ab7644a8bbf37d28e23212bc6f1529cafd8bf7b/extensions/khulnasoft-web/src/extension.ts#L310-L339
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			for _, port := range tt.fields.orderInYaml {
				portsItems = append(portsItems, &khulnasoft.PortsItems{Port: port})
			}
