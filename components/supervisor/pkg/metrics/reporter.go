...
import (
	...
	api "github.com/khulnasoft-com/khulnasoft/ide-metrics-api"
...
func NewGrpcMetricsReporter(khulnasoftHost string) *GrpcMetricsReporter {
	return &GrpcMetricsReporter{
		...
		addCounter: func(name string, labels map[string]string, value uint64) {
			doAddCounter(khulnasoftHost, name, labels, value)
		},
		addHistogram: func(name string, labels map[string]string, count uint64, sum float64, buckets []uint64) {
			doAddHistogram(khulnasoftHost, name, labels, count, sum, buckets)
		},
...
func doAddCounter(khulnasoftHost string, name string, labels map[string]string, value uint64) {
	...
	url := fmt.Sprintf("https://ide.%s/metrics-api/metrics/counter/add/%s", khulnasoftHost, name)
...
// TODO(ak) refactor to use grpc when ide-proxy supports it
func doAddHistogram(khulnasoftHost string, name string, labels map[string]string, count uint64, sum float64, buckets []uint64) {
	...
	url := fmt.Sprintf("https://ide.%s/metrics-api/metrics/histogram/add/%s", khulnasoftHost, name)
	ctx, cancel := context.WithTimeout(context.Background(), time.Second*5)
	defer cancel()
	request, err := http.NewRequestWithContext(ctx, http.MethodPost, url, bytes.NewReader(body))
	if err != nil {
