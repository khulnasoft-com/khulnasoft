...
import { Disposable } from "@khulnasoft/khulnasoft-protocol/lib/util/disposable";

let connected = false;
const workspaceSockets = new Set<IDEWebSocket>();

const workspaceOrigin = new URL(window.location.href).origin;
const khulnasoftOrigin = new URL(serverUrl.toString()).origin;
...
function isKhulnasoftOrigin(url: string): boolean {
    const originUrl = new URL(url);
    originUrl.protocol = window.location.protocol;
    return originUrl.origin === khulnasoftOrigin;
}
/**
...
class IDEWebSocket extends ReconnectingWebSocket {
    constructor(url: string, protocol?: string | string[]) {
        ...
        if (isWorkspaceOrigin(url)) {
            ...
            origin = "localhost";
        } else if (isKhulnasoftOrigin(url)) {
            origin = "khulnasoft";
        }
