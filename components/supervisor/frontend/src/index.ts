...
import { WorkspaceInstancePhase } from "@khulnasoft/khulnasoft-protocol";
import { DisposableCollection } from "@khulnasoft/khulnasoft-protocol/lib/util/disposable";
...
LoadingFrame.load().then(async (loading) => {
    ...
    document.title = frontendDashboardServiceClient.latestInfo.workspaceDescription ?? "khulnasoft";
    window.khulnasoft.loggedUserID = frontendDashboardServiceClient.latestInfo.loggedUserId;
    window.khulnasoft.openDesktopIDE = frontendDashboardServiceClient.openDesktopIDE.bind(frontendDashboardServiceClient);
    window.khulnasoft.decrypt = frontendDashboardServiceClient.decrypt.bind(frontendDashboardServiceClient);
    window.khulnasoft.encrypt = frontendDashboardServiceClient.encrypt.bind(frontendDashboardServiceClient);
    window.khulnasoft.isEncryptedData = frontendDashboardServiceClient.isEncryptedData.bind(frontendDashboardServiceClient);
    ...
    (async () => {
        ...
        //#region khulnasoft browser telemetry
