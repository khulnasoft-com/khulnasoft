...
const (
	khulnasoftLogDir          = "/var/log/khulnasoft"
	supervisorLogFilePath = khulnasoftLogDir + "/supervisor.log"
)
...
func initLog(json bool) io.Closer {
	...
	if err := os.MkdirAll(khulnasoftLogDir, 0755); err != nil {
		log.WithError(err).Error("cannot create khulnasoft log directory")
