...
var callServerCmd = &cobra.Command{
	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		api, err := khulnasoft.ConnectToServer(fmt.Sprintf("ws://%s/api/v1", host), khulnasoft.ConnectToServerOpts{
			Token: token,
			Log:   log.Log,
		})
		if err != nil {
...
func init() {
	...
	callServerCmd.Flags().String("workspace-id", os.Getenv("KHULNASOFT_WORKSPACE_ID"), "workspace ID to listen for")
}
