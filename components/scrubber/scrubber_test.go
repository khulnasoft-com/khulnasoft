	...
	tests := []struct {
	...
	}{
		{
			...
			Struct: &struct {
				...
				ContextURL   string
				LeaveMeAlone string
			}{Username: "foo", Email: "foo@bar.com", Password: "foobar", WorkspaceID: "khulnasoftio-khulnasoft-uesaddev73c", ContextURL: "https://github.com/khulnasoft-com/khulnasoft/pull/19402", LeaveMeAlone: "foo"},
			Expectation: Expectation{
				Result: &struct {
					Username     string
					Email        string
...
func TestJSON(t *testing.T) {
	...
	tests := []struct {
		...
		{
			Name:  "basic happy path",
			Input: `{"ok": true, "email": "foo@bar.com", "workspaceID": "khulnasoftio-khulnasoft-uesaddev73c"}`,
			Expectation: Expectation{
	...
	tests := []struct {
	...
	}{
		{
			...
			Struct: &struct {
				...
				WorkspaceID  string
				LeaveMeAlone string
			}{Username: "foo", Email: "foo@bar.com", Password: "foobar", WorkspaceID: "khulnasoftio-khulnasoft-uesaddev73c", LeaveMeAlone: "foo"},
			Expectation: Expectation{
				Result: &struct {
					Username     string
					Email        string
...
// 	file := "/workspace/khulnasoft/components/scrubber/metrics-file"
// 	var data []string
...
func BenchmarkValue(b *testing.B) {
	const input = "This text contains {\"json\":\"data\"}, a workspace ID khulnasoftio-khulnasoft-uesaddev73c and an email foo@bar.com"
