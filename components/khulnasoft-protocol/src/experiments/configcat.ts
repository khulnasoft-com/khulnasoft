...
export const KHULNASOFT_HOST = "khulnasoft_host";

export class ConfigCatClient implements Client {
    constructor(private readonly client: IConfigCatClient, private readonly khulnasoftHost?: string) {}
    ...
    getValueAsync<T>(experimentName: string, defaultValue: T, attributes: Attributes): Promise<T> {
        return this.client.getValueAsync(
            ...
            defaultValue,
            attributesToUser({
                khulnasoftHost: this.khulnasoftHost,
                ...attributes,
            }),
        );
    }
...
export function attributesToUser(attributes: Attributes): ConfigCatUser {
    ...
    if (attributes.khulnasoftHost) {
        custom[KHULNASOFT_HOST] = attributes.khulnasoftHost;
    }
