...
export class KhulnasoftHostUrl {
    readonly url: URL;
    ...
    withDomainPrefix(prefix: string): KhulnasoftHostUrl {
        return this.with((url) => ({ host: prefix + url.host }));
    }

    withoutWorkspacePrefix(): KhulnasoftHostUrl {
    ...
    withoutDomainPrefix(removeSegmentsCount: number): KhulnasoftHostUrl {
    ...
    with(urlUpdate: UrlUpdate) {
        ...
        return new KhulnasoftHostUrl(result);
    }
    ...
    asWorkspaceAuth(instanceID: string): KhulnasoftHostUrl {
    ...
    asApiLogout(): KhulnasoftHostUrl {
        return this.withApi((url) => ({ pathname: "/logout/" }));
    }

    asIDEProxy(): KhulnasoftHostUrl {
        const hostSegments = this.url.host.split(".");
    ...
    asPublicServices(): KhulnasoftHostUrl {
        const hostSegments = this.url.host.split(".");
    ...
    asIDEMetrics(): KhulnasoftHostUrl {
        let newUrl: KhulnasoftHostUrl = this;
        const hostSegments = this.url.host.split(".");
        if (hostSegments[0] !== "ide") {
