...
const schema = require("../data/khulnasoft-schema.json");
...
@injectable()
export class KhulnasoftFileParser {
    public parse(content: string, parseOptions = {}, defaultConfig: WorkspaceConfig = {}): ParseResult {
        ...
        try {
            ...
            log.error("Unparsable Khulnasoft configuration", err, { content });
            return {
                config: defaultConfig,
                validationErrors: ["Unparsable Khulnasoft configuration: " + err.toString()],
            };
        }
    }
}
