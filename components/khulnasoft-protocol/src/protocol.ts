...
export interface ProfileDetails {
    ...
    jobRoleOther?: string;
    // Reasons user is exploring Khulnasoft when they signed up
...
export interface UserEnvVarValue extends EnvVarWithValue {
    id?: string;
    repositoryPattern: string; // DEPRECATED: Use ProjectEnvVar instead of repositoryPattern - https://github.com/khulnasoft-com/khulnasoft/issues/5322
}
...
export namespace EnvVar {
    export const KHULNASOFT_IMAGE_AUTH_ENV_VAR_NAME = "KHULNASOFT_IMAGE_AUTH";
    /**
     * - KHULNASOFT_IMAGE_AUTH is documented https://www.khulnasoft.com/docs/configure/workspaces/workspace-image#use-a-private-docker-image
     */
    export const WhiteListFromReserved = [KHULNASOFT_IMAGE_AUTH_ENV_VAR_NAME];
    ...
    /**
     * Extracts the "host:credentials" pairs from the KHULNASOFT_IMAGE_AUTH environment variable.
     * @param envVars
     * @returns A map of host to credentials
     */
    export function getKhulnasoftImageAuth(envVars: EnvVarWithValue[]): Map<string, string> {
        const res = new Map<string, string>();
        const imageAuth = envVars.find((e) => e.name === EnvVar.KHULNASOFT_IMAGE_AUTH_ENV_VAR_NAME);
        if (!imageAuth) {
            return res;
        }
...
export namespace UserEnvVar {
    ...
    export function validate(variable: UserEnvVarValue): string | undefined {
        ...
        if (!EnvVar.WhiteListFromReserved.includes(name) && name.startsWith("KHULNASOFT_")) {
            return "Name with prefix 'KHULNASOFT_' is reserved.";
        }
        if (name.trim() === "") {
...
export interface KhulnasoftToken {
...
export enum KhulnasoftTokenType {
    API_AUTH_TOKEN = 0,
...
export namespace WorkspaceImageBuild {
    export type Phase = "BaseImage" | "KhulnasoftLayer" | "Error" | "Done";
...
/**
 * WithDefaultConfig contexts disable the download of the khulnasoft.yml from the repository
