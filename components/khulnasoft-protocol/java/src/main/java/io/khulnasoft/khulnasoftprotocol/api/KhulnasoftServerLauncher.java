...
public class KhulnasoftServerLauncher {
    ...
    private final KhulnasoftClient client;

    private KhulnasoftServerLauncher(
    ...
    public KhulnasoftServerConnection listen(
            String apiUrl,
            String origin,
            String userAgent,
        ...
        String khulnasoftHost = URI.create(apiUrl).getHost();
        ...
        for (Proxy proxy : proxies) {
            ...
            if (!(proxyAddress instanceof InetSocketAddress)) {
                KhulnasoftServerConnectionImpl.LOG.log(Level.WARNING, khulnasoftHost + ": unexpected proxy:", proxy);
                continue;
            }
        ...
        KhulnasoftServerConnectionImpl connection = new KhulnasoftServerConnectionImpl(khulnasoftHost);
        connection.whenComplete((input, exception) -> {
            try {
                container.stop();
            } catch (Throwable t) {
                KhulnasoftServerConnectionImpl.LOG.log(Level.WARNING, khulnasoftHost + ": failed to stop websocket container:", t);
            }
        });
        ...
        connection.setSession(container.connectToServer(new Endpoint() {
            ...
            public void onError(Session session, Throwable thr) {
                KhulnasoftServerConnectionImpl.LOG.log(Level.WARNING, khulnasoftHost + ": connection error:", thr);
    ...
    public static KhulnasoftServerLauncher create(KhulnasoftClient client) {
        return new Builder().create(client);
    }

    private static class Builder extends Launcher.Builder<KhulnasoftServer> {

        public KhulnasoftServerLauncher create(KhulnasoftClient client) {
            setLocalService(client);
            setRemoteInterface(KhulnasoftServer.class);
            ...
            return new KhulnasoftServerLauncher(
                    messageReader,
                    jsonHandler,
                    remoteEndpoint,
                    messageWriter,
