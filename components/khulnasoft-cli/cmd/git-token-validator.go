...
var gitTokenValidator = &cobra.Command{
	...
	Short:  "Khulnasoft's Git token validator",
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		f, err := os.OpenFile(os.TempDir()+"/khulnasoft-git-credential-helper.log", os.O_WRONLY|os.O_CREATE|os.O_APPEND, 0644)
		if err == nil {
			defer f.Close()
			log.SetOutput(f)
		}
		...
		if guessedTokenScopes.Message != "" {
			...
			if result.Action == "Open Access Control" {
				cmd := exec.Command("/proc/self/exe", "preview", "--external", wsinfo.GetKhulnasoftHost()+"/access-control")
				err := cmd.Run()
				if err != nil {
