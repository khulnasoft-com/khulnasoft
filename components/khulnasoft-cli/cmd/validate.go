...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/khulnasoft-cli/pkg/supervisor"
	"github.com/khulnasoft-com/khulnasoft/khulnasoft-cli/pkg/utils"
...
func runRebuild(ctx context.Context, supervisorClient *supervisor.SupervisorClient) error {
	...
	khulnasoftConfig, err := utils.ParseKhulnasoftConfig(checkoutLocation)
	if err != nil {
		fmt.Println("The .khulnasoft.yml file cannot be parsed: please check the file and try again")
		fmt.Println("")
		fmt.Println("For help check out the reference page:")
		fmt.Println("https://www.khulnasoft.com/docs/references/khulnasoft-yml#khulnasoftyml")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MalformedKhulnasoftYaml, Silence: true}
	}

	if khulnasoftConfig == nil {
		fmt.Println("To test the image build, you need to configure your project with a .khulnasoft.yml file")
		fmt.Println("")
		...
		fmt.Println("https://www.khulnasoft.com/docs/configure#configure-khulnasoft")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MissingKhulnasoftYaml, Silence: true}
	}

	var image string
	var dockerfilePath string
	var dockerContext string
	switch img := khulnasoftConfig.Image.(type) {
	case nil:
		...
		if image == "" {
			image = "khulnasoft/workspace-full:latest"
		}
	...
	case map[interface{}]interface{}:
		...
		if _, err := os.Stat(dockerfilePath); os.IsNotExist(err) {
			fmt.Println("Your .khulnasoft.yml points to a Dockerfile that doesn't exist: " + dockerfilePath)
		...
		if string(dockerfile) == "" {
			fmt.Println("Your Khulnasoft's Dockerfile is empty")
			fmt.Println("")
			fmt.Println("To learn how to customize your workspace, check out the following docs:")
			fmt.Println("https://www.khulnasoft.com/docs/configure/workspaces/workspace-image#use-a-custom-dockerfile")
			fmt.Println("")
			fmt.Println("Once you configure your Dockerfile, re-run this command to validate your changes")
			return GpError{Err: err, OutCome: utils.Outcome_UserErr, Silence: true}
		}
	default:
		fmt.Println("Check your .khulnasoft.yml and make sure the image property is configured correctly")
		return GpError{Err: err, OutCome: utils.Outcome_UserErr, ErrorCode: utils.RebuildErrorCode_MalformedKhulnasoftYaml, Silence: true}
	}
	...
	if err != nil {
		dockerPath = "/.supervisor/khulnasoft-docker-cli"
	}
	...
	workspaceLocation := khulnasoftConfig.WorkspaceLocation
	...
	tasks, err := json.Marshal(khulnasoftConfig.Tasks)
	if err != nil {
		return err
	}
	...
	for _, env := range validateOpts.KhulnasoftEnvs {
		envs += env + "\n"
	}
	if validateOpts.Headless {
		envs += "KHULNASOFT_HEADLESS=true\n"
	}
	...
	prepareFS := []mnte{
		...
		{Source: "/workspace/.khulnasoft-debug/.docker-root", Target: "/workspace/.docker-root", Permission: 0710},
		{Source: "/workspace/.khulnasoft-debug/.khulnasoft", Target: "/workspace/.khulnasoft", Permission: 0751},
		{Source: "/workspace/.khulnasoft-debug/.vscode-remote", Target: "/workspace/.vscode-remote", Permission: 0751},
		{Source: "/workspace/.khulnasoft-debug/.cache", Target: "/workspace/.cache", Permission: 0751},
		{Source: "/workspace/.khulnasoft-debug/.config", Target: "/workspace/.config", Permission: 0751},
	...
	for _, mnt := range prepareFS {
		...
		if mnt.Target == "" {
			...
			// if target is not same with source and it not a file, ensure target is created by khulnasoft user
...
Connect using SSH keys (https://khulnasoft.com/keys):
%s
...
func init() {
	...
	setFlags := func(cmd *cobra.Command) {
		...
		// internal
		cmd.PersistentFlags().StringArrayVarP(&validateOpts.KhulnasoftEnvs, "khulnasoft-env", "", nil, "")
		...
		_ = cmd.PersistentFlags().MarkHidden("khulnasoft-env")
