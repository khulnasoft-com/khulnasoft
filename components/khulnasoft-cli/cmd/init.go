...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/khulnasoft-cli/pkg/khulnasoftlib"
	"github.com/khulnasoft-com/khulnasoft/khulnasoft-cli/pkg/utils"
	protocol "github.com/khulnasoft-com/khulnasoft/khulnasoft-protocol"
...
// initCmd initializes the workspace's .khulnasoft.yml file
var initCmd = &cobra.Command{
	Use:   "init",
	Short: "Create a Khulnasoft configuration for this project.",
	Long: `
Create a Khulnasoft configuration for this project.
	`,
	RunE: func(cmd *cobra.Command, args []string) error {
		ctx := cmd.Context()
		cfg := khulnasoftlib.KhulnasoftFile{}
		if interactive {
		...
		if !interactive {
			...
			if err != nil {
				...
				fmt.Println("fallback to khulnasoft default")
				defaultImage = ""
			}
			yml := ""
			if defaultImage != "" {
				yml = yml + fmt.Sprintf("# Image of workspace. Learn more: https://www.khulnasoft.com/docs/configure/workspaces/workspace-image\nimage: %s\n\n", defaultImage)
			}
			yml = yml + `# List the start up tasks. Learn more: https://www.khulnasoft.com/docs/configure/workspaces/tasks
tasks:
  - name: Script Task
    init: echo 'init script' # runs during prebuild => https://www.khulnasoft.com/docs/configure/projects/prebuilds
    command: echo 'start script'

# List the ports to expose. Learn more: https://www.khulnasoft.com/docs/configure/workspaces/ports
ports:
  - name: Frontend
...
# Learn more from ready-to-use templates: https://www.khulnasoft.com/docs/introduction/getting-started/quickstart
`
			d = []byte(yml)
		} else {
			fmt.Printf("\n\n---\n%s", d)
		...
		if _, err = os.Stat(".khulnasoft.yml"); err == nil {
			prompt := promptui.Prompt{
				IsConfirm: true,
				Label:     ".khulnasoft.yml file already exists, overwrite?",
			}
			if _, err = prompt.Run(); err != nil {
				fmt.Printf("Not overwriting .khulnasoft.yml file. Aborting.\n")
				return GpError{Silence: true, Err: err, OutCome: utils.Outcome_Success}
			}
		}

		if err = os.WriteFile(".khulnasoft.yml", d, 0644); err != nil {
			return err
		}

		// open .khulnasoft.yml and Dockerfile
		if v, ok := cfg.Image.(khulnasoftlib.KhulnasoftImage); ok {
			if _, err = os.Stat(v.File); os.IsNotExist(err) {
				if err = os.WriteFile(v.File, []byte(`FROM khulnasoft/workspace-full
...
#
# More information: https://www.khulnasoft.com/docs/config-docker/
`), 0644); err != nil {
					return err
				}
			}
		...
		return openCmd.RunE(cmd, []string{".khulnasoft.yml"})
	},
}

func getDefaultWorkspaceImage(ctx context.Context, wsInfo *api.WorkspaceInfoResponse) (string, error) {
	client, err := khulnasoft.ConnectToServer(ctx, wsInfo, []string{
...
func askForDockerImage(ctx context.Context, cfg *khulnasoftlib.KhulnasoftFile) error {
	...
	dockerFile, err := ask("Dockerfile path", ".khulnasoft.Dockerfile", isRequired)
	if err != nil {
		return err
	}
	...
	cfg.SetImage(khulnasoftlib.KhulnasoftImage{
		File:    dockerFile,
		Context: ctxtPath,
	})
	return nil
...
func askForPorts(cfg *khulnasoftlib.KhulnasoftFile) error {
...
func askForTask(cfg *khulnasoftlib.KhulnasoftFile) error {
