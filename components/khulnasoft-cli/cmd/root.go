...
import (
	...
	ide_metrics "github.com/khulnasoft-com/khulnasoft/ide-metrics-api"
)
...
var rootCmd = &cobra.Command{
	Use:           rootCmdName,
	SilenceErrors: true,
	Short:         "Command line interface for Khulnasoft",
...
func Execute() {
	...
	file, ferr := os.OpenFile(os.TempDir()+"/khulnasoft-cli-errors.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if ferr == nil {
		log.SetOutput(file)
		defer file.Close()
	} else {
...
func errorReport(err error) {
	...
	reportErrorRequest := &ide_metrics.ReportErrorRequest{
		...
		Component:  "khulnasoft-cli",
		Version:    khulnasoft.Version,
	}
