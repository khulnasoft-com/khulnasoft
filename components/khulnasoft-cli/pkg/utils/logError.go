...
func LogError(errToReport error, errorMessage string, wsInfo *api.WorkspaceInfoResponse) {
	...
	file, err := os.OpenFile(os.TempDir()+"/khulnasoft-cli-errors.log", os.O_CREATE|os.O_WRONLY|os.O_APPEND, 0666)
	if err == nil {
		log.SetOutput(file)
	} else {
	...
	parsedUrl, err := url.Parse(wsInfo.KhulnasoftHost)
	if err != nil {
		log.WithError(err).Error("cannot parse KhulnasoftHost")
		return
	}
