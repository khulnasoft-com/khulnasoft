...
export class PromisifiedWorkspaceManagerClient implements Disposable {
                ...
                new Promise<StartWorkspaceResponse>((resolve, reject) => {
                    ...
                    this.client.startWorkspace(
                        ...
                        withTracing({ span }),
                        {
                            ...
                            // https://github.com/khulnasoft-com/khulnasoft/blob/main/components/ws-manager/pkg/manager/manager.go#L171
    ...
    ): Promise<DeleteVolumeSnapshotResponse> {
        return this.retryIfUnavailable(
            (attempt: number) =>
                new Promise<DeleteVolumeSnapshotResponse>((resolve, reject) => {
                    const span = TraceContext.startSpan(`/ws-manager/deleteVolumeSnapshot`, ctx);
                    span.log({ attempt });
                    this.client.deleteVolumeSnapshot(
                        request,
                        withTracing({ span }),
                        this.getDefaultUnaryOptions(),
                        (err, resp) => {
