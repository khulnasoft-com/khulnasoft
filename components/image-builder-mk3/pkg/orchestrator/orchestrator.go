...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/image-builder/api"
	protocol "github.com/khulnasoft-com/khulnasoft/image-builder/api"
	"github.com/khulnasoft-com/khulnasoft/image-builder/api/config"
	"github.com/khulnasoft-com/khulnasoft/image-builder/pkg/auth"
...
func (o *Orchestrator) Build(req *protocol.BuildRequest, resp protocol.ImageBuilder_BuildServer) (err error) {
		...
		swr, err = o.wsman.StartWorkspace(ctx, &wsmanapi.StartWorkspaceRequest{
			...
			Spec: &wsmanapi.StartWorkspaceSpec{
				...
				Envvars: []*wsmanapi.EnvironmentVariable{
					...
					{Name: "KHULNASOFT_TASKS", Value: `[{"name": "build", "init": "sudo -E /app/bob build"}]`},
	...
	for {
		...
		// "cannot pull from reg.khulnasoft.com" error message. Instead the image-build should fail properly.
