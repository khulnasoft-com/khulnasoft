...
func convertRemapUser(arg string, value interface{}) ([]string, error) {
	...
	return []string{"--userns-remap", "khulnasoft"}, nil
}
...
func adaptSubid(oldfile string, id int) error {
	...
	if id != 0 {
		newfile.WriteString(mappingFmt("khulnasoft", 1, id))
		newfile.WriteString(mappingFmt("khulnasoft", khulnasoftUserId, 1))
	} else {
		newfile.WriteString(mappingFmt("khulnasoft", khulnasoftUserId, 1))
		newfile.WriteString(mappingFmt("khulnasoft", 1, khulnasoftUserId-1))
		newfile.WriteString(mappingFmt("khulnasoft", khulnasoftUserId+1, 32200)) // map rest of user ids in the user namespace
	}

	uidScanner := bufio.NewScanner(uid)
	for uidScanner.Scan() {
		l := uidScanner.Text()
		if !strings.HasPrefix(l, "khulnasoft") {
