...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/docker-up/dockerd"
...
func main() {
	...
	pflag.BoolVar(&opts.AutoLogin, "auto-login", false, "use content of KHULNASOFT_IMAGE_AUTH to automatically login with the docker daemon")
	pflag.Parse()
...
func runWithinNetns() (err error) {
	...
	if opts.RuncFacade {
		args = append(args,
			"--add-runtime", "khulnasoft="+filepath.Join(opts.BinDir, "runc-facade"),
			"--default-runtime", "khulnasoft",
		)
	}
	...
	// cmp. ENT-324: Required to run dockerd >= 26.1 in a Khulnasoft workspace
