...
func WithDefaultAuth(infoprov common.WorkspaceInfoProvider) RouteHandlerConfigOpt {
	return func(config *Config, c *RouteHandlerConfig) {
		c.WorkspaceAuthHandler = WorkspaceAuthHandler(config.KhulnasoftInstallation.HostName, infoprov)
	}
}
...
func (ir *ideRoutes) HandleRoot(route *mux.Route) {
	...
	proxyPassWoSensitiveCookies := sensitiveCookieHandler(ir.Config.Config.KhulnasoftInstallation.HostName)(proxyPass(ir.Config, ir.InfoProvider, workspacePodResolver))
...
func installForeignBlobserveRoutes(r *mux.Router, config *RouteHandlerConfig, infoProvider common.WorkspaceInfoProvider) {
	...
	r.Use(sensitiveCookieHandler(config.Config.KhulnasoftInstallation.HostName))
...
func installWorkspacePortRoutes(r *mux.Router, config *RouteHandlerConfig, infoProvider common.WorkspaceInfoProvider) error {
	...
	r.NewRoute().HandlerFunc(
		func(rw http.ResponseWriter, r *http.Request) {
			// a work-around for servers which does not respect case-insensitive headers, see https://github.com/khulnasoft-com/khulnasoft/issues/4047#issuecomment-856566526
		...
		return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
			...
			for _, c := range cookies {
				...
				if cookie == "" {
					...
					// See https://github.com/khulnasoft-com/khulnasoft/issues/2470 for more details.
					var (
						originalName    = c.Name
			...
			// being sent. See https://github.com/khulnasoft-com/khulnasoft/issues/2121.
...
func workspaceMustExistHandler(config *Config, infoProvider common.WorkspaceInfoProvider) mux.MiddlewareFunc {
	return func(h http.Handler) http.Handler {
		return http.HandlerFunc(func(resp http.ResponseWriter, req *http.Request) {
			...
			if info == nil {
				redirectURL := fmt.Sprintf("%s://%s/start/?not_found=true#%s", config.KhulnasoftInstallation.Scheme, config.KhulnasoftInstallation.HostName, coords.ID)
...
func (t *blobserveTransport) asBlobserveURL(image string, path string) string {
	return fmt.Sprintf("%s://ide.%s/blobserve/%s%s%s",
		t.Config.KhulnasoftInstallation.Scheme,
		t.Config.KhulnasoftInstallation.HostName,
		image,
		imagePathSeparator,
		path,
	)
...
func servePortNotFoundPage(config *Config) (http.Handler, error) {
	...
	page = bytes.ReplaceAll(page, []byte("https://khulnasoft.com"), []byte(fmt.Sprintf("%s://%s", config.KhulnasoftInstallation.Scheme, config.KhulnasoftInstallation.HostName)))
