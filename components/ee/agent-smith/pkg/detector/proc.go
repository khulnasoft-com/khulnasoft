...
func (p realProcfs) Environ(pid int) ([]string, error) {
	...
	return parseKhulnasoftEnviron(f)
}

func parseKhulnasoftEnviron(r io.Reader) ([]string, error) {
	// Note: this function is benchmarked in BenchmarkParseKhulnasoftEnviron.
	//       At the time of this wriging it consumed 3+N allocs where N is the number of
	//       env vars starting with KHULNASOFT_.
	//
	...
	for scan.Scan() {
		// we only keep KHULNASOFT_ variables for optimisation
		text := scan.Bytes()
		if !bytes.HasPrefix(text, []byte("KHULNASOFT_")) {
			continue
		}
...
func extractWorkspaceFromWorkspacekit(proc discoverableProcFS, pid int) *common.Workspace {
	...
	for _, e := range env {
		if strings.HasPrefix(e, "KHULNASOFT_OWNER_ID=") {
			ownerID = strings.TrimPrefix(e, "KHULNASOFT_OWNER_ID=")
			continue
		}
		if strings.HasPrefix(e, "KHULNASOFT_WORKSPACE_ID=") {
			workspaceID = strings.TrimPrefix(e, "KHULNASOFT_WORKSPACE_ID=")
			continue
		}
		if strings.HasPrefix(e, "KHULNASOFT_INSTANCE_ID=") {
			instanceID = strings.TrimPrefix(e, "KHULNASOFT_INSTANCE_ID=")
			continue
		}
		if strings.HasPrefix(e, "KHULNASOFT_WORKSPACE_CONTEXT_URL=") {
			gitURL = strings.TrimPrefix(e, "KHULNASOFT_WORKSPACE_CONTEXT_URL=")
			continue
		}
	}
