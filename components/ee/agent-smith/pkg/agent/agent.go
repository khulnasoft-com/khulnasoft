...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/agent-smith/pkg/classifier"
	"github.com/khulnasoft-com/khulnasoft/agent-smith/pkg/common"
	"github.com/khulnasoft-com/khulnasoft/agent-smith/pkg/config"
	"github.com/khulnasoft-com/khulnasoft/agent-smith/pkg/detector"
...
type Smith struct {
	...
	KhulnasoftAPI        khulnasoft.APIInterface
...
func NewAgentSmith(cfg config.Config) (*Smith, error) {
	...
	var api khulnasoft.APIInterface
	if cfg.KhulnasoftAPI.HostURL != "" {
		u, err := url.Parse(cfg.KhulnasoftAPI.HostURL)
		if err != nil {
			return nil, xerrors.Errorf("cannot parse Khulnasoft API host url: %w", err)
		}
		endpoint := fmt.Sprintf("wss://%s/api/v1", u.Hostname())

		api, err = khulnasoft.ConnectToServer(endpoint, khulnasoft.ConnectToServerOpts{
			Context: context.Background(),
			Token:   cfg.KhulnasoftAPI.APIToken,
			Log:     log.Log,
		})
		if err != nil {
			return nil, xerrors.Errorf("cannot connect to Khulnasoft API: %w", err)
		}
	}
