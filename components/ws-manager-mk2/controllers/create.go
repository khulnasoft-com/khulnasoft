...
import (
	...
	wsk8s "github.com/khulnasoft-com/khulnasoft/common-go/kubernetes"
	"github.com/khulnasoft-com/khulnasoft/common-go/tracing"
	csapi "github.com/khulnasoft-com/khulnasoft/content-service/api"
	regapi "github.com/khulnasoft-com/khulnasoft/registry-facade/api"
	"github.com/khulnasoft-com/khulnasoft/ws-manager-mk2/pkg/constants"
	config "github.com/khulnasoft-com/khulnasoft/ws-manager/api/config"
	workspacev1 "github.com/khulnasoft-com/khulnasoft/ws-manager/api/crd/v1"
)
...
const (
	...
	headlessLabel = "khulnasoft.com/headless"
	...
	instanceIDLabel = "khulnasoft.com/instanceID"
...
func createDefiniteWorkspacePod(sctx *startWorkspaceContext) (*corev1.Pod, error) {
	...
	labels["khulnasoft.com/networkpolicy"] = "default"
	...
	if sctx.Config.EnableCustomSSLCertificate {
		volumes = append(volumes, corev1.Volume{
			Name: "khulnasoft-ca-crt",
			VolumeSource: corev1.VolumeSource{
				ConfigMap: &corev1.ConfigMapVolumeSource{
					LocalObjectReference: corev1.LocalObjectReference{Name: "khulnasoft-customer-certificate-bundle"},
				},
			},
		})
	}
	...
	matchExpressions := []corev1.NodeSelectorRequirement{
		{
			Key:      "khulnasoft.com/workload_workspace_" + workloadType,
			Operator: corev1.NodeSelectorOpExists,
		},
		{
			Key:      "khulnasoft.com/ws-daemon_ready_ns_" + sctx.Config.Namespace,
			Operator: corev1.NodeSelectorOpExists,
		},
		{
			Key:      "khulnasoft.com/registry-facade_ready_ns_" + sctx.Config.Namespace,
			Operator: corev1.NodeSelectorOpExists,
		},
	}
	...
	pod := corev1.Pod{
		ObjectMeta: metav1.ObjectMeta{
			...
			Finalizers:  []string{workspacev1.KhulnasoftFinalizerName},
		},
		Spec: corev1.PodSpec{
...
func createWorkspaceContainer(sctx *startWorkspaceContext) (*corev1.Container, error) {
	...
	if sctx.Config.EnableCustomSSLCertificate {
		volumeMounts = append(volumeMounts, corev1.VolumeMount{
			Name:      "khulnasoft-ca-crt",
			MountPath: "/etc/ssl/certs/khulnasoft-ca.crt",
...
func createWorkspaceEnvironment(sctx *startWorkspaceContext) ([]corev1.EnvVar, error) {
	...
	wsUrl, err := config.RenderWorkspaceURL(sctx.Config.WorkspaceURLTemplate, sctx.Workspace.Name, sctx.Workspace.Spec.Ownership.WorkspaceID, sctx.Config.KhulnasoftHostURL)
	if err != nil {
		return nil, fmt.Errorf("cannot render workspace URL: %w", err)
	}

	// Envs that start with KHULNASOFT_ are appended to the Terminal environments
	result := []corev1.EnvVar{}
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_REPO_ROOT", Value: allRepoRoots[0]})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_REPO_ROOTS", Value: strings.Join(allRepoRoots, ",")})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_OWNER_ID", Value: sctx.Workspace.Spec.Ownership.Owner})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_WORKSPACE_ID", Value: sctx.Workspace.Spec.Ownership.WorkspaceID})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_INSTANCE_ID", Value: sctx.Workspace.Name})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_THEIA_PORT", Value: strconv.Itoa(int(sctx.IDEPort))})
	...
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_HOST", Value: sctx.Config.KhulnasoftHostURL})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_WORKSPACE_URL", Value: wsUrl})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_WORKSPACE_CLUSTER_HOST", Value: sctx.Config.WorkspaceClusterHost})
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_WORKSPACE_CLASS", Value: sctx.Workspace.Spec.Class})
	...
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_SSH_CA_PUBLIC_KEY", Value: sctx.Workspace.Spec.SSHGatewayCAPublicKey})

	// We don't require that Git be configured for workspaces
	if sctx.Workspace.Spec.Git != nil {
		result = append(result, corev1.EnvVar{Name: "KHULNASOFT_GIT_USER_NAME", Value: sctx.Workspace.Spec.Git.Username})
		result = append(result, corev1.EnvVar{Name: "KHULNASOFT_GIT_USER_EMAIL", Value: sctx.Workspace.Spec.Git.Email})
	}

	if sctx.Config.EnableCustomSSLCertificate {
		const (
			customCAMountPath = "/etc/ssl/certs/khulnasoft-ca.crt"
	...
	for _, e := range sctx.Workspace.Spec.UserEnvVars {
		switch e.Name {
		case "KHULNASOFT_WORKSPACE_CONTEXT",
			"KHULNASOFT_WORKSPACE_CONTEXT_URL",
			"KHULNASOFT_TASKS",
			"KHULNASOFT_RESOLVED_EXTENSIONS",
			"KHULNASOFT_EXTERNAL_EXTENSIONS",
			"KHULNASOFT_WORKSPACE_CLASS_INFO",
			"KHULNASOFT_IDE_ALIAS",
			"KHULNASOFT_RLIMIT_CORE",
			"KHULNASOFT_IMAGE_AUTH":
			// these variables are allowed - don't skip them
		default:
			if strings.HasPrefix(e.Name, "KHULNASOFT_") {
				// we don't allow env vars starting with KHULNASOFT_ and those that we do allow we've listed above
				continue
			}
		}
	...
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_INTERVAL", Value: fmt.Sprintf("%d", int64(heartbeatInterval/time.Millisecond))})
	...
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_MEMORY", Value: strconv.FormatInt(memoryInMegabyte, 10)})

	cpuCount := res.Cpu().Value()
	result = append(result, corev1.EnvVar{Name: "KHULNASOFT_CPU_COUNT", Value: strconv.FormatInt(int64(cpuCount), 10)})

	if sctx.Headless {
		result = append(result, corev1.EnvVar{Name: "KHULNASOFT_HEADLESS", Value: "true"})
	}
...
func createDefaultSecurityContext() (*corev1.SecurityContext, error) {
	...
	res := &corev1.SecurityContext{
		...
		RunAsGroup:             &khulnasoftGUID,
		RunAsNonRoot:           pointer.Bool(true),
		RunAsUser:              &khulnasoftGUID,
	}
