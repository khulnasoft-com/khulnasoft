...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/ws-manager-mk2/pkg/maintenance"
...
//+kubebuilder:rbac:groups=workspace.khulnasoft.com,resources=workspaces,verbs=get;list;watch;create;update;patch;delete
//+kubebuilder:rbac:groups=workspace.khulnasoft.com,resources=workspaces/status,verbs=get;update;patch
//+kubebuilder:rbac:groups=workspace.khulnasoft.com,resources=workspaces/finalizers,verbs=update
...
func (r *WorkspaceReconciler) actOnStatus(ctx context.Context, workspace *workspacev1.Workspace, workspacePods *corev1.PodList) (result ctrl.Result, err error) {
	...
	if len(workspacePods.Items) == 0 {
		...
		case workspace.Status.Phase == workspacev1.WorkspacePhaseStopped:
			...
			if controllerutil.ContainsFinalizer(workspace, workspacev1.KhulnasoftFinalizerName) {
				controllerutil.RemoveFinalizer(workspace, workspacev1.KhulnasoftFinalizerName)
				if err := r.Update(ctx, workspace); err != nil {
					if apierrors.IsNotFound(err) {
						return ctrl.Result{}, nil
					} else {
						return ctrl.Result{}, fmt.Errorf("failed to remove khulnasoft finalizer from workspace: %w", err)
					}
				}
			}
	...
	case workspace.Status.Phase == workspacev1.WorkspacePhaseStopped:
		hadFinalizer := controllerutil.ContainsFinalizer(pod, workspacev1.KhulnasoftFinalizerName)
		controllerutil.RemoveFinalizer(pod, workspacev1.KhulnasoftFinalizerName)
		if err := r.Client.Update(ctx, pod); err != nil {
			return ctrl.Result{}, fmt.Errorf("failed to remove khulnasoft finalizer from pod: %w", err)
		}
