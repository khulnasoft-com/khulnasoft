...
var _ = Describe("WorkspaceController", func() {
	Context("with regular workspaces", func() {
		It("should handle successful workspace creation and stop request", func() {
			...
			Expect(controllerutil.ContainsFinalizer(pod, workspacev1.KhulnasoftFinalizerName)).To(BeTrue())
...
func expectFinalizerAndMarkBackupCompleted(ws *workspacev1.Workspace, pod *corev1.Pod) {
	...
	Consistently(func() (bool, error) {
		...
		return controllerutil.ContainsFinalizer(pod, workspacev1.KhulnasoftFinalizerName), nil
	}, duration, interval).Should(BeTrue(), "missing khulnasoft finalizer on pod, expected one to wait for backup to succeed")
...
func newWorkspace(name, namespace string) *workspacev1.Workspace {
	...
	return &workspacev1.Workspace{
		TypeMeta: metav1.TypeMeta{
			APIVersion: "workspace.khulnasoft.com/v1",
			Kind:       "Workspace",
		},
		ObjectMeta: metav1.ObjectMeta{
			Name:       name,
