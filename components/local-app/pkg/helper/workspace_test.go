...
func TestObserveWorkspaceUntilStarted(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			Name: "stream retry",
			PrepServer: func(mux *http.ServeMux) {
				mux.Handle(khulnasoft_experimental_v1connect.NewWorkspacesServiceHandler(&TestWorkspaceService{
...
type TestWorkspaceService struct {
	khulnasoft_experimental_v1connect.WorkspacesServiceHandler
