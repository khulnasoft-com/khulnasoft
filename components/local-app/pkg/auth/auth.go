...
const keychainServiceName = "khulnasoft-com"
...
type ErrInvalidKhulnasoftToken struct {
	cause error
}

func (e *ErrInvalidKhulnasoftToken) Error() string {
	return "invalid khulnasoft token: " + e.cause.Error()
}

// ValidateToken validates the given tkn against the given khulnasoft service
func ValidateToken(client khulnasoft.APIInterface, tkn string) error {
	hash := sha256.Sum256([]byte(tkn))
	tokenHash := hex.EncodeToString(hash[:])
	tknScopes, err := client.GetKhulnasoftTokenScopes(context.Background(), tokenHash)
	if e, ok := err.(*khulnasoft.ErrBadHandshake); ok && e.Resp.StatusCode == 401 {
		return &ErrInvalidKhulnasoftToken{err}
	}
	...
	for _, scope := range authScopesLocalCompanion {
		...
		if !ok {
			return &ErrInvalidKhulnasoftToken{fmt.Errorf("%v scope is missing in %v", scope, tknScopes)}
		}
	}
	return nil
}

// SetToken returns the persisted Khulnasoft token
func SetToken(host, token string) error {
	return keyring.Set(keychainServiceName, host, token)
}

// GetToken returns the persisted Khulnasoft token
...
// DeleteToken deletes the persisted Khulnasoft token
	...
	<head>
		...
		<script>
			if (window.opener) {
				...
				console.log("This page was not opened by Khulnasoft.")
...
// Login walks through the login flow for obtaining a Khulnasoft token
func Login(ctx context.Context, opts LoginOpts) (token string, err error) {
	...
	if opts.ExtendScopes {
		authScopesLocalCompanion, err = fetchValidCLIScopes(ctx, opts.KhulnasoftURL)
		if err != nil {
			return "", err
		}
		slog.Debug("Using CLI scopes", "scopes", authScopesLocalCompanion)
		conf = &oauth2.Config{
			ClientID:     "khulnasoft-cli",
			ClientSecret: "khulnasoft-cli-secret",
			Scopes:       authScopesLocalCompanion,
			Endpoint: oauth2.Endpoint{
				AuthURL:  authURL.String(),
				TokenURL: tokenURL.String(),
	...
	if err != nil {
		return "", prettyprint.AddResolution(fmt.Errorf("cannot open browser to URL %s: %s\n", authorizationURL, err),
			"provide a personal access token using --token or the KHULNASOFT_TOKEN environment variable",
		)
	}
	...
	// Extract Khulnasoft token from OAuth token (JWT)
	...
	khulnasoftToken := claims["jti"].(string)
	return khulnasoftToken, nil
}
