	...
	}{
		{
			...
			Expectation: Expectation{
				Manifest: &Manifest{
					...
					Binaries: []Binary{
						{
							Filename: "khulnasoft-linux-amd64",
							OS:       "linux",
							Arch:     "amd64",
...
func TestDownloadManifest(t *testing.T) {
	...
	tests := []struct {
			...
			Expectation: func(url string) Expectation {
				return Expectation{
					Manifest: &Manifest{
						...
						Binaries: []Binary{
							{
								URL:      url + KhulnasoftCLIBasePath + "/khulnasoft-linux-amd64",
								Filename: "khulnasoft-linux-amd64",
								OS:       "linux",
								Arch:     "amd64",
								Digest:   "sha256:9f86d081884c7d659a2feaa0c55ad015a3bf4f1b2b0b822cd15d6c15b0f00a08",
		...
		},
		{
			...
			Expectation: func(url string) Expectation {
				return Expectation{
					Error: "cannot download manifest from " + url + KhulnasoftCLIBasePath + "/manifest.json: 404 Not Found",
				}
			},
		},
	}
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			if test.Manifest != nil {
				mux.Handle(filepath.Join(KhulnasoftCLIBasePath, "/manifest.json"), http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
...
func TestReplaceSelf(t *testing.T) {
			...
			Files: []File{
				{
					OS:       runtime.GOOS,
					Arch:     runtime.GOARCH,
					Filename: "khulnasoft-" + runtime.GOOS + "-" + runtime.GOARCH,
					Content:  []byte("#!/bin/sh"),
				},
			},
		},
	...
	for _, test := range tests {
		t.Run(test.Name, func(t *testing.T) {
			...
			mux := http.NewServeMux()
			mux.Handle(KhulnasoftCLIBasePath+"/manifest.json", http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
				_ = json.NewEncoder(w).Encode(mf)
			}))
			for _, f := range test.Files {
				mux.Handle(KhulnasoftCLIBasePath+"/"+f.Filename, http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
