...
const (
	// KhulnasoftCLIBasePath is the path relative to a Khulnasoft installation where the latest
	// binary and manifest can be found.
	KhulnasoftCLIBasePath = "/static/bin"
)
...
func DownloadManifest(ctx context.Context, baseURL string) (res *Manifest, err error) {
	defer func() {
		if err != nil {
			pth := strings.TrimSuffix(baseURL, "/") + KhulnasoftCLIBasePath
	...
	murl.Path = filepath.Join(murl.Path, KhulnasoftCLIBasePath)
...
func ReplaceSelf(ctx context.Context, manifest *Manifest) error {
	...
	if err != nil && strings.Contains(err.Error(), "permission denied") && runtime.GOOS != "windows" {
		...
		err = prettyprint.AddResolution(err,
			fmt.Sprintf("run `sudo {khulnasoft} --config %s version update`", cfgfn),
		)
	}
	return err
}
