...
import (
	...
	app "github.com/khulnasoft-com/khulnasoft/local-app/api"
	...
	for _, ws := range s.workspaces {
		...
		cfg.Hosts = append(cfg.Hosts, &ssh_config.Host{
			...
			Nodes: []ssh_config.Node{
				...
				&ssh_config.KV{Key: "User", Value: "khulnasoft"},
...
func New(client khulnasoft.APIInterface, localAppTimeout time.Duration, cb Callbacks) *Bastion {
...
type Bastion struct {
	...
	Client    khulnasoft.APIInterface
	Callbacks Callbacks
...
func (b *Bastion) FullUpdate() {
	wss, err := b.Client.GetWorkspaces(b.ctx, &khulnasoft.GetWorkspacesOptions{Limit: float64(100)})
	if err != nil {
...
func generateSSHKeys(instanceID string) (privateKeyFN string, publicKey string, err error) {
	privateKeyFN = filepath.Join(os.TempDir(), fmt.Sprintf("khulnasoft_%s_id_rsa", instanceID))
	useRrandomFile := func() {
		var tmpf *os.File
		tmpf, err = ioutil.TempFile("", "khulnasoft_*_id_rsa")
		if err != nil {
			return
		}
		tmpf.Close()
...
func (b *Bastion) connectTunnelClient(ctx context.Context, ws *Workspace) error {
	...
	h := make(http.Header)
	h.Set("x-khulnasoft-owner-token", ws.OwnerToken)
	webSocket := khulnasoft.NewReconnectingWebsocket(tunnelURL, h, logrus.WithField("workspace", ws.WorkspaceID))
...
func newTunnelClient(ctx context.Context, ws *Workspace, reconnecting *khulnasoft.ReconnectingWebsocket) (client *TunnelClient, closed chan struct{}, err error) {
	logrus.WithField("workspace", ws.WorkspaceID).Info("tunnel: trying to connect ssh client...")
	err = reconnecting.EnsureConnection(func(conn *khulnasoft.WebsocketConnection) (bool, error) {
