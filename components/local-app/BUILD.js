const generatePackage = function (goos, goarch, binaryName, mainFile) {
    ...
    let pkg = {
        ...
        deps: [
            ...
            "components/khulnasoft-protocol/go:lib",
        ...
        config: {
            ...
            buildCommand: [
                ...
                `GOARCH=${goarch} go build -trimpath -ldflags "-X github.com/khulnasoft-com/local-app/pkg/constants.GitCommit=\${__git_commit} -X github.com/khulnasoft-com/local-app/pkg/constants.BuildTime=\$(date +%s)" -o ${binaryName} ${mainFile}`,
            ],
        },
        binaryName,
    };
...
const packages = [];
for (binaryName of ["khulnasoft-local-companion", "khulnasoft-cli"]) {
