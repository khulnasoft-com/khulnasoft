...
var loginCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if loginOpts.OrganizationID == "" {
			clnt, err := getKhulnasoftClient(config.ToContext(context.Background(), cfg))
			if err != nil {
				return fmt.Errorf("cannot connect to Khulnasoft with this context: %w", err)
			}
			if !loginOpts.NonInteractive {
		...
		client, err := getKhulnasoftClient(config.ToContext(cmd.Context(), cfg))
		if err != nil {
			return err
		}
...
func init() {
	...
	if v := os.Getenv("KHULNASOFT_HOST"); v != "" {
		host = v
	}
	loginCmd.Flags().StringVar(&loginOpts.Host, "host", host, "The Khulnasoft instance to log in to (defaults to $KHULNASOFT_HOST)")
	loginCmd.Flags().StringVar(&loginOpts.Token, "token", "", "The token to use for authentication (defaults to $KHULNASOFT_TOKEN)")
