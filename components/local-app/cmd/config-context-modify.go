...
var configContextModifyCmd = &cobra.Command{
	...
	Short:   "Modifies a context entry in the khulnasoft CLI config",
		...
		if configContextModifyOpts.Current {
			if len(args) > 0 {
				return prettyprint.AddResolution(fmt.Errorf("cannot use --current and specify a context name"),
					"modify current context with `{khulnasoft} config set-context --current`",
					"modify/create a different context with `{khulnasoft} config set-context <name>`",
				)
			}
