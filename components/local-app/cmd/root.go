...
import (
	...
	"github.com/khulnasoft-com/local-app/pkg/config"
	"github.com/khulnasoft-com/local-app/pkg/constants"
	"github.com/khulnasoft-com/local-app/pkg/prettyprint"
	"github.com/khulnasoft-com/local-app/pkg/selfupdate"
	"github.com/khulnasoft-com/local-app/pkg/telemetry"
...
var rootCmd = &cobra.Command{
	Use:   "khulnasoft",
	Short: "Khulnasoft: Always ready to code.",
	Long: color.Sprint(`
<fg=ff971d>      .-+*#+           </>      <b>Khulnasoft: Always ready to code.</>
<fg=ff971d>   :=*#####*.          </>      Try the following commands to get started:
<fg=ff971d>  .=*####*+-.    .--:  </>
<fg=ff971d>  +****=:     :=*####+ </>      khulnasoft login              <lightgray>Login to Khulnasoft</>
<fg=ff971d>  ****:   .-+*########.</>      khulnasoft whoami             <lightgray>Show information about the currently logged in user</>
<fg=ff971d>  +***:   *****+--####.</>
<fg=ff971d>  +***:   .-=:.  .#*##.</>      khulnasoft workspace list     <lightgray>List your workspaces</>
<fg=ff971d>  +***+-.      .-+**** </>      khulnasoft workspace create   <lightgray>Create a new workspace</>
<fg=ff971d>  .=*****+=::-+*****+: </>      khulnasoft workspace open     <lightgray>Open a running workspace</>
<fg=ff971d>  .:=+*********=-.     </>      khulnasoft workspace stop     <lightgray>Stop a running workspace</>
<fg=ff971d>      .-++++=:         </>
...
func init() {
	...
	if fn := os.Getenv("KHULNASOFT_CONFIG"); fn != "" {
		configLocation = fn
	}
...
var rootTestingOpts struct {
	Client    *client.Khulnasoft
	WriterOut io.Writer
}

var clientCache *client.Khulnasoft

func getKhulnasoftClient(ctx context.Context) (*client.Khulnasoft, error) {
	...
	if host == nil {
		return nil, prettyprint.AddResolution(fmt.Errorf("active context has no host configured"),
			"set a host using `khulnasoft config set-context --current --host <host>`",
			"login again using `khulnasoft login`",
			"change to a different context using `khulnasoft config use-context <context>`",
		)
	}
	...
	if token == "" {
		return nil, prettyprint.AddResolution(fmt.Errorf("no token found for active context"),
			"provide a token by setting the KHULNASOFT_TOKEN environment variable",
			"login again using `khulnasoft login`",
			"change to a different context using `khulnasoft config use-context <context>`",
			"set a token explicitly using `khulnasoft config set-context --current --token <token>`",
		)
	}

	var apiHost = *gpctx.Host.URL
	apiHost.Host = "api." + apiHost.Host
	slog.Debug("establishing connection to Khulnasoft", "host", apiHost.String())
	res, err := client.New(
