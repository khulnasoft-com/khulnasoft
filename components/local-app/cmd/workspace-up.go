...
import (
	...
	"github.com/khulnasoft-com/local-app/pkg/helper"
...
var workspaceUpCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		khulnasoft, err := getKhulnasoftClient(cmd.Context())
		if err != nil {
			return err
		}

		if workspaceCreateOpts.WorkspaceClass != "" {
			resp, err := khulnasoft.Workspaces.ListWorkspaceClasses(cmd.Context(), connect.NewRequest(&v1.ListWorkspaceClassesRequest{}))
			if err != nil {
		...
		if workspaceCreateOpts.Editor != "" {
			resp, err := khulnasoft.Editors.ListEditorOptions(cmd.Context(), connect.NewRequest(&v1.ListEditorOptionsRequest{}))
			if err != nil {
		...
		for {
			...
			if parentDir == currentDir {
				...
				return prettyprint.AddResolution(fmt.Errorf("no Git repository found"),
					...
					"open a remote repository using `{khulnasoft} workspace create <repo-url>`",
				)
			}
		...
		_ = repo.DeleteRemote("khulnasoft")
		head, err := repo.Head()
		if err != nil {
		...
		newWorkspace, err := khulnasoft.Workspaces.CreateAndStartWorkspace(ctx, connect.NewRequest(
			&v1.CreateAndStartWorkspaceRequest{
				Source:         &v1.CreateAndStartWorkspaceRequest_ContextUrl{ContextUrl: "KHULNASOFTCLI_CONTENT_INIT=push/https://github.com/khulnasoft-com/empty"},
		...
		ws, err := helper.ObserveWorkspaceUntilStarted(ctx, khulnasoft, workspaceID)
		if err != nil {
			return err
		}
		slog.Debug("workspace started", "workspaceID", workspaceID)

		token, err := khulnasoft.Workspaces.GetOwnerToken(ctx, connect.NewRequest(&v1.GetOwnerTokenRequest{WorkspaceId: workspaceID}))
		if err != nil {
			return err
		}
		var (
		...
		pushcmd := exec.Command("git", "push", "--progress", "khulnasoft")
		...
		case workspaceCreateOpts.StartOpts.OpenEditor:
			return helper.OpenWorkspaceInPreferredEditor(ctx, khulnasoft, workspaceID)
		default:
