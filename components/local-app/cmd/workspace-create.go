...
var workspaceCreateCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		_, err = helper.ObserveWorkspaceUntilStarted(ctx, khulnasoft, workspaceID)
		if err != nil {
			return err
		}

		if workspaceCreateOpts.StartOpts.OpenSSH {
			return helper.SSHConnectToWorkspace(ctx, khulnasoft, workspaceID, false)
		}
...
func classCompletionFunc(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	ctx := cmd.Context()
	khulnasoft, err := getKhulnasoftClient(ctx)
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}
	resp, err := khulnasoft.Workspaces.ListWorkspaceClasses(ctx, connect.NewRequest(&v1.ListWorkspaceClassesRequest{}))
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}
...
func editorCompletionFunc(cmd *cobra.Command, args []string, toComplete string) ([]string, cobra.ShellCompDirective) {
	...
	resp, err := khulnasoft.Editors.ListEditorOptions(ctx, connect.NewRequest(&v1.ListEditorOptionsRequest{}))
	if err != nil {
		return nil, cobra.ShellCompDirectiveError
	}
	items := resp.Msg.GetResult()
