...
import { Commit, Repository, User } from "@khulnasoft/khulnasoft-protocol";
...
@suite(timeout(10000), retries(2), skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET")))
class TestBitbucketFileProvider {
    ...
    @test public async testGetFileContents() {
        const result = await this.fileProvider.getFileContent(
            { repository: { owner: "khulnasoft", name: "integration-tests" }, revision: "5a24a0c" } as Commit,
            this.user,
            "README.md",
        );
...
This is the readme of the second branch.`);
    ...
    @test public async testGetLastChangeRevision_ok() {
        const result = await this.fileProvider.getLastChangeRevision(
            { owner: "khulnasoft", name: "integration-tests" } as Repository,
            "second-branch",
            this.user,
            "README.md",
        );
