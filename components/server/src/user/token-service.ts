...
import { Token, Identity, User, TokenEntry } from "@khulnasoft/khulnasoft-protocol";
...
export class TokenService implements TokenProvider {
    static readonly KHULNASOFT_AUTH_PROVIDER_ID = "Khulnasoft";
    /**
     * [mins]
     *
    ...
    async getOrCreateKhulnasoftIdentity(user: User): Promise<Identity> {
        let identity = User.getIdentity(user, TokenService.KHULNASOFT_AUTH_PROVIDER_ID);
        if (!identity) {
            identity = {
                authProviderId: TokenService.KHULNASOFT_AUTH_PROVIDER_ID,
                authId: user.id,
    ...
    async createKhulnasoftToken(user: User, ...scopes: string[]): Promise<TokenEntry> {
        const identity = await this.getOrCreateKhulnasoftIdentity(user);
