...
import { User, Identity, Token, IdentityLookup } from "@khulnasoft/khulnasoft-protocol";
import { BUILTIN_INSTLLATION_ADMIN_USER_ID, EmailDomainFilterDB, MaybeUser, UserDB } from "@khulnasoft/khulnasoft-db/lib";
...
export class UserAuthentication {
    ...
    async deauthorize(user: User, authProviderId: string) {
        const externalIdentities = user.identities.filter(
            (i) => i.authProviderId !== TokenService.KHULNASOFT_AUTH_PROVIDER_ID,
        );
        ...
        if (remainingLoginIdentities.length === 0) {
            throw new Error(
                "Cannot remove last authentication provider for logging in to Khulnasoft. Please delete account if you want to leave.",
            );
        }
    ...
    /**
     * khulnasoft.com: Only installation-level users are allowed to create orgs
    ...
    async mayCreateOrganization(user: User): Promise<boolean> {
        ...
        const isMultiOrgEnabled = await getExperimentsClientForBackend().getValueAsync("enable_multi_org", false, {
            khulnasoftHost: this.config.hostUrl.url.host,
        });
        return (
