...
import { ProjectDB, TeamDB, UserDB } from "@khulnasoft/khulnasoft-db/lib";
import {
    CommitContext,
    EnvVar,
    EnvVarWithValue,
...
export interface ResolvedEnvVars {
     ...
     */
    khulnasoftImageAuth?: Map<string, string>;
...
export class EnvVarService {
    ...
    validateProjectOrOrgEnvVar(envVar: Partial<ProjectEnvVarWithValue>) {
        ...
        if (!EnvVar.WhiteListFromReserved.includes(envVar.name) && envVar.name.startsWith("KHULNASOFT_")) {
            throw new ApplicationError(ErrorCodes.BAD_REQUEST, "Variable name with prefix 'KHULNASOFT_' is reserved");
        }
    ...
    async addOrgEnvVar(requestorId: string, orgId: string, envVar: OrgEnvVarWithValue): Promise<OrgEnvVar> {
        ...
        if (envVar.name !== EnvVar.KHULNASOFT_IMAGE_AUTH_ENV_VAR_NAME) {
            throw new ApplicationError(
                ErrorCodes.BAD_REQUEST,
                "Can only update KHULNASOFT_IMAGE_AUTH env var on org level",
            );
        }
    ...
    async resolveEnvVariables(
        ...
        // 1. first merge the `env` in the .khulnasoft.yml
        if (wsConfig?.env) {
        ...
        // KHULNASOFT_IMAGE_AUTH is a special case: it is only passed into the workspace if the project settings allow it
        const credentials = EnvVar.getKhulnasoftImageAuth([...workspaceEnvVars.values()]);
        let project: Project | undefined;
        if (projectId) {
            project = await this.projectDB.findProjectById(projectId);
        }
        if (!project?.settings?.enableDockerdAuthentication) {
            workspaceEnvVars.delete(EnvVar.KHULNASOFT_IMAGE_AUTH_ENV_VAR_NAME);
        }

        return {
            khulnasoftImageAuth: credentials,
