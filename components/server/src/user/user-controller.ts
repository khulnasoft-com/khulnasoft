...
import { OneTimeSecretDB, TeamDB, UserDB, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
import { BUILTIN_INSTLLATION_ADMIN_USER_ID } from "@khulnasoft/khulnasoft-db/lib/user-db";
import express from "express";
import { Authenticator } from "../auth/authenticator";
import { Config } from "../config";
import { log, LogContext } from "@khulnasoft/khulnasoft-protocol/lib/util/logging";
import { AuthorizationService } from "./authorization-service";
import { Permission } from "@khulnasoft/khulnasoft-protocol/lib/permission";
import { parseWorkspaceIdFromHostname } from "@khulnasoft/khulnasoft-protocol/lib/util/parse-workspace-id";
import { SessionHandler } from "../session-handler";
import { URL } from "url";
import { getRequestingClientInfo } from "../express-util";
import { KhulnasoftToken, KhulnasoftTokenType, User } from "@khulnasoft/khulnasoft-protocol";
...
import { TrustedValue } from "@khulnasoft/khulnasoft-protocol/lib/util/scrubbing";

export const ServerFactory = Symbol("ServerFactory");
export type ServerFactory = () => KhulnasoftServerImpl;
...
export class UserController {
    ...
    get apiRouter(): express.Router {
        ...
        router.get(
            ...
            loginUserWithOts(async (req: express.Request, res: express.Response, user: User, secret: string) => {
                // This mechanism is used by integration tests, cmp. https://github.com/khulnasoft-com/khulnasoft/blob/478a75e744a642d9b764de37cfae655bc8b29dd5/test/tests/ide/vscode/python_ws_test.go#L105
        ...
        router.post(
            ...
            async (req: express.Request, res: express.Response, next: express.NextFunction) => {
                ...
                await runWithSubjectId(SubjectId.fromUserId(user.id), async () => {
                    ...
                    const server = this.createKhulnasoftServer(user, resourceGuard);
                    try {
    ...
    protected async augmentLoginRequest(req: express.Request) {
        ...
        if (returnToURL) {
            // returnToURL -> https://khulnasoft.com/#https://github.com/theia-ide/theia"
    ...
    protected getSafeReturnToParam(req: express.Request) {
        ...
        if (
            ...
            this.urlStartsWith(returnToURL, "https://www.khulnasoft.com")
        ) {
            return returnToURL;
        }
    ...
    private createKhulnasoftServer(user: User, resourceGuard: ResourceAccessGuard) {
