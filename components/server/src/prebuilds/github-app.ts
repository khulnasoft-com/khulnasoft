...
import { log, LogContext, LogrusLogLevel } from "@khulnasoft/khulnasoft-protocol/lib/util/logging";
import {
    WorkspaceConfig,
    User,
    Project,
...
import { ApplicationError, ErrorCode } from "@khulnasoft/khulnasoft-protocol/lib/messaging/error";
...
/**
 * GitHub app urls:
 *    User authorization callback URL: https://khulnasoft.com/install-github-app
 *    Setup URL:                       https://khulnasoft.com/install-github-app
 *    Webhook URL:                     https://khulnasoft.com/apps/github
 *
 * Make sure that the webhook secret you set in GitHub matches what's in your
 * values.yaml file (KHULNASOFT_GITHUB_APP_WEBHOOK_SECRET) - it's not a bad idea to
...
export class GithubApp {
    ...
    private async buildApp(app: Probot, options: ApplicationFunctionOptions) {
        ...
        // Backward-compatibility: Redirect old badge URLs (e.g. "/api/apps/github/pbs/github.com/khulnasoft-com/khulnasoft/5431d5735c32ab7d5d840a4d1a7d7c688d1f0ce9.svg")
        options.getRouter &&
            options
                .getRouter("/pbs")
    ...
    private async handlePushEvent(ctx: Context<"push">): Promise<void> {
        ...
        // https://github.com/khulnasoft-com/khulnasoft/issues/11578
    ...
    private getBadgeImageURL(): string {
        return this.config.hostUrl.with({ pathname: "/button/open-in-khulnasoft.svg" }).toString();
    }
