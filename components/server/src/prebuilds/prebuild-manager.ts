...
import { DBWithTracing, TracedWorkspaceDB, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
import {
    CommitContext,
    CommitInfo,
    PrebuildWithStatus,
...
import { secondsBefore } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
...
export class PrebuildManager {
    ...
    async startPrebuild(
        ...
        try {
            ...
            if (await this.shouldRateLimitPrebuild(span, project)) {
                ...
                prebuild.error =
                    "Prebuild is rate limited. Please contact Khulnasoft if you believe this happened in error.";
    ...
    checkPrebuildPrecondition(params: { config: WorkspaceConfig; project: Project; context: CommitContext }): {
        ...
        if (!config || !config._origin || config._origin !== "repo") {
            // we demand an explicit khulnasoft config
            return { shouldRun: false, reason: "no-khulnasoft-config-in-repo" };
        }

        const hasPrebuildTask = !!config.tasks && config.tasks.find((t) => !!t.before || !!t.init || !!t.prebuild);
        if (!hasPrebuildTask) {
            return { shouldRun: false, reason: "no-tasks-in-khulnasoft-config" };
        }
