...
import { TeamDB, WebhookEventDB } from "@khulnasoft/khulnasoft-db/lib";
...
import { CommitContext, CommitInfo, Project, User, WebhookEvent } from "@khulnasoft/khulnasoft-protocol";
...
export class BitbucketServerApp {
        ...
        this._router.post("/", async (req, res) => {
            try {
                ...
                if (PushEventPayload.is(req.body)) {
                    ...
                    if (!user) {
                        // If the webhook installer is no longer found in Khulnasoft's DB
    ...
    private async findUser(ctx: TraceContext, secretToken: string): Promise<User> {
        ...
        try {
            ...
            const identity = user.identities.find((i) => i.authProviderId === TokenService.KHULNASOFT_AUTH_PROVIDER_ID);
            if (!identity) {
                throw new Error(`User ${user.id} has no identity for '${TokenService.KHULNASOFT_AUTH_PROVIDER_ID}'.`);
            }
    ...
    private getCloneUrl(event: PushEventPayload): string {
        ...
        //     "clone": [
        //         {
        //             "href": "ssh://git@bitbucket.khulnasoft-dev.com:7999/tes/hello-world-zz-1.git",
        //             "name": "ssh"
        //         },
        //         {
        //             "href": "https://bitbucket.khulnasoft-dev.com/scm/tes/hello-world-zz-1.git",
        //             "name": "http"
        //         }
        //     ],
        //     "self": [...]
