    ...
    private async findUser(
        ...
        try {
            ...
            for (const user of projectOwners.users) {
                const khulnasoftIdentity = user.identities.find(
                    (i) => i.authProviderId === TokenService.KHULNASOFT_AUTH_PROVIDER_ID,
                );
                if (!khulnasoftIdentity) {
                    continue;
                }
                // Verify the webhook signature
                const signature = req.header("X-Hub-Signature-256");
                const body = (req as any).rawBody;
                const tokenEntries = (await this.userService.findTokensForIdentity(user.id, khulnasoftIdentity)).filter(
                    (tokenEntry) => {
