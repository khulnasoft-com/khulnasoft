...
import { PrebuiltWorkspaceState, WithCommitHistory } from "@khulnasoft/khulnasoft-protocol/lib/protocol";
import { PrebuildWithWorkspace, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
...
export class IncrementalWorkspaceService {
    ...
    public async findBaseForIncrementalWorkspace(
        ...
        const [recentPrebuilds, imageSource] = await Promise.allSettled([
            ...
            // (e.g., at the time of writing, the Khulnasoft repository has 16K+ prebuilds, but only ~300 not-garbage-collected)
