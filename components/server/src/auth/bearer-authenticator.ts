...
import { UserDB, PersonalAccessTokenDB } from "@khulnasoft/khulnasoft-db/lib";
import { KhulnasoftTokenType, User } from "@khulnasoft/khulnasoft-protocol";
...
export class BearerAuth {
        ...
        return async (req, res, next) => {
            try {
                ...
                if (isBearerAuthError(e)) {
                    // (AT) while investigating https://github.com/khulnasoft-com/khulnasoft/issues/8703 we
    ...
    private async userAndScopesFromToken(token: string): Promise<{ user: User; scopes: string[] }> {
        ...
        //  1. Personal Access Tokens which are prefixed with `khulnasoft_pat_`
        ...
        const userAndToken = await this.userDB.findUserByKhulnasoftToken(hash, KhulnasoftTokenType.API_AUTH_TOKEN);
        if (!userAndToken) {
...
export class PersonalAccessToken {
    ...
    private static PAT_PREFIX = "khulnasoft_pat_";
