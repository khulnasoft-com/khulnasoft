...
import { UserDB } from "@khulnasoft/khulnasoft-db/lib";
import { AuthProviderInfo, Identity, Token, User } from "@khulnasoft/khulnasoft-protocol";
...
import { daysBefore, isDateSmaller } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
import { IAnalyticsWriter } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
...
export abstract class GenericAuthProvider implements AuthProvider {
    ...
    async callback(request: express.Request, response: express.Response, next: express.NextFunction): Promise<void> {
        ...
        if (flowContext) {
            ...
            if (VerifyResult.WithIdentity.is(flowContext)) {
                ...
                // Set all cookies used on website for visitor preferences for .khulnasoft.com domain if no preference exists yet
    ...
    protected async verify(
        ...
        const authProviderId = this.authProviderId;
        let currentKhulnasoftUser: User | undefined = User.is(req.user) ? req.user : undefined;
        ...
        try {
            ...
            if (currentKhulnasoftUser) {
                // user is already logged in

                // check for matching auth ID
                const currentIdentity = currentKhulnasoftUser.identities.find(
                ...
                if (currentIdentity && currentIdentity.authId !== candidate.authId) {
                    log.warn(`User is trying to connect with another provider identity.`, {
                        ...
                        authUser,
                        candidate,
                        currentKhulnasoftUser: currentKhulnasoftUser.id,
                        clientInfo,
                    });
                    done(
                ...
                currentKhulnasoftUser = await this.userAuthentication.findUserForLogin({ candidate });
            ...
            if (currentKhulnasoftUser) {
                const elevateScopes = authFlow.overrideScopes
                    ? undefined
                    : await this.getMissingScopeForElevation(currentKhulnasoftUser, currentScopes);
                const isBlocked = await this.userAuthentication.isBlocked({ user: currentKhulnasoftUser });
            ...
            done(undefined, currentKhulnasoftUser || candidate, flowContext);
        } catch (err) {
...
interface GenericOAuthStrategyOptions {
    scope?: string | string[];
    /**
     * This should be Khulnasoft's hostname.
     */
    userAgent: string;
