...
type KhulnasoftServerMethodType =
    | keyof Omit<KhulnasoftServer, "dispose" | "setClient">
...
type FunctionsConfig = {
    [K in KhulnasoftServerMethodType]: {
        group: GroupKey;
        points: number;
    };
};
 ...
 *
 * @param name
 * @returns True iff the name is a valid function of the KhulnasoftServer interface. name is case sensitive.
 */
export function isValidFunctionName(name: string): boolean {
    const valid: boolean = !!defaultFunctions[name as any as KhulnasoftServerMethodType];
    return valid;
}
...
const defaultFunctions: FunctionsConfig = {
    ...
    getKhulnasoftTokenScopes: { group: "default", points: 1 },
    ...
    getKhulnasoftTokens: { group: "default", points: 1 },
    generateNewKhulnasoftToken: { group: "default", points: 1 },
    deleteKhulnasoftToken: { group: "default", points: 1 },
...
export class UserRateLimiter {
    ...
    async consume(user: string, method: string): Promise<RateLimiterRes> {
        const group = this.config.functions[method as KhulnasoftServerMethodType]?.group || "default";
        if (group !== this.config.functions[method as KhulnasoftServerMethodType]?.group) {
        ...
        const points = this.config.functions[method as KhulnasoftServerMethodType]?.points || 1;
