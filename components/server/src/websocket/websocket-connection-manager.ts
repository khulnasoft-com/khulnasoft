...
import {
    ...
    KhulnasoftServerPath,
    RateLimiterError,
    User,
} from "@khulnasoft/khulnasoft-protocol";
import { ApplicationError, ErrorCode, ErrorCodes } from "@khulnasoft/khulnasoft-protocol/lib/messaging/error";
import { ConnectionHandler } from "@khulnasoft/khulnasoft-protocol/lib/messaging/handler";
import {
    JsonRpcConnectionHandler,
    JsonRpcProxy,
    JsonRpcProxyFactory,
} from "@khulnasoft/khulnasoft-protocol/lib/messaging/proxy-factory";
...
import { KhulnasoftServerImpl } from "../workspace/khulnasoft-server-impl";
...
export type KhulnasoftServiceFactory = () => KhulnasoftServerImpl;
...
export type WebsocketClientType =
    ...
    | "supervisor"
    | "local-companion"
    | "io.khulnasoft.jetbrains.remote"
    | "io.khulnasoft.jetbrains.gateway";
    ...
    export function getClientType(req: express.Request): WebsocketClientType | undefined {
        ...
        if (userAgent) {
            if (userAgent.startsWith("Go-http-client")) {
                ...
                result = "browser";
            } else if (userAgent.startsWith("Khulnasoft Code")) {
                result = "khulnasoft-code";
            } else if (userAgent.startsWith("khulnasoft/supervisor")) {
                result = "supervisor";
            } else if (userAgent.startsWith("khulnasoft/local-companion")) {
                result = "local-companion";
            } else if (userAgent === "io.khulnasoft.jetbrains.remote" || userAgent === "io.khulnasoft.jetbrains.gateway") {
                result = userAgent;
            }
        }
        if (result === undefined) {
...
export namespace ClientMetadata {
    ...
    function getOriginWorkspaceId(req: express.Request): string | undefined {
        ...
        try {
            const u = new KhulnasoftHostUrl(origin);
...
export class WebsocketClientContext {
    ...
    protected servers: KhulnasoftServerImpl[] = [];

    get clientId(): string {
        return this.clientMetadata.id;
    }

    addEndpoint(server: KhulnasoftServerImpl) {
        this.servers.push(server);
    }

    removeEndpoint(server: KhulnasoftServerImpl) {
...
/**
 * Establishes and manages JsonRpc-over-websocket connections from frontends to KhulnasoftServerImpl instances
 */
export class WebsocketConnectionManager implements ConnectionHandler {
    public readonly path = KhulnasoftServerPath;

    protected readonly jsonRpcConnectionHandler: JsonRpcConnectionHandler<KhulnasoftApiClient>;
    protected readonly events = new EventEmitter();
    protected readonly contexts: Map<string, WebsocketClientContext> = new Map();

    constructor(
        protected readonly serverFactory: KhulnasoftServiceFactory,
        ...
        this.jsonRpcConnectionHandler = new KhulnasoftJsonRpcConnectionHandler<KhulnasoftApiClient>(
            this.path,
    ...
    protected createProxyTarget(
        client: JsonRpcProxy<KhulnasoftApiClient>,
        request?: object,
        connectionCtx?: TraceContext,
    ): KhulnasoftServerImpl {
        ...
        const khulnasoftServer = this.serverFactory();
        ...
        client.onDidCloseConnection(() => {
            try {
                khulnasoftServer.dispose();
                increaseApiConnectionClosedCounter();
                this.events.emit(EVENT_CONNECTION_CLOSED, khulnasoftServer, expressReq);

                clientContext.removeEndpoint(khulnasoftServer);
        ...
        clientContext.addEndpoint(khulnasoftServer);

        this.events.emit(EVENT_CONNECTION_CREATED, khulnasoftServer, expressReq);

        return new Proxy<KhulnasoftServerImpl>(khulnasoftServer, {
            get: (target, property: keyof KhulnasoftServerImpl) => {
    ...
    public onConnectionCreated(l: (server: KhulnasoftServerImpl, req: express.Request) => void): Disposable {
    ...
    public onConnectionClosed(l: (server: KhulnasoftServerImpl, req: express.Request) => void): Disposable {
...
class KhulnasoftJsonRpcConnectionHandler<T extends object> extends JsonRpcConnectionHandler<T> {
    constructor(
    ...
    onConnection(connection: MessageConnection, request?: object): void {
        ...
        const factory = new KhulnasoftJsonRpcProxyFactory<T>(
...
class KhulnasoftJsonRpcProxyFactory<T extends object> extends JsonRpcProxyFactory<T> {
    constructor(
    ...
    private async internalOnRequest(span: opentracing.Span, method: string, ...args: any[]): Promise<any> {
        ...
        try {
            ...
            // actual call
            const result = await this.target[method](ctx, ...args); // we can inject TraceContext here because of KhulnasoftServerWithTracing
