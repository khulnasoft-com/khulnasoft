...
@suite(timeout(10000), retries(2), skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_GITLAB")))
class TestGitlabRepositoryProvider {
    ...
    @test public async testSearchRepos_noMatchAgainstWholePath() {
        const result = await this.repositoryProvider.searchRepos(this.user, "khulnasoft-integration-tests/test", 100);
