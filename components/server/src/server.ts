...
import { createWebSocketConnection, toIWebSocket } from "@khulnasoft/khulnasoft-protocol/lib/messaging/node/connection";
...
import { Disposable, DisposableCollection } from "@khulnasoft/khulnasoft-protocol";
import { BearerAuth, isBearerAuthError } from "./auth/bearer-authenticator";
import { HostContextProvider } from "./auth/host-context-provider";
import { CodeSyncService } from "./code-sync/code-sync-service";
import { increaseHttpRequestCounter, observeHttpRequestDuration, setKhulnasoftVersion } from "./prometheus-metrics";
...
export class Server {
    ...
    public async init(app: express.Application) {
        ...
        setKhulnasoftVersion(this.config.version);
        ...
        this.eventEmitter.on(Server.EVENT_ON_START, (httpServer) => {
            ...
            //  - for cookie/session AND Bearer auth: MUST be hostUrl.hostname (khulnasoft.com)
            //  - edge case: empty "Origin" is always permitted
            // We rely on the origin header being set correctly (needed by regular clients to use Khulnasoft:
            // CORS allows subdomains to access khulnasoft.com)
