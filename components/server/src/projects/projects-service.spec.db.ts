...
import { ProjectDB, TypeORM, UserDB, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
import { resetDB } from "@khulnasoft/khulnasoft-db/lib/test/reset-db";
import { Organization, Project, ProjectSettings, User } from "@khulnasoft/khulnasoft-protocol";
...
import { daysBefore } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
...
describe("ProjectsService", async () => {
    ...
    it("prebuild settings migration / old and inactive project / uses defaults", async () => {
        const ps = container.get(ProjectsService);
        const cloneUrl = "https://github.com/khulnasoft-com/khulnasoft.git";
    ...
    it("should find projects by clone url", async () => {
        ...
        await createTestProject(ps, org, owner, {
            name: "my-project-3",
            cloneUrl: "https://github.com/khulnasoft-com/different-repo",
        });
