...
const shouldSkip =
    ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_READ") &&
    ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_WRITE") &&
    ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_ADMIN");
...
class TestBitbucketServerTokenValidator {
    ...
    // https://bitbucket.khulnasoft-dev.com/projects/TES/repos/read-write-permission/permissions
    ...
    @test(skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_READ"))) async test_checkWriteAccess_read_only() {
        const token = process.env["KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_READ"]!;
    ...
    @test(skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_WRITE")))
    async test_checkWriteAccess_write_permissions() {
        const token = process.env["KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_WRITE"]!;
        const result = await this.getValidator(token).checkWriteAccess(Object.assign({}, this.checkParams, { token }));
        expect(result).to.deep.equal({
            found: true,
            isPrivateRepo: true,
    ...
    @test(skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_ADMIN")))
    async test_checkWriteAccess_admin_permissions() {
        const token = process.env["KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER_ADMIN"]!;
        const result = await this.getValidator(token).checkWriteAccess(Object.assign({}, this.checkParams, { token }));
        expect(result).to.deep.equal({
            found: true,
            isPrivateRepo: true,
