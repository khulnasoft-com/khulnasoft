...
import { Repository, User } from "@khulnasoft/khulnasoft-protocol";
...
@suite(timeout(10000), retries(1), skip(ifEnvVarNotSet("KHULNASOFT_TEST_TOKEN_BITBUCKET_SERVER")))
class TestBitbucketServerFileProvider {
    ...
    @test async test_getKhulnasoftFileContent_ok() {
        const result = await this.service.getKhulnasoftFileContent(
            // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
            {
                revision: "main",
                repository: <Repository>{
                    cloneUrl: "https://bitbucket.khulnasoft-dev.com/users/filip/repos/spring-petclinic",
                    webUrl: "https://bitbucket.khulnasoft-dev.com/users/filip/repos/spring-petclinic",
