...
import { IAnalyticsWriter, NullAnalyticsWriter } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
...
import { testContainer } from "@khulnasoft/khulnasoft-db/lib";
...
import { WorkspaceClusterWoTLS } from "@khulnasoft/khulnasoft-protocol/lib/workspace-cluster";
...
export const mockAuthConfig: AuthConfig = {
    ...
    session: {
        issuer: "https://mp-server-d7650ec945.preview.khulnasoft-dev.com",
        lifetimeSeconds: 7 * 24 * 60 * 60,
        cookie: {
            name: "__Host-_khulnasoft_dev_jwt_",
            secure: true,
            httpOnly: true,
            maxAge: 7 * 24 * 60 * 60,
            sameSite: "lax",
...
const mockApplyingContainerModule = new ContainerModule((bind, unbound, isbound, rebind) => {
                    ...
                    <IImageBuilderClient & IWorkspaceManagerClient>{
                        ...
                        build(request, metadata, options) {
                            ...
                            nextTick(() => {
                                ...
                                logInfo.setUrl("https://ws.khulnasoft.com/my-test-image-build/logs");
                        ...
                        startWorkspace(request, metadata, options, callback) {
                            ...
                            response.setUrl(`https://${workspaceId}.ws.khulnasoft.com`);
                            callback(null, response);
                        },
                    },
                ) as any;
        ...
        createMock(<Partial<IDEServiceClient>>{
            async resolveWorkspaceConfig() {
                return {
                    ...
                    webImage: "khulnasoft/code:latest",
                    ideImageLayers: [],
                    refererIde: "code",
                    ideSettings: "",
                    tasks: "",
    ...
    rebind<Partial<Config>>(Config).toConstantValue({
        ...
        workspaceDefaults: {
            workspaceImage: "khulnasoft/workspace-full",
            defaultFeatureFlags: [],
            previewFeatureFlags: [],
        },
        workspaceClasses: [
        ...
        authProviderConfigs: [],
        installationShortname: "khulnasoft",
        auth: mockAuthConfig,
        prebuildLimiter: {
            "*": {
                limit: 50,
