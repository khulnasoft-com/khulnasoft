...
import { TeamDB } from "@khulnasoft/khulnasoft-db/lib";
import { InvalidKhulnasoftYMLError } from "@khulnasoft/public-api-common/lib/public-api-errors";
...
export class ConfigProvider {
    @inject(KhulnasoftFileParser) protected readonly khulnasoftParser: KhulnasoftFileParser;
    ...
    public async fetchCustomConfig(
        ...
        try {
            ...
            if (AdditionalContentContext.is(commit) && commit.additionalFiles[".khulnasoft.yml"]) {
                customConfigString = commit.additionalFiles[".khulnasoft.yml"];
                const parseResult = this.khulnasoftParser.parse(customConfigString);
                customConfig = parseResult.config;
                customConfig._origin = "additional-content";
                if (parseResult.validationErrors) {
                    const err = new InvalidKhulnasoftYMLError({
            ...
            if (!customConfig) {
                ...
                const contextRepoConfig = services.fileProvider.getKhulnasoftFileContent(commit, user);
            ...
            return { customConfig, configBasePath, literalConfig: { ".khulnasoft.yml": customConfigString || "" } };
        } catch (e) {
            TraceContext.setError({ span }, e);
            throw e;
        } finally {
    ...
    private async validateConfig(config: WorkspaceConfig, user: User): Promise<void> {
        ...
        if (checkoutLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Checkout location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .khulnasoft.yml file.`,
                );
            }
        }
        ...
        if (workspaceLocation) {
            ...
            if (this.leavesWorkspaceBase(normalizedPath)) {
                ...
                throw new Error(
                    `Workspace location must not leave the ${POD_PATH_WORKSPACE_BASE} folder. Check your .khulnasoft.yml file.`,
                );
            }
        }
    }
