...
import {
    ...
    UserDB,
    WorkspaceDB,
} from "@khulnasoft/khulnasoft-db/lib";
import { BlockedRepositoryDB } from "@khulnasoft/khulnasoft-db/lib/blocked-repository-db";
import {
...
import { IAnalyticsWriter, TrackMessage } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
import { AttributionId } from "@khulnasoft/khulnasoft-protocol/lib/attribution";
import { Deferred } from "@khulnasoft/khulnasoft-protocol/lib/util/deferred";
import { LogContext, log } from "@khulnasoft/khulnasoft-protocol/lib/util/logging";
import { TraceContext } from "@khulnasoft/khulnasoft-protocol/lib/util/tracing";
import { WorkspaceRegion } from "@khulnasoft/khulnasoft-protocol/lib/workspace-cluster";
...
import { isGrpcError } from "@khulnasoft/khulnasoft-protocol/lib/util/grpc";
...
import { IDESettingsVersion } from "@khulnasoft/khulnasoft-protocol/lib/ide-protocol";
...
export interface StartWorkspaceOptions extends Omit<KhulnasoftServer.StartWorkspaceOptions, "ideSettings"> {
...
export class WorkspaceStarter {
    ...
    public async stopWorkspaceInstance(
        ...
        try {
            ...
            if (workspace && instance && instance?.status.phase !== "stopped") {
                ...
                await this.userDB.trace({ span }).deleteKhulnasoftTokensNamedLike(workspace.ownerId, `${instance.id}-%`);
        ...
        if (blockedRepository.blockFreeUsage) {
            ...
            if (tier === "free") {
                throw new ApplicationError(
                    ...
                    `${contextURL} requires a paid plan on Khulnasoft.`,
                );
            }
        }
        if (!blockedRepository.blockFreeUsage) {
            throw new ApplicationError(ErrorCodes.PRECONDITION_FAILED, `${contextURL} is blocklisted on Khulnasoft.`);
        }
    }
    ...
    private async actuallyStartWorkspace(
        ...
        try {
            ...
            const additionalAuth = envVars.khulnasoftImageAuth || new Map<string, string>();
    ...
    private async createSpec(
        ...
        contextUrlEnv.setName("KHULNASOFT_WORKSPACE_CONTEXT_URL");
        ...
        contextEnv.setName("KHULNASOFT_WORKSPACE_CONTEXT");
        ...
        if (!!info) {
            ...
            workspaceClassInfoEnv.setName("KHULNASOFT_WORKSPACE_CLASS_INFO");
        ...
        const tasks = resolveKhulnasoftTasks(workspace, instance);
        if (tasks.length) {
        ...
        if (workspace.config.coreDump?.enabled) {
            ...
            rLimitCore.setName("KHULNASOFT_RLIMIT_CORE");
            rLimitCore.setValue(
                JSON.stringify({
        ...
        const createKhulnasoftTokenPromise = (async () => {
            const scopes = this.createDefaultKhulnasoftAPITokenScopes(workspace, instance);
            const token = crypto.randomBytes(30).toString("hex");
            const tokenHash = crypto.createHash("sha256").update(token, "utf8").digest("hex");
            const dbToken: KhulnasoftToken = {
                tokenHash,
                name: `${instance.id}-default`,
                type: KhulnasoftTokenType.MACHINE_AUTH_TOKEN,
                userId: user.id,
                scopes,
                created: new Date().toISOString(),
            };
            await this.userDB.trace(traceCtx).storeKhulnasoftToken(dbToken);
        ...
        sysEnvvars.push(
            newEnvVar("KHULNASOFT_COMMIT_ANNOTATION_ENABLED", organizationSettings.annotateGitCommits ? "true" : "false"),
        );

        const orgIdEnv = new EnvironmentVariable();
        orgIdEnv.setName("KHULNASOFT_DEFAULT_WORKSPACE_IMAGE");
        ...
        const [isSetJavaXmx, isSetJavaProcessorCount, disableJetBrainsLocalPortForwarding] = await Promise.all([
            client
                ...
                .then((v) => newEnvVar("KHULNASOFT_IS_SET_JAVA_XMX", String(v))),
            client
                .getValueAsync("supervisor_set_java_processor_count", false, { user })
                .then((v) => newEnvVar("KHULNASOFT_IS_SET_JAVA_PROCESSOR_COUNT", String(v))),
            client
                .getValueAsync("disable_jetbrains_local_port_forwarding", false, { user })
                .then((v) => newEnvVar("KHULNASOFT_DISABLE_JETBRAINS_LOCAL_PORT_FORWARDING", String(v))),
        ]);
        ...
        if (workspaceName && workspaceName.length > 0) {
            ...
            workspaceNameEnv.setName("KHULNASOFT_WORKSPACE_NAME");
            workspaceNameEnv.setValue(workspaceName);
            sysEnvvars.push(workspaceNameEnv);
        }
        const spec = new StartWorkspaceSpec();
        await createKhulnasoftTokenPromise;
    ...
    private createDefaultKhulnasoftAPITokenScopes(workspace: Workspace, instance: WorkspaceInstance): string[] {
        const scopes = [
            ...
            "function:openPort",
            "function:closePort",
            "function:generateNewKhulnasoftToken",
            "function:takeSnapshot",
            "function:waitForSnapshot",
            "function:stopWorkspace",
            "function:getToken",
            "function:getKhulnasoftTokenScopes",
            ...
            // getIDToken is used by Khulnasoft's OIDC Identity Provider to check for authorisation.
    ...
    public async resolveBaseImage(
        ...
        if (organizationId) {
            ...
            const additionalAuth = EnvVar.getKhulnasoftImageAuth(envVars);
            additionalAuth.forEach((val, key) => auth.getAdditionalMap().set(key, val));
        }
...
function resolveKhulnasoftTasks(ws: Workspace, instance: WorkspaceInstance): TaskConfig[] {
