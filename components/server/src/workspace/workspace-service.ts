...
import { ProjectDB, RedisPublisher, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
import {
    CommitContext,
...
import { ErrorCodes, ApplicationError } from "@khulnasoft/khulnasoft-protocol/lib/messaging/error";
...
import { WorkspaceRegion, isWorkspaceRegion } from "@khulnasoft/khulnasoft-protocol/lib/workspace-cluster";
...
import { goDurationToHumanReadable } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
...
import { WatchWorkspaceStatusResponse } from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { ContextParser } from "./context-parser-service";
import { scrubber, TrustedValue } from "@khulnasoft/khulnasoft-protocol/lib/util/scrubbing";
...
export class WorkspaceService {
    ...
    async getWorkspaces(userId: string, options: KhulnasoftServer.GetWorkspacesOptions): Promise<WorkspaceInfo[]> {
    ...
    public async sendHeartBeat(
        userId: string,
        options: KhulnasoftServer.SendHeartBeatOptions,
        check: (instance: WorkspaceInstance, workspace: Workspace) => Promise<void> = async () => {},
    ): Promise<void> {
    ...
    public async validateImageRef(ctx: TraceContext, user: User, imageRef: string, organizationId?: string) {
        try {
            ...
            // see https://github.com/khulnasoft-com/khulnasoft/blob/f3e41f8d86234e4101edff2199c54f50f8cbb656/components/image-builder-mk3/pkg/orchestrator/orchestrator.go#L561
    ...
    public async takeSnapshot(userId: string, options: KhulnasoftServer.TakeSnapshotOptions): Promise<Snapshot> {
