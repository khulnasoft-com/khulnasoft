...
import { TypeORM, WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib";
...
const khulnasoftEmptyContext = {
    ...
    repository: {
        host: "github.com",
        owner: "khulnasoft-com",
        name: "empty",
        cloneUrl: "https://github.com/khulnasoft-com/empty.git",
        defaultBranch: "main",
        private: false,
    },
    normalizedContextURL: "https://github.com/khulnasoft-com/empty",
    revision: "123456",
    title: "khulnasoft-com/empty - main",
};
...
const SNAPSHOT_BUCKET = "https://khulnasoft.com/none-bucket";
...
describe("ContextService", async () => {
    ...
    beforeEach(async () => {
        ...
        container.rebind(ConfigProvider).toConstantValue({
            fetchConfig: () => {
                return {
                    config: {
                        image: "khulnasoft/workspace-base",
                    },
                };
            },
            ...
            getDefaultImage: async (organizationId?: string): Promise<string> => {
                return "khulnasoft/workspace-base";
            },
        ...
        const bindContextParser = () => {
            container.rebind(ContextParser).toConstantValue({
                ...
                handle: async function (ctx: TraceContext, user: User, contextURL: string): Promise<WorkspaceContext> {
                    ...
                    cases.set("https://github.com/khulnasoft-com/empty", () => {
                        return khulnasoftEmptyContext as any;
                    });

                    if (prebuild) {
                        cases.set(
                            `open-prebuild/${prebuild.prebuildId}/https://github.com/khulnasoft-com/empty/tree/main`,
                            () => {
                                return {
                                    ...khulnasoftEmptyContext,
                    ...
                    async function createCommitContextForBranch(branchName: string): Promise<CommitContext> {
                        const branch = await mockRepositoryProvider.getBranch(user, "khulnasoft-com", "empty", branchName);
                        const r: CommitContext = {
                            title: branch.commit.commitMessage,
                            ref: branch.name,
                            refType: "branch",
                            revision: branch.commit.sha,
                            repository: await mockRepositoryProvider.getRepo(user, "khulnasoft-com", "empty"),
                            normalizedContextURL: branch.htmlUrl,
                        };
                        return r;
                    }

                    const branches = await mockRepositoryProvider.getBranches(user, "khulnasoft-com", "empty");
                    ...
                    for (const [_, b] of mockRepositoryProvider.branches) {
                        for (const commit of b.commits) {
                            const commitContextUrl = `https://github.com/khulnasoft-com/empty/commit/${commit.sha}`;
                    ...
                    return createCommitContextForBranch(khulnasoftEmptyContext.repository.defaultBranch);
                },
        ...
        mockRepositoryProvider.addBranch({ name: "main", htmlUrl: "https://github.com/khulnasoft-com/empty/tree/main" }, [
            {
                sha: khulnasoftEmptyContext.revision,
                author: "some-dude",
            ...
            project = await projectService.createProject(
                {
                    name: "my-project",
                    teamId: org.id,
                    cloneUrl: "https://github.com/khulnasoft-com/empty",
    ...
    it("should parse normal context", async () => {
        ...
        const ctx = await svc.parseContext(owner, "https://github.com/khulnasoft-com/empty", {
        ...
        expect(ctx.context.ref).to.equal(khulnasoftEmptyContext.ref);
        expect((ctx.context as CommitContext).revision).to.equal(khulnasoftEmptyContext.revision);
    });
    ...
    it("should ignore unfinished prebuild", async () => {
        ...
        mockRepositoryProvider.addBranch(
            { name: "branch-with-history", htmlUrl: "https://github.com/khulnasoft-com/empty/tree/branch-with-history" },
            [
                {
                    sha: revision1,
                    author: "some-dude",
        ...
        const svc = container.get(ContextService);
        const ctx = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/tree/branch-with-history`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
    ...
    it("should prefer perfect-match prebuild", async () => {
        ...
        const svc = container.get(ContextService);
        const ctx = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/000000`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
    ...
    it("should handle triggering prebuilds out of order with respect to commits", async () => {
        ...
        mockRepositoryProvider.addBranch(
            { name: branchName, htmlUrl: `https://github.com/khulnasoft-com/empty/tree/${branchName}` },
            [commit1],
        );
        ...
        const svc = container.get(ContextService);
        let ctx1 = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/${commit1.sha}`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
        const ctx2 = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/${commit2.sha}`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
        let ctx3 = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/${commit3.sha}`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
        ...
        ctx1 = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/${commit1.sha}`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
        ctx3 = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/commit/${commit3.sha}`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
        const ctxBranch = await svc.parseContext(owner, `https://github.com/khulnasoft-com/empty/tree/branch-2`, {
            projectId: project.id,
            organizationId: org.id,
            forceDefaultConfig: false,
        });
...
async function createTestWorkspace(svc: WorkspaceService, org: Organization, owner: User, project: Project) {
    const ws = await svc.createWorkspace(
        ...
        org.id,
        project,
        khulnasoftEmptyContext as any as CommitContext,
        "github.com/khulnasoft-com/empty",
        undefined,
    );
    return ws;
}
