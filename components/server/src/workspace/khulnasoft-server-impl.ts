...
import { UserDB, WorkspaceDB, DBWithTracing, TracedWorkspaceDB, TeamDB, DBKhulnasoftToken } from "@khulnasoft/khulnasoft-db/lib";
import {
    AuthProviderEntry,
    AuthProviderInfo,
    Configuration,
    ...
    KhulnasoftClient as KhulnasoftApiClient,
    KhulnasoftServer,
    KhulnasoftToken,
    KhulnasoftTokenType,
    PermissionName,
...
} from "@khulnasoft/khulnasoft-protocol";
import { BlockedRepository } from "@khulnasoft/khulnasoft-protocol/lib/blocked-repositories-protocol";
import {
...
} from "@khulnasoft/khulnasoft-protocol/lib/admin-protocol";
import { ApplicationError, ErrorCodes } from "@khulnasoft/khulnasoft-protocol/lib/messaging/error";
import { log, LogContext } from "@khulnasoft/khulnasoft-protocol/lib/util/logging";
import {
    InterfaceWithTraceContext,
    TraceContext,
    TraceContextWithSpan,
} from "@khulnasoft/khulnasoft-protocol/lib/util/tracing";
import { RemoteIdentifyMessage, RemotePageMessage, RemoteTrackMessage } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
import { SupportedWorkspaceClass } from "@khulnasoft/khulnasoft-protocol/lib/workspace-class";
...
import { HeadlessLogUrls } from "@khulnasoft/khulnasoft-protocol/lib/headless-workspace-log";
import { ProjectsService } from "../projects/projects-service";
import { IDEOption, IDEOptions } from "@khulnasoft/khulnasoft-protocol/lib/ide-protocol";
import {
    PartialProject,
    OrganizationSettings,
    Organization,
} from "@khulnasoft/khulnasoft-protocol/lib/teams-projects-protocol";
...
} from "@khulnasoft/khulnasoft-protocol/lib/protocol";
import { ListUsageRequest, ListUsageResponse } from "@khulnasoft/khulnasoft-protocol/lib/usage";
import { VerificationService } from "../auth/verification-service";
import { InstallationService } from "../auth/installation-service";
import { BillingMode } from "@khulnasoft/khulnasoft-protocol/lib/billing-mode";
...
import { CostCenterJSON } from "@khulnasoft/khulnasoft-protocol/lib/usage";
import { getExperimentsClientForBackend } from "@khulnasoft/khulnasoft-protocol/lib/experiments/configcat-server";
...
import { KhulnasoftTokenService } from "../user/khulnasoft-token-service";
...
export type KhulnasoftServerWithTracing = InterfaceWithTraceContext<KhulnasoftServer>;

@injectable()
export class KhulnasoftServerImpl implements KhulnasoftServerWithTracing, Disposable {
    constructor(
        ...
        @inject(KhulnasoftTokenService) private readonly khulnasoftTokenService: KhulnasoftTokenService,
    ...
    private client: KhulnasoftApiClient | undefined;
    ...
    initialize(
        client: KhulnasoftApiClient | undefined,
    ...
    private async listenForPrebuildUpdates(ctx?: TraceContext) {
        ...
        if (!this.disposables.disposed) {
            await runWithRequestContext(
                {
                    requestKind: "khulnasoft-server-impl-listener",
    ...
    setClient(ctx: TraceContext, client: KhulnasoftApiClient | undefined): void {
    ...
    public async getToken(ctx: TraceContext, query: KhulnasoftServer.GetTokenSearchOptions): Promise<Token | undefined> {
    ...
    public async startWorkspace(
        ctx: TraceContext,
        workspaceId: string,
        options: KhulnasoftServer.StartWorkspaceOptions,
    ...
    public async getWorkspaces(
        ctx: TraceContext,
        options: KhulnasoftServer.GetWorkspacesOptions,
    ...
    public async sendHeartBeat(ctx: TraceContext, options: KhulnasoftServer.SendHeartBeatOptions): Promise<void> {
        traceAPIParams(ctx, { options });
    ...
    public async createWorkspace(
        ctx: TraceContext,
        options: KhulnasoftServer.CreateWorkspaceOptions,
    ...
    async takeSnapshot(ctx: TraceContext, options: KhulnasoftServer.TakeSnapshotOptions): Promise<string> {
        traceAPIParams(ctx, { options });
    ...
    public async getKhulnasoftTokens(ctx: TraceContext): Promise<KhulnasoftToken[]> {
        const user = await this.checkAndBlockUser("getKhulnasoftTokens");
        const khulnasoftTokens = await this.khulnasoftTokenService.getKhulnasoftTokens(user.id, user.id);
        await Promise.all(khulnasoftTokens.map((tkn) => this.guardAccess({ kind: "khulnasoftToken", subject: tkn }, "get")));
        return khulnasoftTokens;
    }

    public async generateNewKhulnasoftToken(
        ctx: TraceContext,
        options: { name?: string; type: KhulnasoftTokenType; scopes?: string[] },
    ): Promise<string> {
        traceAPIParams(ctx, { options });

        const user = await this.checkAndBlockUser("generateNewKhulnasoftToken");
        return this.khulnasoftTokenService.generateNewKhulnasoftToken(user.id, user.id, options, (dbToken: DBKhulnasoftToken) => {
            return this.guardAccess({ kind: "khulnasoftToken", subject: dbToken }, "create");
        });
    }

    public async getKhulnasoftTokenScopes(ctx: TraceContext, tokenHash: string): Promise<string[]> {
        traceAPIParams(ctx, {}); // do not trace tokenHash

        const user = await this.checkAndBlockUser("getKhulnasoftTokenScopes");
        const khulnasoftToken = await this.khulnasoftTokenService.findKhulnasoftToken(user.id, user.id, tokenHash);
        if (khulnasoftToken) {
            await this.guardAccess({ kind: "khulnasoftToken", subject: khulnasoftToken }, "get");
        }
        return khulnasoftToken?.scopes ?? [];
    }

    public async deleteKhulnasoftToken(ctx: TraceContext, tokenHash: string): Promise<void> {
        traceAPIParams(ctx, {}); // do not trace tokenHash

        const user = await this.checkAndBlockUser("deleteKhulnasoftToken");
        return this.khulnasoftTokenService.deleteKhulnasoftToken(user.id, user.id, tokenHash, (token: KhulnasoftToken) => {
            return this.guardAccess({ kind: "khulnasoftToken", subject: token }, "delete");
        });
    }
    ...
    async updateOwnAuthProvider(
        ctx: TraceContext,
        { entry }: KhulnasoftServer.UpdateOwnAuthProviderParams,
    ...
    private redactUpdateOwnAuthProviderParams({ entry }: KhulnasoftServer.UpdateOwnAuthProviderParams) {
        const safeEntry =
            "id" in entry
    ...
    async deleteOwnAuthProvider(ctx: TraceContext, params: KhulnasoftServer.DeleteOwnAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async createOrgAuthProvider(
        ctx: TraceContext,
        { entry }: KhulnasoftServer.CreateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async updateOrgAuthProvider(
        ctx: TraceContext,
        { entry }: KhulnasoftServer.UpdateOrgAuthProviderParams,
    ): Promise<AuthProviderEntry> {
        traceAPIParams(ctx, {}); // entry contains PII
    ...
    async getOrgAuthProviders(
        ctx: TraceContext,
        params: KhulnasoftServer.GetOrgAuthProviderParams,
    ): Promise<AuthProviderEntry[]> {
        traceAPIParams(ctx, { params });
    ...
    async deleteOrgAuthProvider(ctx: TraceContext, params: KhulnasoftServer.DeleteOrgAuthProviderParams): Promise<void> {
        traceAPIParams(ctx, { params });
    ...
    async getOnboardingState(ctx: TraceContext): Promise<KhulnasoftServer.OnboardingState> {
    ...
    //#region khulnasoft.com concerns
