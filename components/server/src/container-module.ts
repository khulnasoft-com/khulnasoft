...
import { RedisPublisher, newRedisClient } from "@khulnasoft/khulnasoft-db/lib";
import { IAnalyticsWriter } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
import { KhulnasoftFileParser } from "@khulnasoft/khulnasoft-protocol/lib/khulnasoft-file-parser";
import { PrometheusClientCallMetrics } from "@khulnasoft/khulnasoft-protocol/lib/messaging/client-call-metrics";
import { newAnalyticsWriterFromEnv } from "@khulnasoft/khulnasoft-protocol/lib/util/analytics";
import { DebugApp } from "@khulnasoft/khulnasoft-protocol/lib/util/debug-app";
import {
    IClientCallMetrics,
    createClientCallMetricsInterceptor,
    defaultGRPCOptions,
} from "@khulnasoft/khulnasoft-protocol/lib/util/grpc";
import { prometheusClientMiddleware } from "@khulnasoft/khulnasoft-protocol/lib/util/nice-grpc";
...
export const productionContainerModule = new ContainerModule(
    (bind, unbind, isBound, rebind, unbindAsync, onActivation, onDeactivation) => {
        ...
        bind(KhulnasoftTokenService).toSelf().inSingletonScope();
        ...
        bind(KhulnasoftFileParser).toSelf().inSingletonScope();
        ...
        bind(ServerFactory).toAutoFactory(KhulnasoftServerImpl);
        ...
        bind(KhulnasoftServerImpl).toSelf();
        bind(WebsocketConnectionManager)
            .toDynamicValue((ctx) => {
                const serverFactory = () => ctx.container.get<KhulnasoftServerImpl>(KhulnasoftServerImpl);
