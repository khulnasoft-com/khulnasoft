...
import { Disposable, DisposableCollection, RunningWorkspaceInfo, WorkspaceInstance } from "@khulnasoft/khulnasoft-protocol";
import { inject, injectable } from "inversify";
import { Configuration } from "./config";
import { log, LogContext } from "@khulnasoft/khulnasoft-protocol/lib/util/logging";
import { Metrics } from "./metrics";
import { WorkspaceDB } from "@khulnasoft/khulnasoft-db/lib/workspace-db";
import { DBWithTracing, TracedUserDB, TracedWorkspaceDB } from "@khulnasoft/khulnasoft-db/lib/traced-db";
import { UserDB } from "@khulnasoft/khulnasoft-db/lib/user-db";
import { IAnalyticsWriter } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
import { ClientProvider } from "./wsman-subscriber";
import { repeat } from "@khulnasoft/khulnasoft-protocol/lib/util/repeat";
import { PrebuildUpdater } from "./prebuild-updater";
import { RedisPublisher } from "@khulnasoft/khulnasoft-db/lib";
import { durationLongerThanSeconds } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
...
export class WorkspaceInstanceControllerImpl implements WorkspaceInstanceController, Disposable {
    ...
    async onStopped(ctx: TraceContext, ownerUserID: string, instance: WorkspaceInstance): Promise<void> {
        ...
        try {
            await this.userDB.trace({ span }).deleteKhulnasoftTokensNamedLike(ownerUserID, `${instance.id}-%`);
