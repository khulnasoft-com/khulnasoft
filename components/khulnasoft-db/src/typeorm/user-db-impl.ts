...
import { EncryptionService } from "@khulnasoft/khulnasoft-protocol/lib/encryption/encryption-service";
import {
    DateInterval,
...
import { DBKhulnasoftToken } from "./entity/db-khulnasoft-token";
...
export class TypeORMUserDBImpl extends TransactionalDBImpl<UserDB> implements UserDB {
    ...
    private async getKhulnasoftTokenRepo(): Promise<Repository<DBKhulnasoftToken>> {
        return (await this.getEntityManager()).getRepository<DBKhulnasoftToken>(DBKhulnasoftToken);
    }
    ...
    public async findUserByKhulnasoftToken(
        tokenHash: string,
        tokenType?: KhulnasoftTokenType,
    ): Promise<{ user: User; token: KhulnasoftToken } | undefined> {
        const repo = await this.getKhulnasoftTokenRepo();
        const qBuilder = repo.createQueryBuilder("khulnasoftToken");
        if (!!tokenType) {
            qBuilder.where("khulnasoftToken.tokenHash = :tokenHash AND khulnasoftToken.type = :tokenType", {
                tokenHash,
                tokenType,
            });
        } else {
            qBuilder.where("khulnasoftToken.tokenHash = :tokenHash", { tokenHash });
        }
    ...
    public async findKhulnasoftTokensOfUser(userId: string, tokenHash: string): Promise<KhulnasoftToken | undefined> {
        const repo = await this.getKhulnasoftTokenRepo();
        const qBuilder = repo.createQueryBuilder("khulnasoftToken");
        qBuilder.where("userId = :userId AND khulnasoftToken.tokenHash = :tokenHash", { userId, tokenHash });
        return qBuilder.getOne();
    }

    public async findAllKhulnasoftTokensOfUser(userId: string): Promise<KhulnasoftToken[]> {
        const repo = await this.getKhulnasoftTokenRepo();
        const qBuilder = repo.createQueryBuilder("khulnasoftToken");
    ...
    public async storeKhulnasoftToken(token: KhulnasoftToken): Promise<void> {
        const repo = await this.getKhulnasoftTokenRepo();
        await repo.insert(token);
        await this.cache.invalidate(getUserCacheKey(token.userId));
    }

    public async deleteKhulnasoftToken(tokenHash: string): Promise<void> {
        const repo = await this.getKhulnasoftTokenRepo();
        await repo.delete({ tokenHash });
    }

    public async deleteKhulnasoftTokensNamedLike(userId: string, namePattern: string): Promise<void> {
        const repo = await this.getKhulnasoftTokenRepo();
    ...
    async issueRefreshToken(accessToken: OAuthToken): Promise<OAuthToken> {
        // NOTE(rl): this exists for the OAuth server code - Khulnasoft tokens are non-refreshable (atm)
    ...
    async persist(accessToken: OAuthToken): Promise<void> {
        ...
        let dbToken: KhulnasoftToken;
        const tokenHash = crypto.createHash("sha256").update(accessToken.accessToken, "utf8").digest("hex");
        const userAndToken = await this.findUserByKhulnasoftToken(tokenHash);
        if (userAndToken) {
            // Yes, update it (~)
            ...
            dbToken = userAndToken.token as KhulnasoftToken & { user: DBUser };
            const repo = await this.getKhulnasoftTokenRepo();
            ...
            return this.storeKhulnasoftToken(dbToken);
        }
    }
    async revoke(accessTokenToken: OAuthToken): Promise<void> {
        ...
        await this.deleteKhulnasoftToken(tokenHash);
    }
