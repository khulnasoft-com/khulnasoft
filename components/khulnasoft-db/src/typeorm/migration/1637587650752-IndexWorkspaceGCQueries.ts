    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        if (!(await indexExists(queryRunner, TABLE_NAME, CONTENT_DELETION_INDEX_NAME))) {
            await queryRunner.query(
                `CREATE INDEX ${CONTENT_DELETION_INDEX_NAME} ON ${TABLE_NAME} (contentDeletedTime, creationTime)`,
            );
        }
        ...
        if (!(await indexExists(queryRunner, TABLE_NAME, SOFT_DELETION_INDEX_NAME))) {
            await queryRunner.query(
                `CREATE INDEX ${SOFT_DELETION_INDEX_NAME} ON ${TABLE_NAME} (softDeletedTime, softDeleted)`,
            );
        }
    }
