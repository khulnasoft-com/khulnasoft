...
import { KhulnasoftTokenType, Identity, Workspace, WorkspaceInstance } from "@khulnasoft/khulnasoft-protocol";
...
class UserDBSpec {
    ...
    public async findTokenAndOwner() {
        ...
        await this.db.storeKhulnasoftToken({
            tokenHash: "tokenhash",
        ...
        const result = await this.db.findUserByKhulnasoftToken("tokenhash");
    ...
    @test(timeout(10000))
    public async findKhulnasoftTokenOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeKhulnasoftToken(token);
        const result = await this.db.findKhulnasoftTokensOfUser(user1.id, token.tokenHash);
        expect(result).to.not.be.undefined;
    ...
    @test(timeout(10000))
    public async findAllKhulnasoftTokensOfUser() {
        let user1 = await this.db.newUser();
        user1.name = "ABC";
        user1.identities.push(TestData.ID1);
        user1.identities.push(TestData.ID2);
        ...
        await this.db.storeKhulnasoftToken(token2);
        const result = await this.db.findAllKhulnasoftTokensOfUser(user1.id);
        expect(result).to.not.be.undefined;
