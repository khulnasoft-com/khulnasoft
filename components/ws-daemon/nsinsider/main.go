...
import (
	...
	_ "github.com/khulnasoft-com/khulnasoft/common-go/nsenter"
...
func main() {
		...
		Commands: []*cli.Command{
			...
			},
			{
				...
				Action: func(c *cli.Context) error {
					...
					// nft add table ip khulnasoft
					khulnasoftTable := nftcon.AddTable(&nftables.Table{
						Family: nftables.TableFamilyIPv4,
						Name:   "khulnasoft",
					})

					// nft add chain ip khulnasoft ratelimit { type filter hook postrouting priority 0 \; }
					ratelimit := nftcon.AddChain(&nftables.Chain{
						Table:    khulnasoftTable,
						Name:     "ratelimit",
					...
					// nft add counter khulnasoft connection_drop_stats
					nftcon.AddObject(&nftables.CounterObj{
						Table: khulnasoftTable,
						Name:  drop_stats,
					})

					// nft add set khulnasoft ws-connections { type ipv4_addr; flags timeout, dynamic; }
					set := &nftables.Set{
						Table:      khulnasoftTable,
						Name:       "ws-connections",
					...
					// nft add rule ip khulnasoft ratelimit ip protocol tcp ct state new meter ws-connections
	...
	err := app.Run(os.Args)
	if err != nil {
		log.WithField("instanceId", os.Getenv("KHULNASOFT_INSTANCE_ID")).WithField("args", os.Args).Fatal(err)
	}
}
