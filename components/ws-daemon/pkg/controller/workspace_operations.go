...
import (
	...
	glog "github.com/khulnasoft-com/khulnasoft/common-go/log"
	...
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/content"
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/dispatch"
...
func (wso *DefaultWorkspaceOperations) InitWorkspace(ctx context.Context, options InitOptions) (*csapi.InitializerMetrics, string, error) {
	...
	opts := content.RunInitializerOpts{
		...
		UID: (wsinit.KhulnasoftUID + 100000 - 1),
		GID: (wsinit.KhulnasoftGID + 100000 - 1),
		IdMappings: []archive.IDMapping{
			{ContainerID: 0, HostID: wsinit.KhulnasoftUID, Size: 1},
...
func (wso *DefaultWorkspaceOperations) writeImageInfo(_ context.Context, ws *session.Workspace, imageInfo *workspacev1.WorkspaceImageInfo) error {
	...
	uid := (wsinit.KhulnasoftUID + 100000 - 1)
	gid := (wsinit.KhulnasoftGID + 100000 - 1)
	fp := filepath.Join(ws.Location, ".khulnasoft/image")
