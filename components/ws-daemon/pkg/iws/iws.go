...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/common-go/cgroups"
	v2 "github.com/khulnasoft-com/khulnasoft/common-go/cgroups/v2"
	"github.com/khulnasoft-com/khulnasoft/common-go/log"
	"github.com/khulnasoft-com/khulnasoft/common-go/tracing"
	wsinit "github.com/khulnasoft-com/khulnasoft/content-service/pkg/initializer"
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/api"
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/container"
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/internal/session"
	nsi "github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/nsinsider"
)
...
func (wbs *InWorkspaceServiceServer) PrepareForUserNS(ctx context.Context, req *api.PrepareForUserNSRequest) (*api.PrepareForUserNSResponse, error) {
	...
	err = nsi.Nsinsider(wbs.Session.InstanceID, int(containerPID), func(c *exec.Cmd) {
		c.Args = append(c.Args, "prepare-dev", "--uid", strconv.Itoa(wsinit.KhulnasoftUID), "--gid", strconv.Itoa(wsinit.KhulnasoftGID))
	})
	if err != nil {
...
// └── workspace       drwxr-xr-x 5 khulnasoftUid khulnasoftGid
//
//	└── user        drwxr-xr-x 5 khulnasoftUid khulnasoftGid
func (wbs *InWorkspaceServiceServer) EvacuateCGroup(ctx context.Context, req *api.EvacuateCGroupRequest) (*api.EvacuateCGroupResponse, error) {
	...
	out, err := exec.CommandContext(ctx, "chown", "-R", fmt.Sprintf("%d:%d", wsinit.KhulnasoftUID, wsinit.KhulnasoftGID), filepath.Join(wbs.CGroupMountPoint, workspaceCGroup)).CombinedOutput()
	if err != nil {
