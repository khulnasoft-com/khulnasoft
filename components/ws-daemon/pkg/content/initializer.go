...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/ws-daemon/pkg/libcontainer/specconv"
)
...
func RunInitializer(ctx context.Context, destination string, initializer *csapi.WorkspaceInitializer, remoteContent map[string]storage.DownloadInfo, opts RunInitializerOpts) (*csapi.InitializerMetrics, error) {
	...
	if opts.GID == 0 {
		opts.GID = wsinit.KhulnasoftGID
	}
	if opts.UID == 0 {
		opts.UID = wsinit.KhulnasoftUID
	}
...
func RunInitializerChild(statsFd *os.File) (err error) {
	...
	initializer, err := wsinit.NewFromRequest(ctx, dst, rs, &req, wsinit.NewFromRequestOpts{ForceKhulnasoftUserForGit: false})
	if err != nil {
		return err
	}
	...
	err = wsinit.EnsureCleanDotKhulnasoftDirectory(ctx, dst)
	if err != nil {
		return err
	}
