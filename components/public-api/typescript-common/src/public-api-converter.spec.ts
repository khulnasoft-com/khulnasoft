...
import { OrganizationRole } from "@khulnasoft/public-api/lib/khulnasoft/v1/organization_pb";
import { BranchMatchingStrategy } from "@khulnasoft/public-api/lib/khulnasoft/v1/configuration_pb";
import { AuthProviderType } from "@khulnasoft/public-api/lib/khulnasoft/v1/authprovider_pb";
import { Workspace, WorkspacePhase_Phase, WorkspaceSession_Owner } from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { WorkspaceAndInstance } from "@khulnasoft/khulnasoft-protocol";

describe("PublicAPIConverter", () => {
    const converter = new PublicAPIConverter();

    const testKhulnasoftHost = "https://khulnasoft-test.preview.khulnasoft-dev.com/";
    ...
    describe("golden tests", () => {
        ...
        it("toPrebuild", async () => {
            await startFixtureTest("../fixtures/toPrebuild_*.json", async (input) =>
                converter.toPrebuild(testKhulnasoftHost, input),
            );
        });
    ...
    describe("errors", () => {
        ...
        it("INVALID_KHULNASOFT_YML", () => {
            const connectError = converter.toError(
                new InvalidKhulnasoftYMLError({
                    violations: ['Invalid value: "": must not be empty'],
                }),
            );
            expect(connectError.code).to.equal(Code.FailedPrecondition);
            expect(connectError.rawMessage).to.equal('Invalid khulnasoft.yml: Invalid value: "": must not be empty');

            const details = connectError.findDetails(FailedPreconditionDetails)[0];
            expect(details).to.not.be.undefined;
            expect(details?.reason?.case).to.equal("invalidKhulnasoftYml");
            expect(details?.reason?.value).to.be.instanceOf(InvalidKhulnasoftYMLErrorData);

            let violations = (details?.reason?.value as InvalidKhulnasoftYMLErrorData).violations;
            expect(violations).to.deep.equal(['Invalid value: "": must not be empty']);

            const appError = converter.fromError(connectError);
            expect(appError).to.be.instanceOf(InvalidKhulnasoftYMLError);
            expect(appError.code).to.equal(ErrorCodes.INVALID_KHULNASOFT_YML);
            expect(appError.message).to.equal('Invalid khulnasoft.yml: Invalid value: "": must not be empty');

            violations = (appError as InvalidKhulnasoftYMLError).info.violations;
            expect(violations).to.deep.equal(['Invalid value: "": must not be empty']);
        });
