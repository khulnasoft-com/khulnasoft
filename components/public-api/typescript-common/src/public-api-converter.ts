...
import { KhulnasoftServer } from "@khulnasoft/khulnasoft-protocol";
import { BlockedRepository as ProtocolBlockedRepository } from "@khulnasoft/khulnasoft-protocol/lib/blocked-repositories-protocol";
import { ContextURL } from "@khulnasoft/khulnasoft-protocol/lib/context-url";
import { ApplicationError, ErrorCodes } from "@khulnasoft/khulnasoft-protocol/lib/messaging/error";
import { RoleOrPermission as ProtocolRoleOrPermission } from "@khulnasoft/khulnasoft-protocol/lib/permission";
import {
    AuthProviderInfo,
    ...
    Configuration as KhulnasoftServerInstallationConfiguration,
    NavigatorContext,
    RefType,
    OrgEnvVar,
} from "@khulnasoft/khulnasoft-protocol/lib/protocol";
import { AuditLog as AuditLogProtocol } from "@khulnasoft/khulnasoft-protocol/lib/audit-log";
import {
    OrgMemberInfo,
...
import type { DeepPartial } from "@khulnasoft/khulnasoft-protocol/lib/util/deep-partial";
import { parseGoDurationToMs } from "@khulnasoft/khulnasoft-protocol/lib/util/timeutil";
import { SupportedWorkspaceClass } from "@khulnasoft/khulnasoft-protocol/lib/workspace-class";
import { isWorkspaceRegion } from "@khulnasoft/khulnasoft-protocol/lib/workspace-cluster";
import {
...
} from "@khulnasoft/khulnasoft-protocol/lib/workspace-instance";
import {
    AuthProvider,
    AuthProviderDescription,
    AuthProviderType,
    OAuth2Config,
} from "@khulnasoft/public-api/lib/khulnasoft/v1/authprovider_pb";
import { AuditLog } from "@khulnasoft/public-api/lib/khulnasoft/v1/auditlogs_pb";
import {
...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/configuration_pb";
import { EditorReference } from "@khulnasoft/public-api/lib/khulnasoft/v1/editor_pb";
import {
...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/envvar_pb";
import {
    CellDisabledError,
...
import {
    ...
    OnboardingState,
} from "@khulnasoft/public-api/lib/khulnasoft/v1/installation_pb";
import {
    OnboardingSettings,
...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/organization_pb";
import {
    Prebuild,
...
import { Author, Commit, SCMToken, SuggestedRepository } from "@khulnasoft/public-api/lib/khulnasoft/v1/scm_pb";
import { Sort, SortOrder } from "@khulnasoft/public-api/lib/khulnasoft/v1/sorting_pb";
import { SSHPublicKey } from "@khulnasoft/public-api/lib/khulnasoft/v1/ssh_pb";
import {
    Identity,
    RoleOrPermission,
...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/user_pb";
import {
    AdmissionLevel,
...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { BigIntToJson } from "@khulnasoft/khulnasoft-protocol/lib/util/stringify";
import { getPrebuildLogPath } from "./prebuild-utils";
import { InvalidKhulnasoftYMLError, RepositoryNotFoundError, UnauthorizedRepositoryAccessError } from "./public-api-errors";
...
export class PublicAPIConverter {
    ...
    toError(reason: unknown): ConnectError {
        ...
        if (reason instanceof ApplicationError) {
            ...
            if (reason instanceof InvalidKhulnasoftYMLError) {
                return new ConnectError(
                    reason.message,
                    ...
                    [
                        new FailedPreconditionDetails({
                            reason: {
                                case: "invalidKhulnasoftYml",
                                value: new InvalidKhulnasoftYMLErrorData(reason.info),
                            },
                        }),
                    ],
                    reason,
    ...
    fromError(reason: ConnectError): ApplicationError {
        ...
        if (reason.code === Code.FailedPrecondition) {
            ...
            switch (details?.reason?.case) {
                case "invalidKhulnasoftYml":
                    const invalidKhulnasoftYmlInfo = toPlainMessage(details.reason.value);
                    // eslint-disable-next-line @typescript-eslint/no-unsafe-argument
                    return new InvalidKhulnasoftYMLError(invalidKhulnasoftYmlInfo);
                case "repositoryNotFound":
    ...
    toPrebuilds(khulnasoftHost: string, prebuilds: PrebuildWithStatus[]): Prebuild[] {
        return prebuilds.map((prebuild) => this.toPrebuild(khulnasoftHost, prebuild));
    }

    toPrebuild(khulnasoftHost: string, prebuild: PrebuildWithStatus): Prebuild {
        return new Prebuild({
            id: prebuild.info.id,
            ...
            status: this.toPrebuildStatus(khulnasoftHost, prebuild),
        });
    }

    toPrebuildStatus(khulnasoftHost: string, prebuild: PrebuildWithStatus): PrebuildStatus {
        const tasks: TaskLog[] = [];
        let taskIndex = 0;
                    ...
                    new TaskLog({
                        ...
                        logUrl:
                            ...
                            task.before || task.init || task.prebuild
                                ? new URL(getPrebuildLogPath(prebuild.info.id, `${i}`), khulnasoftHost).toString()
                                : undefined,
                    }),
                );
            }
        ...
        // This is a hack mimicking the supervisor behavior of adding dynamic IDE tasks https://github.com/khulnasoft-com/khulnasoft/blob/e7d79c355e2cd6ac34056ea52d7bdcda45975839/components/ide-service/pkg/server/server.go#L508-L540
        ...
        return new PrebuildStatus({
            ...
            logUrl: new URL(getPrebuildLogPath(prebuild.info.id), khulnasoftHost).toString(),
            taskLogs: tasks,
            imageBuildLogUrl: new URL(getPrebuildLogPath(prebuild.workspace.id, "image-build"), khulnasoftHost).toString(),
        });
    }
    ...
    toOnboardingState(state: KhulnasoftServer.OnboardingState): OnboardingState {
    ...
    toInstallationConfiguration(config: KhulnasoftServerInstallationConfiguration): InstallationConfiguration {
