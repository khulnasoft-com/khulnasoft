...
func ExampleListAllWorkspaces() {
	...
	response, err := khulnasoft.Workspaces.ListWorkspaces(context.Background(), connect.NewRequest(&v1.ListWorkspacesRequest{}))
	if err != nil {
...
func ExampleGetWorkspace() {
	...
	response, err := khulnasoft.Workspaces.GetWorkspace(context.Background(), connect.NewRequest(&v1.GetWorkspaceRequest{
