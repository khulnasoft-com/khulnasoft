...
FROM khulnasoft/openvscode-server-linux-build-agent:centos7-devtoolset8-x64 as dependencies_builder
...
RUN mkdir /gp-code \
    && cd /gp-code \
    && git init \
    && git remote add origin https://github.com/khulnasoft-com/openvscode-server \
...
RUN cp /vscode-web/out/vs/khulnasoft/browser/workbench/workbench.html /vscode-web/index.html \
&& cp /vscode-web/out/vs/khulnasoft/browser/workbench/callback.html /vscode-web/callback.html \
&& sed -i -e "s/{{VERSION}}/$CODE_QUALITY-$CODE_COMMIT/g" /vscode-web/index.html

# cli config: alises to khulnasoft-code
RUN cp /vscode-reh-linux-x64/bin/remote-cli/khulnasoft-code /vscode-reh-linux-x64/bin/remote-cli/code \
&& cp /vscode-reh-linux-x64/bin/remote-cli/khulnasoft-code /vscode-reh-linux-x64/bin/remote-cli/gp-code \
&& cp /vscode-reh-linux-x64/bin/remote-cli/khulnasoft-code /vscode-reh-linux-x64/bin/remote-cli/open
...
ARG CODE_VERSION
ARG CODE_COMMIT
LABEL "io.khulnasoft.ide.version"=$CODE_VERSION
LABEL "io.khulnasoft.ide.commit"=$CODE_COMMIT
