...
func main() {
	...
	url, err := url.Parse(wsInfo.KhulnasoftHost)
	if err != nil {
		log.WithError(err).Errorf("failed to parse KhulnasoftHost %s", wsInfo.KhulnasoftHost)
		return
	}
	domain := url.Hostname()
	...
	args = append(args, "--install-builtin-extension", "khulnasoft.khulnasoft-theme")
	...
	cmdEnv := append(os.Environ(), fmt.Sprintf("KHULNASOFT_CODE_HOST=%s", domain))
	log.WithField("cost", time.Now().Local().Sub(startTime).Milliseconds()).Info("starting server")
	if err := syscall.Exec(Code, append([]string{"khulnasoft-code"}, args...), cmdEnv); err != nil {
...
func getExtensions(repoRoot string) (extensions []Extension, err error) {
	...
	if err != nil {
		...
		err = errors.New("read .khulnasoft.yml file failed: " + err.Error())
		return
	}
	var config *khulnasoft.KhulnasoftConfig
	if err = yaml.Unmarshal(data, &config); err != nil {
		err = errors.New("unmarshal .khulnasoft.yml file failed" + err.Error())
		return
	}
