...
export async function updateCodeIDEConfigMapJson() {
    ...
    const previousCodeVersion = await getIDEVersionOfImage("docker.io/khulnasoft/core-dev/build/" + ideConfigmapJson.ideOptions.options.code.image.replace("{{.Repository}}/", ""));
    const installationCodeVersion = await getIDEVersionOfImage("docker.io/khulnasoft/core-dev/build/" + newJson.ideOptions.options.code.image.replace("{{.Repository}}/", ""));
