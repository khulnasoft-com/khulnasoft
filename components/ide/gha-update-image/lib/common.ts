...
const getInstallerVersion = async (version: string | undefined) => {
    ...
    try {
        tagInfo =
            await $`git ls-remote --tags --sort=-v:refname https://github.com/khulnasoft-com/khulnasoft | grep ${v} | head -n1`.text();
    } catch (e) {
...
export const getLatestInstallerVersions = async (version?: string) => {
    ...
    const versionData =
        await $`docker run --rm docker.io/khulnasoft/core-dev/build/versions:${installationVersion} cat /versions.yaml`
            .text()
            .catch((e) => {
...
export const renderInstallerIDEConfigMap = async (version?: string) => {
    const installationVersion = await getInstallerVersion(version);
    await $`docker run --rm -v /tmp:/tmp docker.io/khulnasoft/core-dev/build/installer:${installationVersion} config init --overwrite --log-level=error -c /tmp/khulnasoft.config.yaml`.catch(
        (e) => {
            throw new Error("Failed to render khulnasoft.config.yaml: " + e);
        },
    );
    const ideConfigMapStr =
        await $`cat /tmp/khulnasoft.config.yaml | docker run -i --rm docker.io/khulnasoft/core-dev/build/installer:${installationVersion} ide-configmap -c -`
            .text()
            .catch((e) => {
...
export const getIDEVersionOfImage = async (img: string) => {
    console.log(
        ...
        `oci-tool fetch image ${img} | jq -r '.config.Labels["io.khulnasoft.ide.version"]'`,
    );
    const version = await $`oci-tool fetch image ${img} | jq -r '.config.Labels["io.khulnasoft.ide.version"]'`
        .text()
        .catch((e) => {
