...
const main = async () => {
    ...
    const commit =
        await $`curl -H 'Accept: application/vnd.github.VERSION.sha' https://api.github.com/repos/khulnasoft-com/openvscode-server/commits/${inputs.branch}`.text();

    const version = JSON.parse(
        await $`curl https://raw.githubusercontent.com/khulnasoft-com/openvscode-server/${commit}/package.json`.text(),
    ).version;

    console.log("fetch khulnasoft-com/openvscode-server with " + inputs.branch, { commit, version });
