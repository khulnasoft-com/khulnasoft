...
import io.khulnasoft.supervisor.api.Info.WorkspaceInfoResponse.WorkspaceClass
import io.khulnasoft.supervisor.api.Status.ResourceStatusSeverity
import kotlin.math.roundToInt

class KhulnasoftMetricProvider: MetricProvider {
    private val manager = service<KhulnasoftManager>()

    override val id: String = "khulnasoftMetricsProvider"
    override fun getMetrics(): Map<String, Metric> {
        ...
        return mapOf(
                "khulnasoft_workspace_cpu_used" to Metric(MetricType.PERFORMANCE, MetricsStatus.NORMAL, roundTo(cpuUsed, 0)),
                "khulnasoft_workspace_cpu_total" to Metric(MetricType.PERFORMANCE, MetricsStatus.NORMAL, roundTo(cpuTotal, 0)),
                "khulnasoft_workspace_cpu_percentage" to Metric(MetricType.PERFORMANCE, cpuStatus, (cpuPercentage * 1000.0).roundToInt() / 1000.0),
                "khulnasoft_workspace_memory_used" to Metric(MetricType.PERFORMANCE, MetricsStatus.NORMAL, roundTo(memoryUsed, 2)),
                "khulnasoft_workspace_memory_total" to Metric(MetricType.PERFORMANCE, MetricsStatus.NORMAL, roundTo(memoryTotal, 2)),
                "khulnasoft_workspace_memory_percentage" to Metric(MetricType.PERFORMANCE, memoryStatus, (memoryPercentage * 1000.0).roundToInt() / 1000.0),
                "khulnasoft_workspace_class" to Metric(MetricType.OTHER, MetricsStatus.NORMAL, workspaceClass)
        )
    }
