...
import io.khulnasoft.khulnasoftprotocol.api.entities.WorkspaceInstancePort
import io.khulnasoft.supervisor.api.Info
import io.khulnasoft.supervisor.api.Status
import io.khulnasoft.supervisor.api.Status.PortVisibility
...
abstract class AbstractKhulnasoftClientProjectSessionTracker(private val project: Project) : Disposable {
    ...
    private fun getForwardedPortUrl(port: PortsStatus): String {
        val localHostUri = serviceOrNull<KhulnasoftPortForwardingService>()
    ...
    private suspend fun makePortPublic(workspaceId: String, port: PortsStatus) {
        ...
        p.visibility = io.khulnasoft.khulnasoftprotocol.api.entities.PortVisibility.PUBLIC.toString()
        p.url = port.exposed.url

        try {
            manager.client.server.openPort(workspaceId, p).await()
        } catch (e: Exception) {
            thisLogger().error("khulnasoft: failed to open port ${port.localPort}: ", e)
        }
    }
    ...
    private val portsObserveJob = GlobalScope.launch {
        ...
        val status = StatusServiceGrpc.newStub(KhulnasoftManager.supervisorChannel)
        while (isActive) {
            try {
                ...
                thisLogger().error("khulnasoft: failed to stream ports status: ", t)
            }
            delay(1000L)
        }
    }
