...
import io.khulnasoft.supervisor.api.Status.PortsStatus
...
abstract class AbstractKhulnasoftPortForwardingService : KhulnasoftPortForwardingService {
    companion object {
        const val FORWARDED_PORT_LABEL = "ForwardedByKhulnasoft"
        const val EXPOSED_PORT_LABEL = "ExposedByKhulnasoft"
    }

    private val perClientPortForwardingManager = service<PerClientPortForwardingManager>()
    private val ignoredPortsForNotificationService = service<KhulnasoftIgnoredPortsForNotificationService>()
    ...
    private fun start() {
        ...
        if (isLocalPortForwardingDisabled()) {
            thisLogger().warn("khulnasoft: Local port forwarding is disabled.")
        }
            ...
            try {
                ...
                when (throwable) {
                    is InterruptedException, is CancellationException -> {
                        cancel("khulnasoft: Stopped observing ports list due to an expected interruption.")
                    }

                    else -> {
                        thisLogger().warn(
                            "khulnasoft: Got an error while trying to get ports list from Supervisor. " +
    ...
    private fun observePortsList(): CompletableFuture<Void> {
        ...
        val statusServiceStub = StatusServiceGrpc.newStub(KhulnasoftManager.supervisorChannel)
        ...
        val portsStatusResponseObserver = object :
            ...
            override fun beforeStart(request: ClientCallStreamObserver<Status.PortsStatusRequest>) {
                lifetime.onTerminationOrNow { request.cancel("khulnasoft: Service lifetime terminated.", null) }
            }
    ...
    private fun isLocalPortForwardingDisabled(): Boolean {
        return System.getenv("KHULNASOFT_DISABLE_JETBRAINS_LOCAL_PORT_FORWARDING")?.toBoolean() ?: false
    }
    ...
    private fun startForwarding(portStatus: PortsStatus) {
        ...
        try {
            ...
            if (throwable !is PortAlreadyForwardedException) {
                thisLogger().warn("khulnasoft: Caught an exception while forwarding port: ${throwable.message}")
            }
        }
    }
