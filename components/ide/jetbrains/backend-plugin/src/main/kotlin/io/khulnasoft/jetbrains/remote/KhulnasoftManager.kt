...
import io.khulnasoft.khulnasoftprotocol.api.entities.RemoteTrackMessage
import io.khulnasoft.jetbrains.remote.services.HeartbeatService
import io.khulnasoft.jetbrains.remote.utils.Retrier.retry
import io.khulnasoft.supervisor.api.*
import io.khulnasoft.supervisor.api.Info.WorkspaceInfoResponse
import io.khulnasoft.supervisor.api.Notification.*
...
@Service
class KhulnasoftManager : Disposable {
    ...
    private val backendKind = System.getenv("JETBRAINS_KHULNASOFT_BACKEND_KIND") ?: "unknown"
    ...
    init {
        ...
        // rate(khulnasoft_jb_backend_low_memory_after_gc_total[5m])
        val lowMemoryCounter = Counter.build()
            .name("khulnasoft_jb_backend_low_memory_after_gc")
    ...
    init {
        val monitoringJob = GlobalScope.launch {
            ...
            // max_over_time(khulnasoft_jb_backend_memory_used_bytes[5m:])/max_over_time(khulnasoft_jb_backend_memory_max_bytes[5m:])
            val allocatedGauge = Gauge.build()
                .name("khulnasoft_jb_backend_memory_max_bytes")
                .help("Total allocated memory of JB backend in bytes.")
                .labelNames("product", "qualifier")
                .register(registry)
            val usedGauge = Gauge.build()
                .name("khulnasoft_jb_backend_memory_used_bytes")
            ...
            while (isActive) {
                ...
                try {
                    ...
                    thisLogger().error("khulnasoft: failed to push monitoring metrics:", t)
                }
                delay(5000)
            }
        }
        ...
        GlobalScope.launch {
            ...
            try {
                ...
                if (response.statusCode() == 200) {
                    ...
                    // the output format should be constant (includes `Khulnasoft gateway link`)
                    // which will be checked in integration test gateway_test.go
                    thisLogger().warn(
                        "\n\n\n*********************************************************\n\n" +
                                "Khulnasoft gateway link: $gatewayLink" +
                                "\n\n*********************************************************\n\n\n"
                    )
                } else {
                ...
                thisLogger().error("khulnasoft: failed to resolve gateway link:", t)
            }
        }
    }
    ...
    val notificationGroup = NotificationGroupManager.getInstance().getNotificationGroup("Khulnasoft Notifications")
    private val notificationsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("khulnasoft: failed to stream notifications: ", t)
            }
            delay(1000L)
        }
    }
    ...
    private val serverJob = GlobalScope.launch {
        ...
        val tokenResponse = retry(3) {
            val request = Token.GetTokenRequest.newBuilder()
                    .setHost(info.khulnasoftApi.host)
        ...
        val plugin = PluginManagerCore.getPlugin(PluginId.getId("io.khulnasoft.jetbrains.remote"))!!
        val connect = {
            val originalClassLoader = Thread.currentThread().contextClassLoader
            try {
                val proxies = CommonProxy.getInstance().select(URL(info.khulnasoftHost))
        ...
        val khulnasoftHost = info.khulnasoftApi.host
        ...
        try {
            while (kotlin.coroutines.coroutineContext.isActive) {
                try {
                    ...
                    thisLogger().warn("$khulnasoftHost: connection closed, reconnecting after $reconnectionDelay milliseconds: $closeReason")
                } catch (t: Throwable) {
        ...
        thisLogger().warn("$khulnasoftHost: connection permanently closed: $closeReason")
    }
    init {
        lifetime.onTerminationOrNow {
            serverJob.cancel()
    ...
    fun trackEvent(eventName: String, props: Map<String, Any?>) {
        ...
        GlobalScope.launch {
            ...
            if (devMode) {
                thisLogger().warn("khulnasoft: $event")
            } else {
    ...
    private val metricsJob = GlobalScope.launch {
        ...
        while (isActive) {
            try {
                ...
                thisLogger().error("khulnasoft: failed to retrieve resource status: ", t)
            }
            delay(1000L)
        }
    }
    ...
    fun openUrlFromAction(url: String) {
        trackEvent("jb_execute_command_khulnasoft_open_link", mapOf("url" to url))
