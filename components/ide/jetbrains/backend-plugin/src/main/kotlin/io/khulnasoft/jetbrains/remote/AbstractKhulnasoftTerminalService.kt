...
import io.khulnasoft.supervisor.api.Status
import io.khulnasoft.supervisor.api.StatusServiceGrpc
import io.khulnasoft.supervisor.api.TerminalOuterClass
import io.khulnasoft.supervisor.api.TerminalServiceGrpc
...
abstract class AbstractKhulnasoftTerminalService(project: Project) : Disposable {
    private val lifetime = defineNestedLifetime()
    private val terminalServiceFutureStub = TerminalServiceGrpc.newFutureStub(KhulnasoftManager.supervisorChannel)
    private val terminalServiceStub = TerminalServiceGrpc.newStub(KhulnasoftManager.supervisorChannel)
    private val statusServiceStub = StatusServiceGrpc.newStub(KhulnasoftManager.supervisorChannel)
    ...
    protected fun start() {
        ...
        runJob(lifetime) {
            try {
                ...
                thisLogger().info("khulnasoft: attaching tasks ${tasks.size}, terminals ${terminals.size}")
                if (tasks.isEmpty() && terminals.isEmpty()) {
                    return@runJob
                }
                // see internal chat https://khulnasoft.slack.com/archives/C02BRJLGPGF/p1716540080028119
                delay(5000L)
                application.invokeLater {
                    createTerminalsAttachedToTasks(terminals, tasks)
                }
            } catch (e: TimeoutCancellationException) {
                thisLogger().error("khulnasoft: timeout while fetching tasks or terminals", e)
            } catch (e: Exception) {
                thisLogger().error("khulnasoft: error while attaching tasks", e)
            }
        }
    }
    ...
    private fun createTerminalsAttachedToTasks(
        ...
        tasks.forEachIndexed { index, task ->
            ...
            if (terminal == null) {
                thisLogger().warn("khulnasoft: found no terminal for task ${task.id}, expecting ${task.terminal}")
                return@forEachIndexed
            }
            val title = terminal.title.takeIf { !it.isNullOrBlank() } ?: "Khulnasoft Task ${index + 1}"
            thisLogger().info("khulnasoft: attaching task ${terminal.title} (${task.terminal}) with title $title")
            createAttachedSharedTerminal(title, terminal)
            thisLogger().info("khulnasoft: attached task ${terminal.title} (${task.terminal})")
        }
    }
    ...
    private tailrec suspend fun getSupervisorTasksList(): List<Status.TaskStatus> {
        ...
        try {
            ...
            thisLogger().error(
                "khulnasoft: Got an error while trying to get tasks list from Supervisor. Trying again in one second.",
                throwable
            )
        }
    ...
    private tailrec suspend fun getSupervisorTerminalsList(): List<TerminalOuterClass.Terminal> {
        ...
        try {
            ...
            thisLogger().error(
                "khulnasoft: Got an error while trying to get terminals list from Supervisor. Trying again in one second.",
                throwable
            )
        }
    ...
    private fun listenForTaskTerminationAndTitleChanges(
        ...
        while (hasOpenSessions) {
                    ...
                    override fun beforeStart(request: ClientCallStreamObserver<TerminalOuterClass.ListenTerminalRequest>) {
                        ...
                        shellTerminalWidget.addListener(object : TerminalWidgetListener {
                            override fun allSessionsClosed(widget: TerminalWidget) {
                                ...
                                request.cancel("khulnasoft: Terminal closed on the client.", null)
                            }
                        })
                    }
            ...
            try {
                ...
                thisLogger().error(
                    "khulnasoft: got an error while listening to '${supervisorTerminal.title}' terminal. Trying again in one second.",
                    throwable
                )
            }
        ...
        shellTerminalWidget.addListener(object : TerminalWidgetListener {
            override fun allSessionsClosed(widget: TerminalWidget) {
                runJob(lifetime) {
                    delay(5000)
                    try {
                        thisLogger().info("khulnasoft: shutdown task ${supervisorTerminal.title} (${supervisorTerminal.alias})")
                        ...
                        thisLogger().error(
                            "khulnasoft: got an error while shutting down '${supervisorTerminal.title}' terminal.",
                            throwable
                        )
                    }
                }
    ...
    private fun closeTerminalWidgetWhenClientGetsClosed(
        ...
        lifetime.onTerminationOrNow {
            thisLogger().debug("khulnasoft: closing task terminal service ${supervisorTerminal.title} (${supervisorTerminal.alias})")
            shellTerminalWidget.close()
        }
    }
}
