...
import io.khulnasoft.jetbrains.remote.utils.LocalHostUri
...
class KhulnasoftCLIService : RestService() {
    ...
    override fun execute(urlDecoder: QueryStringDecoder, request: FullHttpRequest, context: ChannelHandlerContext): String? {
        ...
        /**
         * prod: curl http://localhost:63342/api/khulnasoft/cli?op=metrics
         * dev:  curl http://localhost:63343/api/khulnasoft/cli?op=metrics
         *
        ...
        if (operation == "preview") {
            ...
            return withClient(request, context) { project ->
                ...
                val khulnasoftPortForwardingService = serviceOrNull<KhulnasoftPortForwardingService>()

                if (localHostUriMetadata.isPresent && khulnasoftPortForwardingService != null) {
                    var localHostUriFromPort = Optional.empty<URI>()

                    application.invokeAndWait {
                        localHostUriFromPort = khulnasoftPortForwardingService
    ...
    private fun parseFilePath(path: String): Path? {
        return try {
            ...
            thisLogger().warn("khulnasoft cli: failed to parse file path:", e)
            null
        }
    }

    companion object {
        const val SERVICE_NAME = "khulnasoft/cli"
    }
}
