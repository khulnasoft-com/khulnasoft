...
DEBUG_PORT=44444
JB_QUALIFIER="latest"
TEST_REPO=https://github.com/khulnasoft-samples/spring-petclinic
RUN_FROM="release"
...
TEST_PLUGIN_DIR="$TEST_PLUGINS_DIR/khulnasoft-remote"
rm -rf $TEST_PLUGIN_DIR

KHULNASOFT_PLUGIN_DIR=/workspace/khulnasoft/components/ide/jetbrains/backend-plugin
$KHULNASOFT_PLUGIN_DIR/gradlew -PenvironmentName="$JB_QUALIFIER" buildPlugin

# TODO(ak) actually should be gradle task to make use of output
KHULNASOFT_PLUGIN_DIST="$KHULNASOFT_PLUGIN_DIR/build/distributions/khulnasoft-remote.zip"
unzip $KHULNASOFT_PLUGIN_DIST -d $TEST_PLUGINS_DIR
...
export CWM_HOST_STATUS_OVER_HTTP_TOKEN=khulnasoft

# Build and move idea-cli, then overwrite environment variables initially defined by `components/ide/jetbrains/image/blazedock.Dockerfile`
...
export JETBRAINS_KHULNASOFT_BACKEND_KIND=intellij
