...
var pluginVersion = "${properties("pluginVersion")}-${properties("khulnasoftVersion")}"
...
project(":") {
    kotlin {
        ...
        sourceSets["main"].kotlin.exclude("io/khulnasoft/jetbrains/remote/${excludedPackage}/**")

        if (properties("platformType") == "RD") {
            print("Rider: exclude unnecessary files")
            sourceSets["main"].kotlin.exclude("**/KhulnasoftForceUpdateMavenProjectsActivity.kt")
...
dependencies {
    ...
    implementation(project(":khulnasoft-protocol")) {
        artifact {
            type = "jar"
        }
    }
...
tasks.withType<KotlinCompile> {
    if (properties("platformType") == "RD") {
        print("Rider: exclude unnecessary files")
        exclude("**/KhulnasoftForceUpdateMavenProjectsActivity.kt")
        exclude("**/maven.xml")
    }
}
