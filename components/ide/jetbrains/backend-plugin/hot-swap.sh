...
clusterHost=$(kubectl exec -it "$podName" -- printenv KHULNASOFT_WORKSPACE_CLUSTER_HOST |sed -e "s/\s//g")
...
# prepare build
component="khulnasoft-remote-$qualifier-$(date +%F_T"%H-%M-%S")"
tarDir="/tmp/hot-swap/$component"
mkdir -p "$tarDir"
echo "Build Dir: $tarDir"

# prepare ssh
ownerToken=$(kubectl get pod "$podName" -o=json | jq ".metadata.annotations.\"khulnasoft\/ownerToken\"" -r)
...
scp -F "$sshConfig" -r "$tarDir/build/khulnasoft-remote" "$workspaceId":"$uploadDest"

# link
link="/ide-desktop/backend/plugins/khulnasoft-remote"
