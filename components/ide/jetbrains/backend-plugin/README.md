...
<!-- Plugin description -->
Provides integrations within a Khulnasoft workspace.
...
The ideal setup to develop this plugin is using IntelliJ in Khulnasoft.

1. Choose IntelliJ as your editor in [khulnasoft.com/preferences](https://khulnasoft.com/preferences)
...
-r # Specify a test repository (e.g. -r https://github.com/khulnasoft-com/empty)
-p # Specify a debug port (useful if used in combination with the Remote JVM debugger in IntelliJ)
```

To use the script, from your khulnasoft's workspace:
...
1. Launch the script:
  ...
  ```
2. Connect the IDE to the test backend instance. Find the `Khulnasoft gateway link` in the logs and open it, e.g:
  ```console
  *********************************************************

  Khulnasoft gateway link: jetbrains-gateway://connect#khulnasoftHost=ak-jb-backend-debug-flow.staging.khulnasoft-dev.com&workspaceId=khulnasoftio-khulnasoft-rvg0x7havor&backendPort=63343

  *********************************************************
  ```

By default, the test-repository is [khulnasoft-samples/spring-petclinic](https://github.com/khulnasoft-samples/spring-petclinic). You can specify a different test repo using the `-r` argument, e.g:

```bash
./launch-dev-server.sh -r https://github.com/khulnasoft-com/empty
```
