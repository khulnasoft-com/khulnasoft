...
func TestGetProductConfig(t *testing.T) {
	...
	actual := getProductConfig(&protocol.KhulnasoftConfig{
...
func TestParseKhulnasoftConfig(t *testing.T) {
	khulnasoftConfig, _ := parseKhulnasoftConfig("testdata")
	assert.Equal(t, 1, len(khulnasoftConfig.Jetbrains.Intellij.Plugins))
	assert.Equal(t, "both", khulnasoftConfig.Jetbrains.Intellij.Prebuilds.Version)
	assert.Equal(t, "-Xmx3g", khulnasoftConfig.Jetbrains.Intellij.Vmoptions)
	assert.Equal(t, "-Xmx4096m -XX:MaxRAMPercentage=75", khulnasoftConfig.Jetbrains.Goland.Vmoptions)
}
...
func TestUpdateVMOptions(t *testing.T) {
	tests := []struct {
		...
		{"idea64.vmoptions (KHULNASOFT_CPU_COUNT)", "intellij", map[string]string{"INTELLIJ_VMOPTIONS": "-Xmx4096m", "KHULNASOFT_CPU_COUNT": "1"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx4096m\n-Dsun.tools.attach.tmp.only=true\n-XX:+UseContainerSupport\n-XX:ActiveProcessorCount=1\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true"},
		{"idea64.vmoptions (KHULNASOFT_CPU_COUNT 2)", "intellij", map[string]string{"INTELLIJ_VMOPTIONS": "-Xmx4096m", "KHULNASOFT_CPU_COUNT": "12"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx4096m\n-Dsun.tools.attach.tmp.only=true\n-XX:+UseContainerSupport\n-XX:ActiveProcessorCount=12\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true"},
		...
		{"KHULNASOFT_MEMORY with 1G", "intellij", map[string]string{"KHULNASOFT_MEMORY": "1024"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true\n-XX:+UseContainerSupport\n"},
		{"KHULNASOFT_MEMORY with 4G", "intellij", map[string]string{"KHULNASOFT_MEMORY": "4096"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx2457m\n-Dsun.tools.attach.tmp.only=true\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true\n-XX:+UseContainerSupport\n"},
		{"KHULNASOFT_MEMORY with 256G will use max 8G", "intellij", map[string]string{"KHULNASOFT_MEMORY": "262144"}, "-Xms128m\n-Xmx2g\n-Dsun.tools.attach.tmp.only=true", "-Xms128m\n-Xmx8192m\n-Dsun.tools.attach.tmp.only=true\n-Dfreeze.reporter.profiling=false\n-Dgtw.disable.exit.dialog=true\n-Djdk.configure.existing=true\n-XX:+UseContainerSupport\n"},
	}
	os.Unsetenv("KHULNASOFT_CPU_COUNT")
	os.Unsetenv("KHULNASOFT_MEMORY")
