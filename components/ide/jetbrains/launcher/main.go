...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/jetbrains/launcher/pkg/constant"
...
type LaunchContext struct {
	...
	// shouldWaitBackendPlugin is controlled by env KHULNASOFT_WAIT_IDE_BACKEND
...
func main() {
	...
	// supervisor refer see https://github.com/khulnasoft-com/khulnasoft/blob/main/components/supervisor/pkg/supervisor/supervisor.go#L961
	shouldWaitBackendPlugin := os.Getenv("KHULNASOFT_WAIT_IDE_BACKEND") == "true"
	debugEnabled := os.Getenv("SUPERVISOR_DEBUG_ENABLE") == "true"
	preferToolbox := os.Getenv("KHULNASOFT_PREFER_TOOLBOX") == "true"
...
// isBackendPluginReady checks if the backend plugin is ready via backend plugin CLI KhulnasoftCLIService.kt
func isBackendPluginReady(ctx context.Context, backendPort string, shouldWaitBackendPlugin bool) error {
	...
	url, err := url.Parse("http://localhost:" + backendPort + "/api/khulnasoft/cli?op=metrics")
	if err != nil {
		return err
	}
...
func resolveToolboxLink(wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	khulnasoftUrl, err := url.Parse(wsInfo.KhulnasoftHost)
	if err != nil {
		return "", err
	}
	...
	link := url.URL{
		Scheme:   "jetbrains",
		Host:     "gateway",
		Path:     "io.khulnasoft.toolbox.gateway/open-in-toolbox",
		RawQuery: fmt.Sprintf("host=%s&workspaceId=%s&debugWorkspace=%t", khulnasoftUrl.Hostname(), wsInfo.WorkspaceId, debugWorkspace),
	}
...
func resolveGatewayLink(backendPort string, wsInfo *supervisor.WorkspaceInfoResponse) (string, error) {
	...
	link := url.URL{
		...
		Host:     "connect",
		Fragment: fmt.Sprintf("khulnasoftHost=%s&workspaceId=%s&backendPort=%s&debugWorkspace=%t", khulnasoftUrl.Hostname(), wsInfo.WorkspaceId, backendPort, debugWorkspace),
	}
	return link.String(), nil
}
...
func resolveJsonLink(backendPort string) (string, error) {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=khulnasoft"
...
func terminateIDE(backendPort string) error {
	var (
		hostStatusUrl = "http://localhost:" + backendPort + "/codeWithMe/unattendedHostStatus?token=khulnasoft&exit=true"
...
func launch(launchCtx *LaunchContext) {
	...
	khulnasoftConfig, err := parseKhulnasoftConfig(projectDir)
	if err != nil {
		log.WithError(err).Error("failed to parse .khulnasoft.yml")
	}
	...
	err = configureVMOptions(khulnasoftConfig, launchCtx.alias, launchCtx.vmOptionsFile)
	if err != nil {
	...
	err = installPlugins(khulnasoftConfig, launchCtx)
	...
	// install khulnasoft plugin
	err = linkRemotePlugin(launchCtx)
	if err != nil {
		log.WithError(err).Error("failed to install khulnasoft-remote plugin")
	}
...
func run(launchCtx *LaunchContext) {
	...
	cmd.Env = append(cmd.Env, "JETBRAINS_KHULNASOFT_BACKEND_KIND="+launchCtx.alias)
	workspaceUrl, err := url.Parse(launchCtx.wsInfo.WorkspaceUrl)
	if err == nil {
		cmd.Env = append(cmd.Env, "JETBRAINS_KHULNASOFT_WORKSPACE_HOST="+workspaceUrl.Hostname())
	}
	// Enable host status endpoint
	cmd.Env = append(cmd.Env, "CWM_HOST_STATUS_OVER_HTTP_TOKEN=khulnasoft")
...
func configureVMOptions(config *khulnasoft.KhulnasoftConfig, alias string, vmOptionsPath string) error {
...
func updateVMOptions(
	config *khulnasoft.KhulnasoftConfig,
	alias string,
	...
	// Khulnasoft's default customization
	var khulnasoftVMOptions []string
	khulnasoftVMOptions = append(khulnasoftVMOptions, "-Dgtw.disable.exit.dialog=true")
	// temporary disable auto-attach of the async-profiler to prevent JVM crash
	// see https://youtrack.jetbrains.com/issue/IDEA-326201/SIGSEGV-on-startup-2023.2-IDE-backend-on-khulnasoft.com?s=SIGSEGV-on-startup-2023.2-IDE-backend-on-khulnasoft.com
	khulnasoftVMOptions = append(khulnasoftVMOptions, "-Dfreeze.reporter.profiling=false")
	if alias == "intellij" {
		khulnasoftVMOptions = append(khulnasoftVMOptions, "-Djdk.configure.existing=true")
	}
	// container relevant options
	khulnasoftVMOptions = append(khulnasoftVMOptions, "-XX:+UseContainerSupport")
	cpuCount := os.Getenv("KHULNASOFT_CPU_COUNT")
	...
	if err == nil && parsedCPUCount > 0 && parsedCPUCount <= 16 {
		khulnasoftVMOptions = append(khulnasoftVMOptions, "-XX:ActiveProcessorCount="+cpuCount)
	}

	memory := os.Getenv("KHULNASOFT_MEMORY")
	...
	if err == nil && parsedMemory > 0 {
		...
		if xmx > minDefaultXmx {
			khulnasoftVMOptions = append(khulnasoftVMOptions, fmt.Sprintf("-Xmx%dm", int(xmx)))
		}
	}

	vmoptions := deduplicateVMOption(ideaVMOptionsLines, khulnasoftVMOptions, filterFunc)
	...
	// project-defined vmoptions (.khulnasoft.yml)
	if config != nil {
...
func collectSyncSources(launchCtx *LaunchContext, target SyncTarget) ([]string, error) {
	...
	for _, srcDir := range []string{
		fmt.Sprintf("%s/.khulnasoft/jetbrains/%s", userHomeDir, target),
		fmt.Sprintf("%s/.khulnasoft/jetbrains/%s/%s", userHomeDir, launchCtx.alias, target),
		fmt.Sprintf("%s/.khulnasoft/jetbrains/%s", launchCtx.projectDir, target),
		fmt.Sprintf("%s/.khulnasoft/jetbrains/%s/%s", launchCtx.projectDir, launchCtx.alias, target),
	} {
...
func installPlugins(config *khulnasoft.KhulnasoftConfig, launchCtx *LaunchContext) error {
...
func getPlugins(config *khulnasoft.KhulnasoftConfig, alias string) ([]string, error) {
	var plugins []string
...
func getProductConfig(config *khulnasoft.KhulnasoftConfig, alias string) *khulnasoft.JetbrainsProduct {
	defer func() {
	...
	productConfig, ok := v.(*khulnasoft.JetbrainsProduct)
	if !ok {
		return nil
	}
...
func linkRemotePlugin(launchCtx *LaunchContext) error {
	...
	remotePluginDir := remotePluginsFolder + "/khulnasoft-remote"
	if err := os.MkdirAll(remotePluginsFolder, 0755); err != nil {
		return err
	}

	// added for backwards compatibility, can be removed in the future
	sourceDir := "/ide-desktop-plugins/khulnasoft-remote-" + os.Getenv("JETBRAINS_BACKEND_QUALIFIER")
	_, err := os.Stat(sourceDir)
	if err == nil {
		return safeLink(sourceDir, remotePluginDir)
	}

	return safeLink("/ide-desktop-plugins/khulnasoft-remote", remotePluginDir)
}
...
func configureToolboxCliProperties(backendDir string) error {
	...
	debuggingToolbox := os.Getenv("KHULNASOFT_TOOLBOX_DEBUGGING")
