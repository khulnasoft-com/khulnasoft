...
package io.khulnasoft.jetbrains.auth
...
@Service
internal class KhulnasoftAuthService : OAuthServiceBase<Credentials>() {
    override val name: String
        get() = SERVICE_NAME

    fun authorize(khulnasoftHost: String): CompletableFuture<Credentials> {
        return authorize(KhulnasoftAuthRequest(khulnasoftHost))
    }
    ...
    private class KhulnasoftAuthRequest : OAuthRequest<Credentials> {
        ...
        constructor(khulnasoftHost: String) {
            val codeVerifier = generateCodeVerifier()
            val codeChallenge = generateCodeChallenge(codeVerifier)
            val serviceUrl = newFromEncoded("https://$khulnasoftHost/api/oauth")
            credentialsAcquirer = KhulnasoftAuthCredentialsAcquirer(
    ...
    private class KhulnasoftAuthCredentialsAcquirer(
    ...
    companion object {
        ...
        val instance: Supplier<KhulnasoftAuthService> = CachedSingletonsRegistry.lazy { service() }

        private const val SERVICE_NAME = "khulnasoft/oauth"
        private const val CLIENT_ID = "jetbrains-gateway-khulnasoft-plugin"
        val scopes = arrayOf(
        ...
        fun hasAccessToken(khulnasoftHost: String) =
            getAccessToken(khulnasoftHost) != null

        fun getAccessToken(khulnasoftHost: String) =
            System.getenv("KHULNASOFT_TEST_ACCESSTOKEN") ?: PasswordSafe.instance.getPassword(getAccessTokenCredentialAttributes(khulnasoftHost))

        fun setAccessToken(khulnasoftHost: String, accessToken: String?) {
            PasswordSafe.instance.setPassword(getAccessTokenCredentialAttributes(khulnasoftHost), accessToken)
            dispatcher.multicaster.didChange()
        }

        suspend fun authorize(khulnasoftHost: String): String {
            val accessToken = instance.get().authorize(khulnasoftHost).await().accessToken
            setAccessToken(khulnasoftHost, accessToken)
            return accessToken
        }

        private fun getAccessTokenCredentialAttributes(khulnasoftHost: String) =
            CredentialAttributes(generateServiceName("Khulnasoft", khulnasoftHost))
