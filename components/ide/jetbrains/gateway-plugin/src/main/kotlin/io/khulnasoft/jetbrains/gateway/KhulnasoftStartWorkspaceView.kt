...
import io.khulnasoft.khulnasoftprotocol.api.entities.IDEOption
...
class KhulnasoftStartWorkspaceView(
    lifetime: Lifetime
) {
    ...
    val component = panel {
        ...
        row {
            ...
            val contextUrl = textField()
                ...
                .align(AlignX.FILL)
                .applyToComponent {
                    this.text = "https://github.com/khulnasoft-samples/spring-petclinic"
                }
                ...
                if (!hasContextUrlChangedFromUi) {
                    contextUrl.applyToComponent {
                        backendsModel.selectedItem.let {
                            text = when(it) {
                                "IntelliJ IDEA" -> "https://github.com/khulnasoft-samples/spring-petclinic"
                                "WebStorm" -> "https://github.com/khulnasoft-samples/template-typescript-react"
                                "PyCharm" -> "https://github.com/khulnasoft-samples/template-python-django"
                                "GoLand" -> "https://github.com/khulnasoft-samples/template-golang-cli"
                                "Rider" -> "https://github.com/khulnasoft-samples/template-dotnet-core-cli-csharp"
                                "CLion" -> "https://github.com/khulnasoft-samples/template-cpp"
                                "RubyMine" -> "https://github.com/khulnasoft-samples/template-ruby-on-rails-postgres"
                                "PhpStorm" -> "https://github.com/khulnasoft-samples/template-php-laravel-mysql"
                                "RustRover" -> "https://github.com/khulnasoft-samples/template-rust-cli"
                                else -> "https://github.com/khulnasoft-com/empty"
                            }
                        }
                    }
                }
                ...
                if (contextUrl.component.text.isNotBlank()) {
                    backendsModel.selectedItem?.let {
                        backendToId[it]?.let { backend ->
                            BrowserUtil.browse("https://${settings.khulnasoftHost}#referrer:jetbrains-gateway:$backend/${contextUrl.component.text}")
                        }
                    }
                }
    ...
    init {
        ...
        toDispose.add(KhulnasoftAuthService.addListener { update() })
