...
class KhulnasoftWebSocketTunnelServer(
    ...
    private val clients = CopyOnWriteArrayList<KhulnasoftWebSocketTunnelClient>()

    fun start(lifetime: Lifetime) {
        val job = GlobalScope.launch(Dispatchers.IO) {
            thisLogger().info("khulnasoft: tunnel[$url]: listening on port $port")
            try {
                while (isActive) {
                    try {
                        ...
                        if (isActive) {
                            thisLogger().error("khulnasoft: tunnel[$url]: failed to accept", t)
                        }
                    }
                }
            } catch (t: Throwable) {
                if (isActive) {
                    thisLogger().error("khulnasoft: tunnel[$url]: failed to listen", t)
                }
            } finally {
                thisLogger().info("khulnasoft: tunnel[$url]: stopped")
            }
        }
    ...
    private fun handleClientConnection(clientSocket: Socket) {
        val socketClient = KhulnasoftWebSocketTunnelClient(url, clientSocket)
        try {
            ...
            socketClient.onMessageCallback = { data ->
                ...
                thisLogger().trace("khulnasoft: tunnel[$url]: received ${data.size} bytes")
            }
            ...
            while (inputStream.read(buffer).also { read = it } != -1) {
                ...
                thisLogger().trace("khulnasoft: tunnel[$url]: sent $read bytes")
            }
        } catch (t: Throwable) {
            if (t is SocketException && t.message?.contains("Socket closed") == true) {
                return
            }
            thisLogger().error("khulnasoft: tunnel[$url]: failed to pipe", t)
        } finally {
    ...
    private fun connectToWebSocket(socketClient: KhulnasoftWebSocketTunnelClient) {
        ...
        for (proxy in proxies) {
            ...
            if (proxyAddress !is InetSocketAddress) {
                thisLogger().warn("khulnasoft: tunnel[$url]: unexpected proxy: $proxy")
                continue
            }
        ...
        val config = ClientEndpointConfig.Builder.create()
            .configurator(object : Configurator() {
                override fun beforeRequest(headers: MutableMap<String, List<String>>) {
                    headers["x-khulnasoft-owner-token"] = Collections.singletonList(ownerToken)
                }
            })
            .build()
...
class KhulnasoftWebSocketTunnelClient(
    private val url: String,
    ...
    override fun onClose(session: Session, closeReason: CloseReason) {
        thisLogger().info("khulnasoft: tunnel[$url]: closed ($closeReason)")
        this.doClose()
    }

    override fun onError(session: Session?, thr: Throwable?) {
        thisLogger().error("khulnasoft: tunnel[$url]: failed", thr)
        this.doClose()
    }
    ...
    private fun doClose() {
        try {
            ...
            thisLogger().error("khulnasoft: tunnel[$url]: failed to close socket", t)
        }
        try {
            container?.stop()
        } catch (t: Throwable) {
            thisLogger().error("khulnasoft: tunnel[$url]: failed to stop container", t)
        }
    }
    ...
    fun close() {
        try {
            ...
            thisLogger().error("khulnasoft: tunnel[$url]: failed to close", t)
        }
        try {
            container?.stop()
        } catch (t: Throwable) {
