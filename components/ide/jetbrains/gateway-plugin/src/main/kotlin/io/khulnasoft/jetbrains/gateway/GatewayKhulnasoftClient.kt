...
import io.khulnasoft.khulnasoftprotocol.api.entities.WorkspaceInfo
...
class GatewayKhulnasoftClient(
    private val lifetimeDefinition: LifetimeDefinition,
    private val khulnasoftHost: String
) : KhulnasoftClient() {
    ...
    private fun scheduleTimeout(reason: String) {
        ...
        timeoutJob = GlobalScope.launch {
            thisLogger().info("$khulnasoftHost: connection times out in $timeoutDelayInMinutes minutes: $reason")
    ...
    private fun cancelTimeout(reason: String) {
        if (timeoutJob?.isActive == true) {
            thisLogger().info("$khulnasoftHost: canceled connection timeout: $reason")
            timeoutJob!!.cancel()
        }
    }
        ...
        syncJob = GlobalScope.launch {
            for (id in listeners.keys) {
                ...
                try {
                    ...
                    thisLogger().error("$khulnasoftHost: $id: failed to sync", t)
                }
            }
        }
    }
