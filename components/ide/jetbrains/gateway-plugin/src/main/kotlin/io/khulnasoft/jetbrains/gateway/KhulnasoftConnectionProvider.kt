...
class KhulnasoftConnectionProvider : GatewayConnectionProvider {
    private val activeConnections = ConcurrentHashMap<String, LifetimeDefinition>()
    private val khulnasoft = service<KhulnasoftConnectionService>()
    private val connectionHandleFactory = service<KhulnasoftConnectionHandleFactory>()
    private val settings = service<KhulnasoftSettingsState>()
    ...
    override suspend fun connect(
        ...
        if (parameters["khulnasoftHost"] == null) {
            throw IllegalArgumentException("bad khulnasoftHost parameter")
        }
        if (parameters["workspaceId"] == null) {
            throw IllegalArgumentException("bad workspaceId parameter")
        }
        val connectParams = ConnectParams(
            parameters["khulnasoftHost"]!!,
        ...
        var connectionKeyId =
            "${connectParams.khulnasoftHost}-${connectParams.resolvedWorkspaceId}-${connectParams.backendPort}"
        ...
        val client = khulnasoft.obtainClient(connectParams.khulnasoftHost)
        ...
        val connectionPanel = panel {
            indent {
                row {
                    ...
                    panel {
                        row {
                            icon(KhulnasoftIcons.Logo2x).align(AlignX.CENTER)
                        }
                        row {
                            cell(phaseMessage)
                                .bold()
        ...
        GlobalScope.launch {
            ...
            var canceledByKhulnasoft = false

            val ownerToken = client.server.getOwnerToken(connectParams.actualWorkspaceId).await()

            if (settings.additionalHeartbeat) {
                thisLogger().info("khulnasoft: additional heartbeat enabled for ${connectParams.resolvedWorkspaceId}")
                    ...
                    while (isActive) {
                        ...
                        if (thinClientJob?.isActive == true) {
                            try {
                                ...
                                thisLogger().error(
                                    "khulnasoft: failed to send additional heartbeat for ${connectParams.resolvedWorkspaceId}",
                                    t
                                )
                            }
                        } else {
                            thisLogger().debug("khulnasoft: thinClient is not active, skipping additional heartbeat for ${connectParams.resolvedWorkspaceId}")
                        }
            ...
            try {
                for (update in updates) {
                    try {
                        ...
                        if (thinClientJob == null && update.status.phase == "running") {
                            thinClientJob = launch thinClientJob@{
                                try {
                                    ...
                                    if (credentials == null) {
                                        setErrorMessage("${connectParams.khulnasoftHost} installation does not allow SSH access")
                                        ...
                                        backendStatusJob = launch backendStatusJob@{
                                            while (isActive) {
                                                try {
                                                    ...
                                                    thisLogger().error(
                                                        "${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to reconnect:",
                                                        t
                                                    )
                                                }
                                            }
                                    ...
                                    thisLogger().error(
                                        "${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to connect:",
                                        t
                                    )
                        ...
                        thisLogger().error(
                            "${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to process workspace update:",
                            e
                        )
                    }
                }
                ...
                thisLogger().error(
                    "${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to process workspace updates:",
                    t
                )
        ...
        return connectionHandleFactory.createKhulnasoftConnectionHandle(connectionLifetime, connectionPanel, connectParams)
    }
    ...
    private suspend fun resolveCredentialsWithWebSocketTunnel(
        ...
        try {
            ...
            val sshWebSocketServer = KhulnasoftWebSocketTunnelServer(
            ...
            thisLogger().error(
                "${connectParams.khulnasoftHost}: web socket tunnel: failed to connect:",
                t
            )
            return null
        }
    ...
    private suspend fun resolveCredentialsWithDirectSSH(
        ...
        if (hostKeys.isNullOrEmpty()) {
            thisLogger().error("${connectParams.khulnasoftHost}: direct SSH: failed to resolve host keys for")
            return null
        }
        ...
        try {
            ...
            thisLogger().error(
                "${connectParams.khulnasoftHost}: direct SSH: failed to resolve credentials",
                t
            )
            return null
        }
        ...
        if (resp.statusCode != 200) {
            if (!resp.body.isNullOrBlank() && resp.body.contains("not implemented")) {
                if (!sendHeartBeatThroughSupervisorLogOnce) {
                    thisLogger().warn("khulnasoft: sendHeartbeat ${connectParams.actualWorkspaceId} failed: method is not implemented in supervisor")
                    sendHeartBeatThroughSupervisorLogOnce = true
                }
                return
            }
            thisLogger().error("khulnasoft: sendHeartbeat ${connectParams.actualWorkspaceId} failed: ${resp.statusCode}, body: ${resp.body}")
            return
        }
        thisLogger().debug("khulnasoft: sendHeartbeat succeed for ${connectParams.actualWorkspaceId}")
    }
    ...
    private suspend fun fetchWS(
        ...
        if (!ownerToken.isNullOrBlank()) {
            httpRequestBuilder = httpRequestBuilder.header("x-khulnasoft-owner-token", ownerToken)
        }
        ...
        while (true) {
            ...
            try {
                ...
                if (response.statusCode() < 500) {
                    thisLogger().error("${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl': ${response.statusCode()}")
                    return null
                }
                thisLogger().warn("${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl', trying again...: ${response.statusCode()}")
            } catch (t: Throwable) {
                if (t is CancellationException) {
                    throw t
                }
                thisLogger().warn(
                    "${connectParams.khulnasoftHost}: ${connectParams.resolvedWorkspaceId}: failed to fetch '$endpointUrl', trying again...:",
                    t
                )
            }
    ...
    override fun isApplicable(parameters: Map<String, String>): Boolean =
        parameters.containsKey("khulnasoftHost")

    data class ConnectParams(
        val khulnasoftHost: String,
        ...
        val title = "$resolvedWorkspaceId ($khulnasoftHost)"
    }
