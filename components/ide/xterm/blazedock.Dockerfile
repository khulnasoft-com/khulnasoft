...
RUN mkdir /build \
    && cd /build \
    && git init \
    && git remote add origin https://github.com/khulnasoft-com/xterm-web-ide \
...
ARG XTERM_COMMIT
ARG XTERM_VERSION
LABEL "io.khulnasoft.ide.commit"=$XTERM_COMMIT
LABEL "io.khulnasoft.ide.version"=$XTERM_VERSION
