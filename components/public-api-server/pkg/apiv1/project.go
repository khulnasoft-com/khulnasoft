...
func (s *ProjectsService) getConnection(ctx context.Context) (protocol.APIInterface, error) {
	...
	if err != nil {
		...
		return nil, connect.NewError(connect.CodeInternal, errors.New("Failed to establish connection to downstream services. If this issue persists, please contact Khulnasoft Support."))
	}
...
func projectToAPIResponse(p *protocol.Project) *v1.Project {
	return &v1.Project{
		...
		Name:         p.Name,
		CloneUrl:     p.CloneURL,
		CreationTime: parseKhulnasoftTimeStampOrDefault(p.CreationTime),
