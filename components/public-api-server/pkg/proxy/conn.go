...
type ServerConnectionPool interface {
	...
	Get(ctx context.Context, token auth.Token) (khulnasoft.APIInterface, error)
}
...
func (p *NoConnectionPool) Get(ctx context.Context, token auth.Token) (khulnasoft.APIInterface, error) {
	...
	opts := khulnasoft.ConnectToServerOpts{
		Context: ctx,
		Log:     logger,
	...
	conn, err := khulnasoft.ConnectToServer(p.ServerAPI.String(), opts)
	if err != nil {
...
func NewConnectionPool(address *url.URL, poolSize int) (*ConnectionPool, error) {
	cache, err := lru.NewWithEvict(poolSize, func(_, value interface{}) {
		...
		conn, ok := value.(khulnasoft.APIInterface)
		if !ok {
			log.Errorf("Failed to cast cache value to khulnasoft API Interface")
			return
		}
	...
	return &ConnectionPool{
		cache: cache,
		connConstructor: func(ctx context.Context, token auth.Token) (khulnasoft.APIInterface, error) {
			opts := khulnasoft.ConnectToServerOpts{
			...
			conn, err := khulnasoft.ConnectToServer(endpoint, opts)
			if err != nil {
				return nil, fmt.Errorf("failed to create new connection to server: %w", err)
			}
...
type ConnectionPool struct {
	connConstructor func(context.Context, auth.Token) (khulnasoft.APIInterface, error)

	// cache stores token to connection mapping
	cache *lru.Cache
}

func (p *ConnectionPool) Get(ctx context.Context, token auth.Token) (khulnasoft.APIInterface, error) {
	origin := origin.FromContext(ctx)
	...
	if found {
		conn, ok := cached.(*khulnasoft.APIoverJSONRPC)
		if ok {
			return conn, nil
		}
	}
...
func getEndpointBasedOnToken(t auth.Token, u *url.URL) (string, error) {
	...
	case auth.CookieTokenType:
		return fmt.Sprintf("%s/khulnasoft", u.String()), nil
	default:
