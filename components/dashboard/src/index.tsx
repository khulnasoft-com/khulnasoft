...
import { getURLHash, isKhulnasoftIo, isWebsiteSlug } from "./utils";

const bootApp = () => {
    // khulnasoft.com specific boot logic
    if (isKhulnasoftIo()) {
        // Redirect to www website for any website slugs
        if (isWebsiteSlug(window.location.pathname)) {
            window.location.host = "www.khulnasoft.com";
            return;
        }
    }
    ...
    const KhulnasoftQueryClientProvider = setupQueryClientProvider();
        ...
        <React.StrictMode>
            <ThemeContextProvider>
                <ReloadPageErrorBoundary>
                    <BrowserRouter>
                        <KhulnasoftQueryClientProvider>
                            {/* This needs to be inside of the KhulnasoftQueryClientProvider so it can reset queries if needed */}
                            <QueryErrorBoundary>
