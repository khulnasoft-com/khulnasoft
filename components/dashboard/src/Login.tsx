...
import * as KhulnasoftCookie from "@khulnasoft/khulnasoft-protocol/lib/util/khulnasoft-cookie";
...
import { AuthProviderDescription } from "@khulnasoft/public-api/lib/khulnasoft/v1/authprovider_pb";
import { Button, ButtonProps } from "@podkit/buttons/Button";
import { cn } from "@podkit/lib/cn";
import { userClient } from "./service/public-api";
import { ProductLogo } from "./components/ProductLogo";
import { useIsDataOps } from "./data/featureflag-query";
import KhulnasoftClassicCard from "./images/khulnasoft-classic-card.png";
import { LoadingState } from "@podkit/loading/LoadingState";
import { isKhulnasoftIo } from "./utils";

export function markLoggedIn() {
    document.cookie = KhulnasoftCookie.generateCookie(window.location.hostname);
}

export function hasLoggedInBefore() {
    return KhulnasoftCookie.isPresent(document.cookie);
}
...
const LoginContent = ({
    ...
    const isKhulnasoftIoUser = isKhulnasoftIo();

    const authProviders = useAuthProviderDescriptions();
    const [errorMessage, setErrorMessage] = useState<string | undefined>(undefined);

    const updateUser = useCallback(async () => {
        await getKhulnasoftService().reconnect();
        ...
        <div className="rounded-xl px-10 py-10 mx-auto w-full max-w-lg">
            ...
            <div className="mx-auto text-center pb-8 space-y-2">
                {isDataOps ? (
                    ...
                    <Heading1>Log in to Khulnasoft</Heading1>
                ) : (
                    <>
                        <Heading1>Log in to Khulnasoft Classic</Heading1>
            ...
            <div>
                <div className="justify-center md:justify-start mb-6 md:mb-8">
                    ...
                    <p className="text-pk-content-secondary mb-2">
                        Automated, standardized development environments hosted by us in Khulnasoft’s infrastructure. Users
                        who joined before October 1, 2024 on non-Enterprise plans are considered Khulnasoft Classic users.
                    </p>

                    <p className="text-pk-content-secondary mb-2">
                        Khulnasoft Classic is sunsetting fall 2025.{" "}
                        <a className="gp-link font-bold" href="https://app.khulnasoft.com" target="_blank" rel="noreferrer">
                            Try the new Khulnasoft
                        </a>{" "}
                        now (hosted compute coming soon).
                    </p>
                </div>
                <img src={KhulnasoftClassicCard} alt="Khulnasoft Classic" className="w-full" />
            </div>
        </div>
    );
};
    ...
    return (
        <div className="flex-none mx-auto text-center px-4 pb-4">
            <span className="text-gray-400 dark:text-gray-500 text-sm">
                ...
                <a className="gp-link hover:text-gray-600" target="khulnasoft-terms" href="https://www.khulnasoft.com/terms/">
                    terms of service
                </a>{" "}
                and{" "}
                <a
                    className="gp-link hover:text-gray-600"
                    target="khulnasoft-privacy"
