...
} from "@khulnasoft/khulnasoft-protocol/lib/util/parse-workspace-id";
...
export function WorkspaceSearch(props: Props) {
    ...
    const search = async (page: number = 1) => {
        ...
        if (isKhulnasoftIo() && !props.user && queryTerm.length === 0) {
            return;
        }
        ...
        try {
            ...
            if (isKhulnasoftIo() && !query.ownerId && !query.instanceIdOrWorkspaceId && !query.workspaceId) {
                return;
            }

            const result = await getKhulnasoftService().server.adminGetWorkspaces({
                limit: pageLength,
