...
export default function UserDetail(p: { user: User }) {
    ...
    const verifyUser = async () => {
        await updateUser(async (u) => {
            return await getKhulnasoftService().server.adminVerifyUser(u.id);
        });
    };
    ...
    const toggleBlockUser = async () => {
        await updateUser(async (u) => {
            u.blocked = !u.blocked;
            await getKhulnasoftService().server.adminBlockUser({
                blocked: u.blocked,
                id: u.id,
            });
            return u;
    ...
    const deleteUser = async () => {
        await updateUser(async (u) => {
            ...
            await getKhulnasoftService().server.adminDeleteUser(u.id);
            return u;
        });
    };
    ...
    return Object.entries(WorkspaceFeatureFlags)
        ...
        .map((name) => {
            ...
            return {
                ...
                onClick: async () => {
                    await updateUser(async (u) => {
                        return await getKhulnasoftService().server.adminModifyPermanentWorkspaceFeatureFlag({
                            id: user.id,
                            changes: [
                                {
                                    featureFlag: name,
...
function getRopEntries(user: User, updateUser: UpdateUserFunction): Entry[] {
    const createRopEntry = (name: RoleOrPermission, role?: boolean) => {
        ...
        return {
            ...
            onClick: async () => {
                await updateUser(async (u) => {
                    return await getKhulnasoftService().server.adminModifyRoleOrPermission({
                        id: user.id,
                        rpp: [
                            {
                                r: name,
