...
import { User, WorkspaceAndInstance, ContextURL, WorkspaceInstance } from "@khulnasoft/khulnasoft-protocol";
...
export default function WorkspaceDetail(props: { workspace: WorkspaceAndInstance }) {
    ...
    useEffect(() => {
        getKhulnasoftService().server.adminGetUser(props.workspace.ownerId).then(setUser);
        getKhulnasoftService().server.adminGetWorkspaceInstances(props.workspace.workspaceId).then(setWorkspaceInstances);
    }, [props.workspace, workspace.workspaceId]);

    const stopWorkspace = async () => {
        try {
            setActivity(true);
            await getKhulnasoftService().server.adminForceStopWorkspace(workspace.workspaceId);
    ...
    const reload = async () => {
        try {
            ...
            const [ws, workspaceInstances] = await Promise.all([
                await getKhulnasoftService().server.adminGetWorkspace(workspace.workspaceId),
                await getKhulnasoftService().server.adminGetWorkspaceInstances(workspace.workspaceId),
            ]);
            setWorkspace(ws);
        ...
        <div className="app-container">
            <div className="flex mt-8">
                ...
                <Button
                    ...
                    className="ml-3"
                    onClick={() => {
                        window.location.href = new KhulnasoftHostUrl(window.location.href)
                            .with({
                                    ...
                                    ? [
                                          {
                                              label: "Restore & Pin",
                                              onClick: async () => {
                                                  await getKhulnasoftService().server.adminRestoreSoftDeletedWorkspace(
