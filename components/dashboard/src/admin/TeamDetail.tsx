...
import { Team, TeamMemberInfo, TeamMemberRole, VALID_ORG_MEMBER_ROLES } from "@khulnasoft/khulnasoft-protocol";
...
import { CostCenterJSON, CostCenter_BillingStrategy } from "@khulnasoft/khulnasoft-protocol/lib/usage";
...
export default function TeamDetail(props: { team: Team }) {
    ...
    const initialize = () => {
        (async () => {
            const members = await getKhulnasoftService().server.adminGetTeamMembers(team.id);
            if (members.length > 0) {
                setTeamMembers(members);
            }
        })();
        getKhulnasoftService().server.adminGetBillingMode(attributionId).then(setBillingMode);
        getKhulnasoftService().server.adminGetCostCenter(attributionId).then(setCostCenter);
        getKhulnasoftService().server.adminGetUsageBalance(attributionId).then(setUsageBalance);
    };
    ...
    const setTeamMemberRole = async (userId: string, role: TeamMemberRole) => {
        await getKhulnasoftService().server.adminSetTeamMemberRole(team!.id, userId, role);
        setTeamMembers(await getKhulnasoftService().server.adminGetTeamMembers(team!.id));
    };
    return (
        <>
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            if (usageLimit !== undefined) {
                                await getKhulnasoftService().server.adminSetUsageLimit(attributionId, usageLimit || 0);
                    ...
                    <Button
                        ...
                        onClick={async () => {
                            if (creditNote.credits !== 0 && !!creditNote.note) {
                                await getKhulnasoftService().server.adminAddUsageCreditNote(
                                    attributionId,
                                    creditNote.credits,
                                    creditNote.note,
                                );
