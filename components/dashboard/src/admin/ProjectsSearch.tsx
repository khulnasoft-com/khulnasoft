...
import { AdminGetListResult, Project } from "@khulnasoft/khulnasoft-protocol";
...
export function ProjectsSearch() {
    ...
    useEffect(() => {
        (async () => {
            if (currentProject) {
                const owner = await getKhulnasoftService().server.adminGetTeamById(currentProject.teamId);
                if (owner) {
    ...
    const search = async (page: number = 1) => {
        setSearching(true);
        try {
            const result = await getKhulnasoftService().server.adminGetProjectsBySearchTerm({
                searchTerm,
                limit: pageLength,
