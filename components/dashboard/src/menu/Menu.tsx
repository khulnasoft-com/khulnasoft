...
import { getKhulnasoftService, khulnasoftHostUrl } from "../service/service";
...
import { User, RoleOrPermission } from "@khulnasoft/public-api/lib/khulnasoft/v1/user_pb";
...
export default function Menu() {
    ...
    const isKhulnasoftIo = isInstallationConfigLoading ? false : !installationConfig?.isDedicatedInstallation;

    useEffect(() => {
        const { server } = getKhulnasoftService();
...
const UserMenu: FC<UserMenuProps> = ({ user, className, withAdminLink, withFeedbackLink, onFeedback }) => {
    ...
    const extraSection = useMemo(() => {
        ...
        if (withFeedbackLink && isKhulnasoftIo) {
            items.push({
                title: "Feedback",
                onClick: onFeedback,
            });
        ...
        return items;
    }, [isKhulnasoftIo, onFeedback, user?.rolesOrPermissions, withAdminLink, withFeedbackLink]);
    ...
    const menuEntries = useMemo(() => {
        return [
            ...
            {
                title: "Help",
                href: "https://www.khulnasoft.com/support/",
                target: "_blank",
                rel: "noreferrer",
                separator: true,
            },
            ...
            {
                title: "Log out",
                href: khulnasoftHostUrl.asApiLogout().toString(),
            },
        ];
    }, [extraSection, user]);
