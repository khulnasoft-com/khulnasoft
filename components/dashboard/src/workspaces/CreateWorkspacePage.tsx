...
import { SuggestedRepository } from "@khulnasoft/public-api/lib/khulnasoft/v1/scm_pb";
import { SelectAccountPayload } from "@khulnasoft/khulnasoft-protocol/lib/auth";
...
import { khulnasoftHostUrl } from "../service/service";
...
import { AuthProviderType } from "@khulnasoft/public-api/lib/khulnasoft/v1/authprovider_pb";
import {
...
import { CreateAndStartWorkspaceRequest } from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { PartialMessage } from "@bufbuild/protobuf";
import { User_WorkspaceAutostartOption } from "@khulnasoft/public-api/lib/khulnasoft/v1/user_pb";
...
import { OrganizationPermission } from "@khulnasoft/public-api/lib/khulnasoft/v1/organization_pb";
...
export function CreateWorkspacePage() {
    ...
    const createWorkspace = useCallback(
        ...
        async (options?: PartialMessage<CreateAndStartWorkspaceRequest>) => {
            ...
            // if user received an INVALID_KHULNASOFT_YML yml for their contextURL they can choose to proceed using default configuration
            if (
                workspaceContext.error &&
                ApplicationError.hasErrorCode(workspaceContext.error) &&
                workspaceContext.error.code === ErrorCodes.INVALID_KHULNASOFT_YML
            ) {
    ...
    const continueButtonDisabled = useMemo(() => {
        ...
        if (workspaceContext.error) {
            // For INVALID_KHULNASOFT_YML we don't want to disable the button
...
const ErrorMessage: FunctionComponent<ErrorMessageProps> = ({ error, reset, setSelectAccountError }) => {
    ...
    switch (error.code) {
        case ErrorCodes.INVALID_KHULNASOFT_YML:
            return (
...
export const RepositoryNotFound: FC<{ error: StartWorkspaceError }> = ({ error }) => {
    ...
    const missingScope =
        authProvider.type === AuthProviderType.GITHUB
            ...
            ? "api"
            : "";
    const authorizeURL = khulnasoftHostUrl
        .withApi({
            pathname: "/authorize",
    ...
    if (!userScopes.includes(missingScope)) {
        return (
            <RepositoryInputError
                title="The repository may be private. Please authorize Khulnasoft to access private repositories."
    ...
    if (!updatedRecently) {
        return (
            <RepositoryInputError
                title={`Permission to access private repositories has been granted. If you are a member of '${owner}', please try to request access for Khulnasoft.`}
                message={errorMessage}
    ...
    if (authProvider.id.toLocaleLowerCase() === "public-github" && isKhulnasoftIo()) {
        return (
            <RepositoryInputError
    ...
    return (
        <RepositoryInputError
            title={`Your access token was updated recently. Please try again if the repository exists and Khulnasoft was approved for '${owner}'.`}
            message={errorMessage}
...
export function LimitReachedModal(p: { children: ReactNode }) {
    ...
    return (
        ...
        <Modal visible={true} closeable={false} onClose={() => {}}>
            ...
            <ModalFooter>
                <a href={khulnasoftHostUrl.asDashboard().toString()}>
                    <Button variant="secondary">Go to Dashboard</Button>
                </a>
                <a href={khulnasoftHostUrl.with({ pathname: "plans" }).toString()} className="ml-2">
