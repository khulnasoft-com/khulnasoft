...
function GitProviders() {
    ...
    const disconnect = async (ap: AuthProviderDescription) => {
        ...
        const returnTo = khulnasoftHostUrl.with({ pathname: "complete-auth", search: "message=success" }).toString();
        const deauthorizeUrl = khulnasoftHostUrl
            .withApi({
    ...
    return (
        <div>
            ...
            <Subheading>
                ...
                <a
                    className="gp-link"
                    href="https://www.khulnasoft.com/docs/configure/authentication"
                    target="_blank"
                    rel="noreferrer"
                >
                    Learn more
...
export function GitIntegrationModal(
    ...
    const callbackUrl = useMemo(() => khulnasoftHostUrl.with({ pathname: `/auth/callback` }).toString(), []);
    ...
    const getRedirectUrlDescription = (type: AuthProviderType, host: string) => {
        if (type === AuthProviderType.AZURE_DEVOPS) {
            return (
                <span>
                    ...
                    <a
                        href="https://www.khulnasoft.com/docs/azure-devops-integration/#oauth-application"
                        target="_blank"
        ...
        switch (type) {
            case AuthProviderType.GITHUB:
                docsUrl = `https://www.khulnasoft.com/docs/github-integration/#oauth-application`;
                break;
            case AuthProviderType.GITLAB:
                docsUrl = `https://www.khulnasoft.com/docs/gitlab-integration/#oauth-application`;
                break;
            default:
                return undefined;
        }
