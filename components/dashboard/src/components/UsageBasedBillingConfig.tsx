...
export default function UsageBasedBillingConfig({ hideSubheading = false }: Props) {
    ...
    useEffect(() => {
        if (attributionId) {
            getKhulnasoftService().server.getPriceInformation(attributionId).then(setPriceInformation);
        }
    }, [attributionId]);
    ...
    const refreshSubscriptionDetails = useCallback(
        async (attributionId: string) => {
            ...
            try {
                getKhulnasoftService().server.getStripePortalUrl(attributionId).then(setStripePortalUrl);
                getKhulnasoftService().server.getUsageBalance(attributionId).then(setCurrentUsage);
                const costCenter = await getKhulnasoftService().server.getCostCenter(attributionId);
    ...
    const subscribeToStripe = useCallback(
        async (stripeParams: { paymentIntentId?: string; redirectStatus: string }) => {
            ...
            try {
                ...
                const newLimit = await getKhulnasoftService().server.subscribeToStripe(
                    attributionId,
                    paymentIntentId || "",
                    limit,
                );
    ...
    const updateUsageLimit = useCallback(
        async (newLimit: number) => {
            ...
            try {
                await getKhulnasoftService().server.setUsageLimit(attributionId, newLimit);
                setUsageLimit(newLimit);
