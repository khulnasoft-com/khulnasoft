...
import { GetWorkspaceRequest, WorkspacePhase_Phase } from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { Prebuild, PrebuildPhase_Phase } from "@khulnasoft/public-api/lib/khulnasoft/v1/prebuild_pb";
...
function retryWatchWorkspaceImageBuildLogs(workspaceId: string): Disposable {
    ...
    (async () => {
        ...
        while (true) {
            ...
            try {
                await getKhulnasoftService().server.watchWorkspaceImageBuildLogs(workspaceId);
            } catch (err) {
...
function watchHeadlessLogs(
    ...
    const startWatchingLogs = async () => {
        ...
        try {
            const logSources = await getKhulnasoftService().server.getHeadlessLog(instanceId);
