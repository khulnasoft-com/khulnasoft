...
import { AuthProviderDescription, AuthProviderType } from "@khulnasoft/public-api/lib/khulnasoft/v1/authprovider_pb";
...
export function isKhulnasoftIo() {
    return (
        window.location.hostname === "khulnasoft.com" ||
        window.location.hostname === "khulnasoft-staging.com" ||
        window.location.hostname.endsWith("khulnasoft-dev.com") ||
        window.location.hostname.endsWith("khulnasoft-com-dev.com")
    );
}
...
export function isWebsiteSlug(pathName: string) {
    const slugs = [
        ...
        "features",
        "for",
        "khulnasoft-vs-github-codespaces",
        "guides",
        "imprint",
        "media-kit",
        "memes",
