...
import { AdditionalUserData, User as UserProtocol } from "@khulnasoft/khulnasoft-protocol";
...
export const useUpdateCurrentUserMutation = () => {
    return useMutation({
        mutationFn: async (partialUser: UpdateCurrentUserArgs) => {
            const current = await getKhulnasoftService().server.getLoggedInUser();
            ...
            const user = await getKhulnasoftService().server.updateLoggedInUser(update);
