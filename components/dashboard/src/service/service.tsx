...
import {
    ...
    KhulnasoftService,
    KhulnasoftServiceImpl,
    Disposable,
} from "@khulnasoft/khulnasoft-protocol";
import { WebSocketConnectionProvider } from "@khulnasoft/khulnasoft-protocol/lib/messaging/browser/connection";
...
import { IDEFrontendDashboardService } from "@khulnasoft/khulnasoft-protocol/lib/frontend-dashboard-service";
import { RemoteTrackMessage } from "@khulnasoft/khulnasoft-protocol/lib/analytics";
...
import { LotsOfRepliesResponse } from "@khulnasoft/public-api/lib/khulnasoft/experimental/v1/dummy_pb";
...
import { Workspace, WorkspaceSpec_WorkspaceType, WorkspaceStatus } from "@khulnasoft/public-api/lib/khulnasoft/v1/workspace_pb";
import { sendTrackEvent } from "../Analytics";

export const khulnasoftHostUrl = new KhulnasoftHostUrl(window.location.toString());

function createKhulnasoftService<C extends KhulnasoftClient, S extends KhulnasoftServer>() {
    const host = khulnasoftHostUrl.asWebsocket().with({ pathname: KhulnasoftServerPath }).withApi();
    ...
    return new KhulnasoftServiceImpl<C, S>(proxy, { onReconnect });
}
        ...
        return originalCreateWebSocket.call(
            ...
            new Proxy(WebSocket, {
                construct(target: any, argArray) {
                    ...
                    instrumentWebSocket(webSocket, "khulnasoft");
                    return webSocket;
                },
            }),
        );
...
export function getKhulnasoftService(): KhulnasoftService {
    const w = window as any;
    const _gp = w._gp || (w._gp = {});
    let service = _gp.khulnasoftService;
    if (!service) {
        service = _gp.khulnasoftService = createKhulnasoftService();
                ...
                if (propKey === "getWorkspace") {
                    try {
                        ...
                        const isTest = await getExperimentsClient().getValueAsync(
                            ...
                            {
                                user,
                                khulnasoftHost: window.location.host,
                            },
                        );
                        if (isTest) {
...
export function getIDEFrontendService(workspaceID: string, sessionId: string, service: KhulnasoftService) {
...
export class IDEFrontendService implements IDEFrontendDashboardService.IServer {
    ...
    constructor(
        ...
        private service: KhulnasoftService,
        ...
        window.addEventListener("unload", () => {
            ...
            const khulnasoftHostUrl = new KhulnasoftHostUrl(window.location.toString());
            const url = khulnasoftHostUrl.withApi({ pathname: `/auth/workspacePageClose/${this.instanceID}` }).toString();
    ...
    private async redirectToCustomUrl(info: IDEFrontendDashboardService.Info) {
        const isDataOps = await getExperimentsClient().getValueAsync("dataops", false, {
            ...
            khulnasoftHost: khulnasoftHostUrl.toString(),
        });
    ...
    private async auth() {
        ...
        const url = khulnasoftHostUrl.asWorkspaceAuth(this.instanceID).toString();
        await fetch(url, {
            credentials: "include",
        });
    }
    ...
    openDesktopIDE(url: string): void {
        ...
        try {
            ...
            if (
                ...
                !desktopLink.href.startsWith("jetbrains://gateway/io.khulnasoft.toolbox.gateway/")
            ) {
                redirect = false;
            }
        } catch (e) {
