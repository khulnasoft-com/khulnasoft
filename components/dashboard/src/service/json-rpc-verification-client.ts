...
} from "@khulnasoft/public-api/lib/khulnasoft/v1/verification_pb";
...
export class JsonRpcVerificationClient implements PromiseClient<typeof VerificationService> {
    async sendPhoneNumberVerificationToken(
        ...
        const info = await getKhulnasoftService().server.sendPhoneNumberVerificationToken(request.phoneNumber);
    ...
    async verifyPhoneNumberVerificationToken(
        ...
        const info = await getKhulnasoftService().server.verifyPhoneNumberVerificationToken(
            request.phoneNumber,
            request.token,
