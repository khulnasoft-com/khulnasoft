...
The Workspace Proxy (ws-proxy) is a specialized component in Khulnasoft that handles routing and proxying of HTTP and WebSocket traffic to workspaces. It acts as an intermediary between the main Khulnasoft proxy and individual workspace pods, providing workspace-specific routing, port forwarding, and SSH gateway functionality.
...
- Khulnasoft installation details (hostname, workspace domain patterns)
...
- Authenticates users using Khulnasoft's authentication system
...
4. **Main Proxy**: Receives traffic from the main Khulnasoft proxy
...
- **Proxy**: Main Khulnasoft proxy that routes traffic to ws-proxy
