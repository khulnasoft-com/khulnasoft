...
## Overview
The Local App API defines the gRPC interfaces for the Local App service, which facilitates communication between a user's local machine and Khulnasoft workspaces. This API enables port tunneling, SSH connection management, and other local-to-remote interactions.
...
The Local App API is primarily used by the local-app component, which is a desktop application that runs on users' machines to facilitate communication with remote Khulnasoft workspaces.
