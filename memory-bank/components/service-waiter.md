...
- `--username`, `-u`: Database username (from `DB_USERNAME` env var, default: khulnasoft)
...
```bash
service-waiter database --host mysql.example.com --port 3306 --username khulnasoft --password secret
```
...
```bash
service-waiter component --namespace default --component server --labels "app=khulnasoft,component=server" --image khulnasoft/server:latest
```
...
initContainers:
- name: wait-for-db
  image: khulnasoft/service-waiter:latest
...
- `components/khulnasoft-db`: For database migration information
