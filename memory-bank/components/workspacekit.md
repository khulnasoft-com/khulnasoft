...
- `KHULNASOFT_WORKSPACE_ID`: Workspace identifier
- `WORKSPACEKIT_FSSHIFT`: Filesystem shift method (e.g., SHIFTFS)
- `KHULNASOFT_WORKSPACEKIT_BIND_MOUNTS`: Additional bind mounts
- `WORKSPACEKIT_RING2_ENCLAVE`: Commands to run in the Ring2 namespace
- `KHULNASOFT_WORKSPACEKIT_SUPERVISOR_PATH`: Path to the supervisor binary
- `KHULNASOFT_RLIMIT_CORE`: Core dump size limits
- `KHULNASOFT_WORKSPACEKIT_SLEEP_FOR_DEBUGGING`: Enable debugging sleep
