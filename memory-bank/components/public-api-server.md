...
- Serve as the canonical way to access Khulnasoft functionality programmatically
...
3. **Proxy Layer**: Routes requests to internal Khulnasoft services
...
The component is designed to be the primary entry point for all programmatic interactions with Khulnasoft, abstracting away internal implementation details and providing a stable interface.
...
```json
{
  ...
  "khulnasoftServiceURL": "https://khulnasoft.com/api",
  "publicURL": "https://api.khulnasoft.com",
  "sessionServiceAddress": "session-service:3000",
  "databaseConfigPath": "/etc/khulnasoft/db",
  "redis": {
    "address": "redis:6379"
  },
  "auth": {
    "pki": {
      "privateKeyPath": "/etc/khulnasoft/auth/private-key.pem",
      "publicKeyPath": "/etc/khulnasoft/auth/public-key.pem"
    },
    "session": {
  ...
  "personalAccessTokenSigningKeyPath": "/etc/khulnasoft/auth/pat-key",
  "stripeWebhookSigningSecretPath": "/etc/khulnasoft/stripe/webhook-secret",
...
- `components/khulnasoft-protocol`: Khulnasoft protocol definitions
- `components/khulnasoft-db`: Database access
...
1. **Khulnasoft Server**: For core Khulnasoft functionality
...
2. Integrate Khulnasoft with CI/CD pipelines
