# Active Context: Khulnasoft

## Current Work Focus

We are focusing on understanding the Khulnasoft codebase and architecture. The primary goal is to build a comprehensive knowledge base that will allow for effective development, troubleshooting, and enhancement of the Khulnasoft platform.
...
- Created detailed documentation for key components:
  ...
  - dashboard: Web-based user inSterface for Khulnasoft
  ...
  - khulnasoft-cli: Command-line interface for interacting with Khulnasoft workspaces
  - khulnasoft-db: Database layer for the Khulnasoft platform
  - khulnasoft-protocol: Core type definitions and shared protocol library
  - ide: Packages and manages IDEs available in Khulnasoft workspaces
  ...
  - local-app: Provides tools for interacting with Khulnasoft workspaces from local machine
  - public-api-server: Provides a stable, versioned API for programmatic access to Khulnasoft
...
As we begin working with the Khulnasoft codebase, several questions and uncertainties exist:
...
Initial exploration of the Khulnasoft codebase has revealed:

- **Microservices Architecture**: Khulnasoft is built as a collection of loosely coupled services, each with specific responsibilities
...
- **Build System Approaches**: Khulnasoft uses two primary approaches for building components:
...
- **Server Health Checks**: The Khulnasoft server uses two distinct health check mechanisms:
