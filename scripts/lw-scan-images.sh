...
"$OCI_TOOL" fetch file -o "$INSTALLER" --platform=linux-amd64 "docker.io/khulnasoft/core-dev/build/installer:${VERSION}" app/installer
echo ""
chmod +x "$INSTALLER"
...
# There, we can see the results in the "Vulnerabilities" tab, by searching for the Khulnasoft version
