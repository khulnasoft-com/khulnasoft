...
import (
	...
	builder "github.com/khulnasoft-com/khulnasoft/image-builder/api"
)
...
var imagebuildsBuildBatch = &cobra.Command{
	...
	mysql -N -B -u khulnasoft -p -h 127.0.0.1 khulnasoft -e 'SELECT ws.baseImageNameResolved FROM d_b_workspace_instance wsi LEFT JOIN d_b_workspace ws ON ws.id = workspaceId WHERE wsi.creationTime > (NOW() - INTERVAL 30 DAY)' | \
	sort | \
	uniq | \
...
func buildWorkspaceImage(wg *sync.WaitGroup, ctx context.Context, client builder.ImageBuilderClient, forceRebuild bool, ref string) {
	...
	br, err := client.Build(ctx, &builder.BuildRequest{
		...
		SupervisorRef: "docker.io/khulnasoft/core-dev/build/supervisor:commit-4cb5b6b9c0e993f3964e978e387fb0e7c1c04276",
