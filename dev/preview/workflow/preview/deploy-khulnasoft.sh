...
KHULNASOFT_AGENT_SMITH_TOKEN="$(openssl rand -hex 30)"
KHULNASOFT_AGENT_SMITH_TOKEN_HASH="$(echo -n "$KHULNASOFT_AGENT_SMITH_TOKEN" | sha256sum - | tr -d '  -')"
KHULNASOFT_CONTAINER_REGISTRY_URL="docker.io/khulnasoft/dev-artifact/image-build/";
KHULNASOFT_IMAGE_PULL_SECRET_NAME="image-pull-secret";
KHULNASOFT_PROXY_SECRET_NAME="proxy-config-certificates";
KHULNASOFT_ANALYTICS="${KHULNASOFT_ANALYTICS:-}"
KHULNASOFT_WORKSPACE_FEATURE_FLAGS="${KHULNASOFT_WORKSPACE_FEATURE_FLAGS:-}"
KHULNASOFT_WITH_DEDICATED_EMU="${KHULNASOFT_WITH_DEDICATED_EMU:-false}"
PREVIEW_GCP_PROJECT="khulnasoft-dev-preview"
...
INSTALLER_CONFIG_PATH="${INSTALLER_CONFIG_PATH:-$(mktemp "/tmp/XXXXXX.khulnasoft.config.yaml")}"
...
if ! test -f "/tmp/versions.yaml"; then
  ec=0
  docker run --rm "docker.io/khulnasoft/dev-artifact/build/versions:$VERSION" cat /versions.yaml > /tmp/versions.yaml || ec=$?
  if [[ ec -ne 0 ]];then
...
function copyCachedCertificate {
  DESTINATION_CERT_NAME="$KHULNASOFT_PROXY_SECRET_NAME"
...
#
yq w -i "${INSTALLER_CONFIG_PATH}" certificate.name "${KHULNASOFT_PROXY_SECRET_NAME}"
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.inCluster "false"

yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.url "${KHULNASOFT_CONTAINER_REGISTRY_URL}"
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.certificate.kind secret
yq w -i "${INSTALLER_CONFIG_PATH}" containerRegistry.external.certificate.name "${KHULNASOFT_IMAGE_PULL_SECRET_NAME}"

#
# configureDomain
#
DOMAIN="${PREVIEW_NAME}.preview.khulnasoft-dev.com"
...
if [[ "${KHULNASOFT_WITH_DEDICATED_EMU}" != "true" ]]
then
  ...
  | jq -r 'to_entries | .[] | @base64'); do
      ...
      data="${data//preview.khulnasoft-dev.com/${DOMAIN}}"
...
yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_PREVENT_METADATA_ACCESS).value' "true"

yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers[+].name' "workspace"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_PREVENT_METADATA_ACCESS).value' "true"

yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers[+].name' "workspace"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_PREVENT_METADATA_ACCESS"
yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_PREVENT_METADATA_ACCESS).value' "true"

#
# includeAnalytics
#
if [[ "${KHULNASOFT_ANALYTICS}" == "segment" ]]; then
  KHULNASOFT_ANALYTICS_SEGMENT_TOKEN="$(gcloud secrets versions access latest --secret="segment-staging-write-key" --project=${PREVIEW_GCP_PROJECT})"
  if [[ -z "${KHULNASOFT_ANALYTICS_SEGMENT_TOKEN}" ]]; then
    echo "KHULNASOFT_ANALYTICS_SEGMENT_TOKEN is empty"
    exit 1
  fi

  yq w -i "${INSTALLER_CONFIG_PATH}" analytics.writer segment
  yq w -i "${INSTALLER_CONFIG_PATH}" analytics.segmentKey "${KHULNASOFT_ANALYTICS_SEGMENT_TOKEN}"
  ...
  yq w -i "${INSTALLER_CONFIG_PATH}" experimental.webapp.proxy.analyticsPlugin.trustedSegmentKey "${KHULNASOFT_ANALYTICS_SEGMENT_TOKEN}"
  yq w -i "${INSTALLER_CONFIG_PATH}" experimental.webapp.proxy.analyticsPlugin.untrustedSegmentKey "${KHULNASOFT_ANALYTICS_SEGMENT_TOKEN}"

  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'workspace.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"

  # add to g1-standard workspace class
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-standard.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"

  # add to g1-small workspace class
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_WRITER"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_WRITER).value' "segment"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env[+].name' "KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT"
  yq w -i "${INSTALLER_CONFIG_PATH}" 'experimental.workspace.classes.g1-small.templates.default.spec.containers.(name==workspace).env.(name==KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT).value' "https://${DOMAIN}/analytics"
else
...
for feature in ${KHULNASOFT_WORKSPACE_FEATURE_FLAGS}; do
...
WITH_VM=true "$SCRIPT_PATH/post-process.sh" "${PREVIEW_NAME}" "${KHULNASOFT_AGENT_SMITH_TOKEN}"
...
blazedock run components:add-smith-token \
  -DTOKEN="${KHULNASOFT_AGENT_SMITH_TOKEN_HASH}" \
