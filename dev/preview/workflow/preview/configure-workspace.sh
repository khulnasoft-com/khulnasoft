...
if [ -f "/usr/local/khulnasoft/config/initial-spec.json" ]; then
  gcloud iam workload-identity-pools create-cred-config \
    projects/184212049955/locations/global/workloadIdentityPools/khulnasoft-flex/providers/khulnasoft-flex-provider \
    --service-account=preview-environmnet-dev@khulnasoft-dev-preview.iam.gserviceaccount.com \
    --service-account-token-lifetime-seconds=1h \
    --output-file="${PREVIEW_ENV_DEV_SA_KEY_PATH}" \
    --executable-command='node /workspace/khulnasoft/dev/flex-oidc/oidc.js' \
