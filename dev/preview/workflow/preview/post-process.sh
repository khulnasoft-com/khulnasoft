...
while [ "$documentIndex" -le "$DOCS" ]; do
   ...
   if [[ " ${WORKSPACE_COMPONENTS[*]} " =~ " ${NAME} " ]] && { [[ "$KIND" == "Deployment" ]] || [[ "$KIND" == "DaemonSet" ]]; }; then
      LABEL="khulnasoft.com/workspace_$NODE_POOL_INDEX"
      ...
      LABEL="khulnasoft.com/workload_meta"
      echo "setting $LABEL for $NAME"
      touch /tmp/"$NAME"pool.yaml
      # create a matching expression
      yq w -i /tmp/"$NAME"pool.yaml spec.template.spec.affinity.nodeAffinity.requiredDuringSchedulingIgnoredDuringExecution.nodeSelectorTerms[0].matchExpressions[0].key "$LABEL"
   ...
   if [[ "blobserve" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      jq 'if .blobserve.repos["docker.io/khulnasoft/core-dev/build/ide/code"] == null then .blobserve.repos["docker.io/khulnasoft/core-dev/build/ide/code"] = .blobserve.repos["docker.io/khulnasoft/dev-artifact/build/ide/code"] else . end' /tmp/"$NAME"-overrides.json > /tmp/"$NAME"-updated-overrides.json
   ...
   if [[ "agent-smith" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
      ...
      # replace khulnasoft token
      yq r /tmp/"$NAME"-"$KIND"-overrides.yaml 'data.[config.json]' \
      | jq ".khulnasoftAPI.apiToken = \"$SMITH_TOKEN\"" > /tmp/"$NAME"-"$KIND"-overrides.json
   ...
   if [[ "khulnasoft-telemetry" == "$NAME" ]] && [[ "$KIND" == "CronJob" ]]; then
      WORK="suspend $NAME $KIND"
      echo "$WORK"
      yq w -i k8s.yaml -d "$documentIndex" spec.suspend "true"
   fi

   # Uncomment to change or remove resources from the configmap which can be used to uninstall Khulnasoft
   ...
   # if [[ "khulnasoft-app" == "$NAME" ]] && [[ "$KIND" == "ConfigMap" ]]; then
   #    WORK="overrides for $NAME $KIND"
   #    echo "$WORK"
   #    # Get a copy of the config we're working with
   ...
   #       #    echo "Avoiding writing the $CONFIG_NAME $CONFIG_KIND to the khulnasoft-app ConfigMap"
   #       #    ci=$((ci + 1))
   #       #    continue
   #       # fi

   #       yq r /tmp/"$NAME"-"$KIND"-original.yaml -d "$ci" > /tmp/khulnasoft-app_config_"$ci"
   #       if [ "$ci" -gt 0 ]; then
   #          # add a document separater
   #          echo "---" >> /tmp/"$NAME"-"$KIND"-overrides.yaml
   #       fi

   #       cat /tmp/khulnasoft-app_config_"$ci" >> /tmp/"$NAME"-"$KIND"-overrides.yaml
   #       ci=$((ci + 1))
   #       new_ci=$((new_ci + 1))
   #    done
