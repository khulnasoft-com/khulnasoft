...
func DefaultEnv(cfg *config.Config) []corev1.EnvVar {
	...
	return MergeEnv(
		[]corev1.EnvVar{
			{Name: "KHULNASOFT_DOMAIN", Value: cfg.Domain},
			{Name: "KHULNASOFT_INSTALLATION_SHORTNAME", Value: cfg.Metadata.InstallationShortname},
			{Name: "KHULNASOFT_REGION", Value: cfg.Metadata.Region},
			...
			//  - https://linear.app/khulnasoft/issue/CLC-1264/investigate-public-api-server-connectivity-issues-during-sso-login#comment-f2daa302
			//  - https://linear.app/khulnasoft/issue/CLC-1067/go-upgrade-from-123x-to-124x-once-available for details
...
func AnalyticsEnv(cfg *config.Config) (res []corev1.EnvVar) {
	...
	return []corev1.EnvVar{{
		Name:  "KHULNASOFT_ANALYTICS_WRITER",
		Value: cfg.Analytics.Writer,
	}, {
		Name:  "KHULNASOFT_ANALYTICS_SEGMENT_KEY",
		Value: cfg.Analytics.SegmentKey,
	}, {
		Name:  "KHULNASOFT_ANALYTICS_SEGMENT_ENDPOINT",
...
// If config registry is set to the Khulnasoft registry, the third-party registry is returned. If
...
func ThirdPartyContainerRepo(configRegistry string, thirdPartyRegistry string) string {
	...
	if configRegistry == KhulnasoftContainerRegistry {
