...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/components/workspace"
	wsmanagermk2 "github.com/khulnasoft-com/khulnasoft/installer/pkg/components/ws-manager-mk2"
	configv1 "github.com/khulnasoft-com/khulnasoft/installer/pkg/config/v1"
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/config/v1/experimental"

	"github.com/khulnasoft-com/khulnasoft/common-go/baseserver"
	"github.com/khulnasoft-com/khulnasoft/common-go/util"
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/common"
	"github.com/khulnasoft-com/khulnasoft/ws-proxy/pkg/config"
	"github.com/khulnasoft-com/khulnasoft/ws-proxy/pkg/proxy"
...
func configmap(ctx *common.RenderContext) ([]runtime.Object, error) {
	...
	khulnasoftInstallationHostName := ctx.Config.Domain
	...
	khulnasoftInstallationWorkspaceHostSuffix := fmt.Sprintf(".ws%s.%s", installationShortNameSuffix, ctx.Config.Domain)
	khulnasoftInstallationWorkspaceHostSuffixRegex := fmt.Sprintf("\\.ws[^\\.]*\\.%s", ctx.Config.Domain)
	...
	ctx.WithExperimental(func(ucfg *experimental.Config) error {
		...
		if ucfg.Workspace.WSProxy.KhulnasoftInstallationHostName != "" {
			khulnasoftInstallationHostName = ucfg.Workspace.WSProxy.KhulnasoftInstallationHostName
		}
		if ucfg.Workspace.WSProxy.KhulnasoftInstallationWorkspaceHostSuffix != "" {
			khulnasoftInstallationWorkspaceHostSuffix = ucfg.Workspace.WSProxy.KhulnasoftInstallationWorkspaceHostSuffix
		}
		if ucfg.Workspace.WSProxy.KhulnasoftInstallationWorkspaceHostSuffixRegex != "" {
			khulnasoftInstallationWorkspaceHostSuffixRegex = ucfg.Workspace.WSProxy.KhulnasoftInstallationWorkspaceHostSuffixRegex
		}
	...
	wspcfg := config.Config{
		...
		Proxy: proxy.Config{
			...
			KhulnasoftInstallation: &proxy.KhulnasoftInstallation{
				Scheme:                   "https",
				HostName:                 khulnasoftInstallationHostName,
				WorkspaceHostSuffix:      khulnasoftInstallationWorkspaceHostSuffix,
				WorkspaceHostSuffixRegex: khulnasoftInstallationWorkspaceHostSuffixRegex,
			},
