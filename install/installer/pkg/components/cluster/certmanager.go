...
func certmanager(ctx *common.RenderContext) ([]runtime.Object, error) {
	issuerName := "khulnasoft-self-signed-issuer"
	secretCAName := "khulnasoft-identity-trust-root"

	khulnasoftCaBundleSources := []trust.BundleSource{
		{
	...
	khulnasoftCustomCertificateBundleSource := []trust.BundleSource{}

	if ctx.Config.CustomCACert != nil {
		khulnasoftCaBundleSources = append(khulnasoftCaBundleSources, trust.BundleSource{
			Secret: &trust.SourceObjectKeySelector{
		...
		khulnasoftCustomCertificateBundleSource = append(khulnasoftCustomCertificateBundleSource, trust.BundleSource{
			Secret: &trust.SourceObjectKeySelector{
				Name:        ctx.Config.CustomCACert.Name,
				KeySelector: trust.KeySelector{Key: "ca.crt"},
			},
	...
	objects := []runtime.Object{
		...
		&v1.Certificate{
			...
			Spec: v1.CertificateSpec{
				...
				CommonName: "root.khulnasoft.cluster.local",
		...
		&v1.Certificate{
			...
			Spec: v1.CertificateSpec{
				...
				CommonName: "ca.khulnasoft.cluster.local",
		...
		&trust.Bundle{
			...
			ObjectMeta: metav1.ObjectMeta{
				Name: "khulnasoft-ca-bundle",
			},
			Spec: trust.BundleSpec{
				Sources: khulnasoftCaBundleSources,
		...
		// single khulnasoft Bundle (used by registry-facade)
		&trust.Bundle{
			TypeMeta: common.TypeMetaBundle,
			ObjectMeta: metav1.ObjectMeta{
				Name: "khulnasoft-ca",
	...
	if ctx.Config.CustomCACert != nil {
		objects = append(objects,
			...
			&trust.Bundle{
				TypeMeta: common.TypeMetaBundle,
				ObjectMeta: metav1.ObjectMeta{
					Name: "khulnasoft-customer-certificate-bundle",
				},
				Spec: trust.BundleSpec{
					Sources: khulnasoftCustomCertificateBundleSource,
					Target: trust.BundleTarget{
						ConfigMap: &trust.KeySelector{
							Key: "ca-certificates.crt",
						},
