<p align="center">
  <a href="https://www.khulnasoft.com">
    <img src="https://raw.githubusercontent.com/khulnasoft-com/khulnasoft/master/components/dashboard/src/icons/khulnasoft.svg" height="60">
    <h3 align="center">Khulnasoft</h3>
  </a>
...
The best way to get started with Khulnasoft is by using our recommended & default installation method [described in our documentation](https://www.khulnasoft.com/docs/self-hosted/latest/getting-started). In fact, our default installation method actually wraps this installer into a UI that helps you manage, update and configure Khulnasoft in a streamlined way. This document describes how to use the installer directly.

> The installer is an internal tool and as such not expected to be used by those external to Khulnasoft.
...
- A [Kubernetes cluster configured](https://www.khulnasoft.com/docs/self-hosted/latest)
- A [TLS certificate](#tls-certificates)

Or, [open a Khulnasoft workspace](https://khulnasoft.com/from-referrer/)

The process to install Khulnasoft is:
...
Releases can be downloaded from [GitHub Releases](https://github.com/khulnasoft-com/khulnasoft/releases/)
Select your desired binary, download and install

1. Download the latest release with the command:

```shell
curl -fsSLO https://github.com/khulnasoft-com/khulnasoft/releases/latest/download/khulnasoft-installer-linux-amd64
```
...
```shell
curl -fsSLO https://github.com/khulnasoft-com/khulnasoft/releases/latest/download/khulnasoft-installer-linux-amd64.sha256
```

Validate the binary against the checksum file:

```shell
echo "$(<khulnasoft-installer-linux-amd64.sha256)" | sha256sum --check
```

If valid, the output is:

```shell:
khulnasoft-installer-linux-amd64: OK
```

3. Install the binary

```shell
sudo install -o root -g root khulnasoft-installer-linux-amd64 /usr/local/bin/khulnasoft-installer
```

4. Test to ensure the version you installed is up-to-date:

```shell
khulnasoft-installer version
```
...
khulnasoft-installer validate config --config khulnasoft.config.yaml
```
...
```shell
# Checks that your cluster is ready to install Khulnasoft
khulnasoft-installer validate cluster --kubeconfig ~/.kube/config --config khulnasoft.config.yaml
```
...
After a few minutes, your Khulnasoft installation will be available on the
specified `domain`.

## Uninstallation

The Installer generates a ConfigMap with the metadata of every Kubernetes
object generated by the Installer. This can be retrieved to remove Khulnasoft
from your cluster.

```shell
kubectl get configmaps khulnasoft-app -o jsonpath='{.data.app\.yaml}' \
...
The Khulnasoft Installer is designed as a way of providing you a robust and well-tested
framework for installing Khulnasoft to your own infrastructure. There may be times when
...
```shell
error: error validating "khulnasoft.yaml": error validating data: ValidationError(StatefulSet.status): missing required field "availableReplicas" in io.k8s.api.apps.v1.StatefulSetStatus; if you choose to ignore these errors, turn validation off with --validate=false
```
...
OpenVSX proxy (see [#8529](https://github.com/khulnasoft-com/khulnasoft/issues/8529)).
...
| `kind` | Y | Installation type to run - for most users, this will be `Full` | Available options are: <ul><li>`Meta`: To install the tools that make up the front-end facing side of `Khulnasoft` </li><li>`Workspace`: To install the components that make up the `Khulnasoft Workspaces`</li><li>`Full`: To install the complete setup, i.e. both `Meta` and `Workspace`</li> |
...
Khulnasoft must be connected to a Git provider. This can be done via the
...
Khulnasoft requires certain services for it to function correctly. The Installer
...
Khulnasoft is compatible with any registry that implements the [Docker Registry HTTP API V2](https://docs.docker.com/registry/spec/api/)
...
To configure Khulnasoft to use Amazon, you will need to use the in-cluster
...
Khulnasoft requires an instance of MySQL 5.7 for data storage.
...
Khulnasoft supports the following object storage providers:
...
- `khulnasoft.com/workload_meta`
- `khulnasoft.com/workload_ide`
- `khulnasoft.com/workload_services`
- `khulnasoft.com/workload_workspace_regular`
- `khulnasoft.com/workload_workspace_headless`
...
`ClusterIssuer` called `khulnasoft-issuer` - please change this if necessary.

This certificate is called `https-certificates` - please use that in your
Khulnasoft installer [config](#config).
...
By default, Khulnasoft will be installed to the `default`
[Kubernetes namespace](https://kubernetes.io/docs/concepts/overview/working-with-objects/namespaces).
To install to a different namespace, pass a `namespace` flag to the `render`
command.

```shell
khulnasoft-installer render --config khulnasoft.config.yaml --namespace khulnasoft > khulnasoft.yaml
```
...
```shell
khulnasoft-installer validate cluster --kubeconfig ~/.kube/config --config khulnasoft.config.yaml --namespace khulnasoft
```

**IMPORTANT**: this does not create the namespace, so you will need to create
that separately. This is so that uninstallation of Khulnasoft does not remove any
