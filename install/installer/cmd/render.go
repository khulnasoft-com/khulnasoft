...
import (
	...
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/components"
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/config"
	...
	"github.com/khulnasoft-com/khulnasoft/installer/pkg/postprocess"
...
var renderCmd = &cobra.Command{
	Use:   "render",
	Short: "Renders the Kubernetes manifests required to install Khulnasoft",
	Long: `Renders the Kubernetes manifests required to install Khulnasoft

A config file is required which can be generated with the init command.`,
	Example: `  # Default install.
  khulnasoft-installer render --config config.yaml | kubectl apply -f -

  # Install Khulnasoft into a non-default namespace.
  khulnasoft-installer render --config config.yaml --namespace khulnasoft | kubectl apply -f -`,
...
func init() {
	...
	renderCmd.PersistentFlags().StringVarP(&renderOpts.ConfigFN, "config", "c", getEnvvar("KHULNASOFT_INSTALLER_CONFIG", filepath.Join(dir, "khulnasoft.config.yaml")), "path to the config file, use - for stdin")
