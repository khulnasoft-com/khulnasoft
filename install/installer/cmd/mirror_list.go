...
image to the "target" repo`,
	Example: `
  khulnasoft-installer mirror list --config config.yaml > mirror.json

  # Pull original and push to target
  for row in $(khulnasoft-installer mirror list --config ./config.yaml | jq -c '.[]'); do
...
func init() {
	...
	mirrorListCmd.Flags().BoolVar(&mirrorListOpts.ExcludeThirdParty, "exclude-third-party", false, "exclude non-Khulnasoft images")
	mirrorListCmd.Flags().StringVarP(&mirrorListOpts.ConfigFN, "config", "c", os.Getenv("KHULNASOFT_INSTALLER_CONFIG"), "path to the config file")
...
func generateMirrorList(cfgVersion string, cfg *configv1.Config) ([]mirrorListRepo, error) {
	// Throw error if set to the default Khulnasoft repository
	if cfg.Repository == common.KhulnasoftContainerRegistry {
		return nil, fmt.Errorf("cannot mirror images to repository %s", common.KhulnasoftContainerRegistry)
	}

	// Get the target repository from the config
	targetRepo := strings.TrimRight(cfg.Repository, "/")

	// Use the default Khulnasoft registry to pull from
	cfg.Repository = common.KhulnasoftContainerRegistry
	...
	for _, img := range rawImages {
		...
		if img == common.KhulnasoftContainerRegistry {
			continue
		}
		// Ignore empty image
		if img == "" {
		...
		if strings.Contains(img, cfg.Repository) {
			// This is the Khulnasoft registry
...
// getGenericImages this is a bit brute force - anything starting "docker.io" or with Khulnasoft repo is found
// this will be in ConfigMaps and could be anything, so will need cleaning up
func getGenericImages(k8sObj string) []string {
	var images []string

	// Search for anything that matches docker.io or the Khulnasoft repo - docker.io needed for khulnasoft/workspace-full
	re := regexp.MustCompile(fmt.Sprintf("%s(.*)|%s(.*)", "docker.io", common.KhulnasoftContainerRegistry))
